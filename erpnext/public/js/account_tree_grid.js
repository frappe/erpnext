...
erpnext.AccountTreeGrid = class AccountTreeGrid extends frappe.views.TreeGridReport {
	...
	prepare_balances() {
		...
		$.each(this.data, function (i, v) {
			v.opening_dr = v.opening_cr = v.debit = v.credit = v.closing_dr = v.closing_cr = 0;
		});
	...
	update_balances(account, posting_date, v) {
		...
		if (posting_date < this.opening_date || v.is_opening === "Yes") {
			if (
				...
				var opening_bal =
					flt(account.opening_dr) - flt(account.opening_cr) + flt(v.debit) - flt(v.credit);
		...
		// closing
		var closing_bal =
			flt(account.opening_dr) - flt(account.opening_cr) + flt(account.debit) - flt(account.credit);
	...
	set_fiscal_year() {
		...
		$.each(frappe.report_dump.data["Fiscal Year"], function (i, v) {
			if (
				me.opening_date >= frappe.datetime.str_to_obj(v.year_start_date) &&
