...
frappe.ui.form.ItemQuickEntryForm = frappe.ui.form.QuickEntryForm.extend({
	...
	render_dialog: function(){
		...
		this.mandatory.splice(5, 0, {fieldname:'col_break1', fieldtype:'Column Break'});
	...
	get_variant_fields: function(){
		var variant_fields = [{fieldname:"create_variant", fieldtype:"Check", label:__("Create Variant")},
			{fieldname:'item_template', label:__('Item Template'), reqd: 0, fieldtype:'Link',
			options:"Item",
	...
	get_manufacturing_fields: function(){
		this.manufacturer_fields = [
			{fieldtype:'Link', options:'Manufacturer', label:'Manufacturer', fieldname:"manufacturer", 
				hidden:1, reqd:0},
	...
	get_attributes_fields: function () {
		var attribute_fields = [{fieldname:'sec_br', fieldtype:'Section Break'},
				{fieldname:'attribute_html', fieldtype:'HTML'}]
	...
	init_for_view_attributes: function(){
		...
		$(this.dialog.fields_dict.attribute_html.wrapper).find(".view-attributes").click( function() {
			var html = frappe.render_template("variant_attribute", {"attributes":me.attributes, "attribute_values":me.attribute_values});
	...
	render_attributes: function(attributes){
		...
		$.each(attributes, function(index, row){
			...
			$(repl(`<p class="help-box small text-muted hidden-xs">%(desc)s</p>`, {"desc":desc}))
	...
	get_variant_doc: function(){
		...
		if(Object.keys(attribute).length){
			frappe.call({
				...
				args:{"template":me.dialog.fields_dict.item_template.$input.val(), args:attribute},
				async:false,
				callback:function(r){
					if (Object.prototype.toString.call(r.message) == "[object Object]"){
						...
						msgprint_dialog = frappe.msgprint(__("Item Variant {0} already exists with same attributes",
							[repl('<a class="strong variant-click" data-item-code="%(item)s" \
								>%(item)s</a>', {
								item: r.message
							})]
						));
