...
class TestAsset(AssetSetup):
	...
	def test_gle_made_by_asset_sale_for_existing_asset(self):
		...
		asset = create_asset(
			...
			opening_booked_depreciations=2,
	...
	def test_asset_splitting(self):
		asset = create_asset(
			...
			opening_booked_depreciations=1,
			frequency_of_depreciation=10,
...
class TestDepreciationBasics(AssetSetup):
	...
	def test_opening_booked_depreciations(self):
		"""Tests if an error is raised when opening_booked_depreciations is not specified when opening_accumulated_depreciation is."""
	...
	def test_number_of_depreciations(self):
		"""Tests if an error is raised when opening_booked_depreciations >= total_number_of_depreciations."""

		# opening_booked_depreciations > total_number_of_depreciations
		asset = create_asset(
			item_code="Macbook Pro",
			calculate_depreciation=1,
			available_for_use_date="2019-12-31",
			...
			depreciation_start_date="2020-07-01",
			opening_accumulated_depreciation=10000,
			opening_booked_depreciations=5,
			do_not_save=1,
		)
	...
	def test_expected_value_change(self):
		...
		asset.opening_booked_depreciations = 1
	...
	asset = frappe.get_doc(
		{
			...
			"opening_booked_depreciations": args.opening_booked_depreciations or 0,
