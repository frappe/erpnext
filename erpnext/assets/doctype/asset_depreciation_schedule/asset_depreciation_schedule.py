...
class AssetDepreciationSchedule(Document):
	...
	def have_asset_details_been_modified(self, asset_doc):
		return (
			...
			or asset_doc.opening_booked_depreciations != self.opening_booked_depreciations
		)
	...
	def set_draft_asset_depr_schedule_details(self, asset_doc, row):
		...
		self.opening_booked_depreciations = asset_doc.opening_booked_depreciations or 0
	...
	def _make_depr_schedule(
		...
		final_number_of_depreciations = cint(row.total_number_of_depreciations) - cint(
			self.opening_booked_depreciations
		)
		...
		for n in range(start, final_number_of_depreciations):
			...
			if date_of_disposal and getdate(schedule_date) >= getdate(date_of_disposal):
				from_date = add_months(
					...
					(asset_doc.opening_booked_depreciations * row.frequency_of_depreciation),
				)
			...
			if (
				...
				if not is_first_day_of_the_month(getdate(asset_doc.available_for_use_date)):
					from_date = get_last_day(
						add_months(
							getdate(asset_doc.available_for_use_date),
							((self.opening_booked_depreciations - 1) * row.frequency_of_depreciation),
						)
					)
				else:
					from_date = add_months(
						getdate(add_days(asset_doc.available_for_use_date, -1)),
						(self.opening_booked_depreciations * row.frequency_of_depreciation),
					)
				depreciation_amount, days, months = _get_pro_rata_amt(
					row,
					depreciation_amount,
			...
			# For last row
				...
				if not asset_doc.flags.increase_in_asset_life:
					...
					asset_doc.to_date = add_months(
						...
						(n + self.opening_booked_depreciations) * cint(row.frequency_of_depreciation),
					)
...
def _check_is_pro_rata(asset_doc, row, wdv_or_dd_non_yearly=False):
	...
	# otherwise, if opening_booked_depreciations = 2, available_for_use_date = 01/01/2020 and frequency_of_depreciation = 12
...
def _get_modified_available_for_use_date(asset_doc, row, wdv_or_dd_non_yearly=False):
	if wdv_or_dd_non_yearly:
		return add_months(
			asset_doc.available_for_use_date,
			(asset_doc.opening_booked_depreciations * 12),
		)
	else:
		return add_months(
			asset_doc.available_for_use_date,
	...
	# if the Depreciation Schedule is being prepared for the first time
		...
		if row.daily_prorata_based:
			...
			return (
				...
				- flt(asset.opening_accumulated_depreciation)
				- flt(row.expected_value_after_useful_life)
			) / flt(row.total_number_of_depreciations - asset.opening_booked_depreciations)
