...
class AssetMovement_(Document):
	...
	def validate_asset_transfer(self, asset):
		...
		if not asset.target_location:
			frappe.throw(_("Target Location is required while transferring Asset {0}")
				.format(asset.asset))
	...
	def validate_asset_receipt(self, asset):
		...
		if not asset.source_location and not (asset.target_location or asset.to_employee):
			frappe.throw(_("Target Location or To Employee is required while receiving Asset {0}")
				.format(asset.asset))

		elif asset.source_location:
			if asset.target_location and not asset.from_employee:
				frappe.throw(_("From employee is required while receiving Asset {0} at a target location")
					.format(asset.asset))

			if asset.from_employee and not asset.target_location:
				frappe.throw(_("Target Location is required while receiving Asset {0} from an employee")
					.format(asset.asset))
