...
class DepreciationEntry(AccountsController):
	...
	def validate_reference_doctype(self):
		...
		if self.reference_doctype == "Asset Serial No" and not self.serial_no:
			frappe.throw(
				_("Reference Document Type cannot be {0} when Serial No has not been entered.").format(
	...
	def validate_reference_docname(self):
		if self.reference_doctype in ["Asset", "Asset Serial No"]:
			...
			if self.reference_docname != ideal_reference_docname:
				frappe.throw(
					_("Reference Document Name cannot be {0} when the {1} entered is {2}.").format(
						self.reference_docname, self.reference_doctype, ideal_reference_docname
					),
					title=_("Invalid Reference"),
				)
			...
			asset_linked_with_depr_schedule = frappe.get_value(
				"Depreciation Schedule", self.reference_docname, fieldname
			)
			asset_linked_with_depr_entry = self.get_asset_or_serial_no()

			if asset_linked_with_depr_schedule != asset_linked_with_depr_entry:
				frappe.throw(
					_("Depreciation Schedule {0} cannot be used here as it is linked with {1}, not {2}.").format(
						self.reference_docname, asset_linked_with_depr_schedule, asset_linked_with_depr_entry
					),
					title=_("Invalid Reference"),
				)
	...
	def get_finance_books_linked_with_asset(self, asset_or_serial_no):
		return frappe.get_all(
			"Asset Finance Book", filters={"parent": asset_or_serial_no}, pluck="finance_book"
		)
				...
				self.get_gl_dict(
					{
						...
						+ "_in_account_currency": flt(
							self.depreciation_amount, self.precision("depreciation_amount")
						),
