frappe.pages['telephony-dashboard'].on_page_load = function(wrapper) {
...
frappe.pages['customer-lookup'].refresh = function(wrapper) {
...
frappe.CallCenterConsole = Class.extend({
	...
	get_info: function() {
		...
		if (text.val() && !isNaN(text.val())) {
			...
			frappe.call({
				method: "refreshednow_erpnext.ccc_api.get_caller_info",
				args: { "caller_number": text.val().trim() },
				callback: function(r){
					if(r) {
						me.page.main.find("#cc_console").remove("#cc_console"); 
						//console.log(r.message)
						content = frappe.render_template("cc_console", {"info": r.message || null});
