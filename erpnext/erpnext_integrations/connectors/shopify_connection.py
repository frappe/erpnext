...
def validate_item(order, shopify_settings):
	for item in order.get("line_items"):
		if item.get("product_id") and not frappe.db.get_value(
			"Item", {"shopify_product_id": item.get("product_id")}, "name"
		):
...
def create_sales_order(shopify_order, shopify_settings, company=None):
	...
	customer = frappe.db.get_value(
		"Customer", {"shopify_customer_id": shopify_order.get("customer", {}).get("id")}, "name"
	)
	so = frappe.db.get_value("Sales Order", {"shopify_order_id": shopify_order.get("id")}, "name")

	if not so:
		items = get_order_items(
			shopify_order.get("line_items"), shopify_settings, getdate(shopify_order.get("created_at"))
		)
...
def create_sales_invoice(shopify_order, shopify_settings, so, old_order_sync=False):
	if (
		...
		and cint(shopify_settings.sync_sales_invoice)
	):
...
def get_item_code(shopify_item):
	item_code = frappe.db.get_value(
		"Item", {"shopify_variant_id": shopify_item.get("variant_id")}, "item_code"
	)
	if not item_code:
		item_code = frappe.db.get_value(
			"Item", {"shopify_product_id": shopify_item.get("product_id")}, "item_code"
		)
	if not item_code:
...
def get_tax_account_head(tax):
	...
	tax_account = frappe.db.get_value(
		"Shopify Tax Account", {"parent": "Shopify Settings", "shopify_tax": tax_title}, "tax_account"
	)
...
def get_url(shopify_settings):
	...
	if not last_order_id:
		if shopify_settings.sync_based_on == "Date":
			url = get_shopify_url(
				"admin/api/2021-04/orders.json?limit=250&created_at_min={0}&since_id=0".format(
					get_datetime(shopify_settings.from_date)
				),
				shopify_settings,
			)
		else:
			url = get_shopify_url(
				"admin/api/2021-04/orders.json?limit=250&since_id={0}".format(shopify_settings.from_order_id),
				shopify_settings,
			)
	else:
		url = get_shopify_url(
			"admin/api/2021-04/orders.json?limit=250&since_id={0}".format(last_order_id), shopify_settings
		)
