...
class MWS(object):
	...
	def __init__(
		self, access_key, secret_key, account_id, region="US", domain="", uri="", version=""
	):
	...
	def make_request(self, extra_data, method="GET", **kwargs):
		...
		request_description = "&".join(
			["%s=%s" % (k, quote(params[k], safe="-_.~")) for k in sorted(params)]
		)
...
class Feeds(MWS):
	...
	def submit_feed(
		self, feed, feed_type, marketplaceids=None, content_type="text/xml", purge="false"
	):
		"""
		...
		return self.make_request(
			data, method="POST", body=feed, extra_headers={"Content-MD5": md, "Content-Type": content_type}
		)
	...
	def get_feed_submission_count(
		...
		data = dict(Action="GetFeedSubmissionCount", SubmittedFromDate=fromdate, SubmittedToDate=todate)
		data.update(self.enumerate_param("FeedTypeList.Type.", feedtypes))
		data.update(self.enumerate_param("FeedProcessingStatusList.Status.", processingstatuses))
		return self.make_request(data)

	def cancel_feed_submissions(self, feedids=None, feedtypes=None, fromdate=None, todate=None):
		data = dict(Action="CancelFeedSubmissions", SubmittedFromDate=fromdate, SubmittedToDate=todate)
		data.update(self.enumerate_param("FeedSubmissionIdList.Id.", feedids))
		data.update(self.enumerate_param("FeedTypeList.Type.", feedtypes))
		return self.make_request(data)
...
class Reports(MWS):
	...
	def get_report_list(
		self, requestids=(), max_count=None, types=(), acknowledged=None, fromdate=None, todate=None
	):
		data = dict(
	...
	def get_report_request_count(
		...
		data = dict(Action="GetReportRequestCount", RequestedFromDate=fromdate, RequestedToDate=todate)
		data.update(self.enumerate_param("ReportTypeList.Type.", report_types))
		data.update(self.enumerate_param("ReportProcessingStatusList.Status.", processingstatuses))
		return self.make_request(data)

	def get_report_request_list(
		self, requestids=(), types=(), processingstatuses=(), max_count=None, fromdate=None, todate=None
	):
		data = dict(
	...
	def request_report(self, report_type, start_date=None, end_date=None, marketplaceids=()):
		data = dict(
			Action="RequestReport", ReportType=report_type, StartDate=start_date, EndDate=end_date
		)
		data.update(self.enumerate_param("MarketplaceIdList.Id.", marketplaceids))
		return self.make_request(data)
...
class Products(MWS):
	...
	def get_matching_product_for_id(self, marketplaceid, type, id):
		...
		"""
		data = dict(Action="GetMatchingProductForId", MarketplaceId=marketplaceid, IdType=type)
	...
	def get_product_categories_for_sku(self, marketplaceid, sku):
		data = dict(Action="GetProductCategoriesForSKU", MarketplaceId=marketplaceid, SellerSKU=sku)
		return self.make_request(data)

	def get_product_categories_for_asin(self, marketplaceid, asin):
		data = dict(Action="GetProductCategoriesForASIN", MarketplaceId=marketplaceid, ASIN=asin)
		return self.make_request(data)

	def get_my_price_for_sku(self, marketplaceid, skus, condition=None):
		data = dict(Action="GetMyPriceForSKU", MarketplaceId=marketplaceid, ItemCondition=condition)
		data.update(self.enumerate_param("SellerSKUList.SellerSKU.", skus))
		return self.make_request(data)

	def get_my_price_for_asin(self, marketplaceid, asins, condition=None):
		data = dict(Action="GetMyPriceForASIN", MarketplaceId=marketplaceid, ItemCondition=condition)
		data.update(self.enumerate_param("ASINList.ASIN.", asins))
		return self.make_request(data)
...
class Recommendations(MWS):
	...
	def get_last_updated_time_for_recommendations(self, marketplaceid):
		...
		data = dict(Action="GetLastUpdatedTimeForRecommendations", MarketplaceId=marketplaceid)
		return self.make_request(data, "POST")
	...
	def list_recommendations_by_next_token(self, token):
		...
		data = dict(Action="ListRecommendationsByNextToken", NextToken=token)
		return self.make_request(data, "POST")
...
class Finances(MWS):
	...
	def list_financial_events(
		self, posted_after=None, posted_before=None, amazon_order_id=None, max_results="100"
	):
