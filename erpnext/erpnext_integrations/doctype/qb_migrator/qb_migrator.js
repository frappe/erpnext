...
frappe.ui.form.on('QB Migrator', {
	...
	onload: function(frm) {
		...
		var redirect_url = `${domain}/api/method/erpnext.erpnext_integrations.doctype.qb_migrator.qb_migrator.callback`
		...
		frm.trigger("set_indicator")
 		frappe.realtime.on("qb_progress_update", function (data) {
	...
	refresh: function(frm) {
		...
		if (!frm.doc.access_token) {
			// Unset access_token signifies that we don't have enough information to connect to qb api and fetch data
			if (frm.doc.authorization_url) {
				frm.add_custom_button(__("Connect to QB"), function () {
					frm.trigger("connect")
				});
			}
		}
		if (frm.doc.access_token) {
			...
			frm.remove_custom_button(__("Connect to QB"))
	...
	set_indicator: function(frm) {
		var indicator_map = {
			"Connecting to QB": [__("Connecting to QB"), "orange"],
			"Connected to QB": [__("Connected to QB"), "green"],
