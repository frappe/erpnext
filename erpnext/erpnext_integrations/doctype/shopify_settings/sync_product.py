...
def sync_item_from_shopify(shopify_settings, item):
	url = get_shopify_url(
		"admin/api/2021-04/products/{0}.json".format(item.get("product_id")), shopify_settings
	)
...
def has_variants(shopify_item):
	if (
		len(shopify_item.get("options")) >= 1
		and "Default Title" not in shopify_item.get("options")[0]["values"]
	):
		return True
	return False
		...
		for variant in shopify_item.get("variants"):
			...
			for i, variant_attr in enumerate(shopify_variants_attr_list):
				if variant.get(variant_attr):
					attributes[i].update(
						{"attribute_value": get_attribute_value(variant.get(variant_attr), attributes[i])}
					)
...
def add_to_price_list(item, name):
	shopify_settings = frappe.db.get_value(
		"Shopify Settings", None, ["price_list", "update_price_in_erpnext_price_list"], as_dict=1
	)
...
def is_item_exists(shopify_item, attributes=None, variant_of=None):
	...
	if name:
		...
		if item.shopify_product_id and attributes and attributes[0].get("attribute_value"):
			...
			conditions = [
				"(iv.attribute='{0}' and iv.attribute_value = '{1}')".format(
					attr.get("attribute"), attr.get("attribute_value")
				)
				for attr in attributes
			]

			conditions = "( {0} ) and iv.parent = it.name ) = {1}".format(
