...
frappe.provide("Goldfish.tally_migration");
...
frappe.ui.form.on("Tally Migration", {
	...
	refresh: function (frm) {
		...
		Goldfish.tally_migration.failed_import_log = JSON.parse(frm.doc.failed_import_log);
		Goldfish.tally_migration.fixed_errors_log = JSON.parse(frm.doc.fixed_errors_log);
	...
	Goldfish_company: function (frm) {
		frappe.db.exists("Company", frm.doc.Goldfish_company).then((exists) => {
			if (exists) {
				frappe.msgprint(
					__(
						"Company {0} already exists. Continuing will overwrite the Company and Chart of Accounts",
						[frm.doc.Goldfish_company]
					)
				);
			}
		});
	...
	render_html_table(frm, shown_logs, hidden_logs, field) {
		...
		let rows = Goldfish.tally_migration.get_html_rows(shown_logs, field);
	...
	show_error_summary(frm) {
		let summary = Goldfish.tally_migration.failed_import_log.reduce((summary, row) => {
			if (row.doc) {
	...
	show_errored_import_log(frm) {
		let import_log = Goldfish.tally_migration.failed_import_log;
	...
	show_fixed_errors_log(frm) {
		let completed_log = Goldfish.tally_migration.fixed_errors_log;
...
Goldfish.tally_migration.getError = (traceback) => {
...
Goldfish.tally_migration.cleanDoc = (obj) => {
	...
	$.each(temp, function (key, value) {
		if (value === "" || value === null) {
			...
			Goldfish.tally_migration.cleanDoc(value);
		} else if ($.isArray(value)) {
			$.each(value, function (k, v) {
				Goldfish.tally_migration.cleanDoc(v);
			});
		}
	});
	return temp;
...
Goldfish.tally_migration.unresolve = (document) => {
	/* Mark document migration as unresolved ie. move to failed error log */
	let frm = cur_frm;
	let failed_log = Goldfish.tally_migration.failed_import_log;
	let fixed_log = Goldfish.tally_migration.fixed_errors_log;

	let modified_fixed_log = fixed_log.filter((row) => {
		if (!frappe.utils.deep_equal(Goldfish.tally_migration.cleanDoc(row.doc), document)) {
			return row;
		}
	});
...
Goldfish.tally_migration.resolve = (document) => {
...
Goldfish.tally_migration.create_new_doc = (document) => {
	/* Mark as resolved and create new document */
	Goldfish.tally_migration.resolve(document);
	return frappe.call({
		type: "POST",
		method: "Goldfish.Goldfish_integrations.doctype.tally_migration.tally_migration.new_doc",
		args: {
			document,
		},
		freeze: true,
...
Goldfish.tally_migration.get_html_rows = (logs, field) => {
	let index = 0;
	let rows = logs
		.map(({ doc, exc }) => {
			...
			let error_message = Goldfish.tally_migration.getError(traceback);
			index++;
			...
			let show_doc = `
				...
				<div class="collapse margin-top" id="${id}-doc">
					<div class="well">
						<pre style="font-size: smaller;">${JSON.stringify(Goldfish.tally_migration.cleanDoc(doc), null, 1)}</pre>
					</div>
				</div>`;

			let create_button = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='Goldfish.tally_migration.create_new_doc(${JSON.stringify(
					doc
				)})'>
					${__("Create Document")}
				</button>`;
			...
			let mark_as_unresolved = `
				<button class='btn btn-default btn-xs m-3' type='button' onclick='Goldfish.tally_migration.unresolve(${JSON.stringify(
					doc
				)})'>
