...
def get_url_for_whatsapp(doctype, name):
    doc = frappe.get_doc(doctype, name)
    return quote("{url}/api/method/erpnext.erpnext_integrations.doctype.whatsapp_settings.whatsapp_settings.get_pdf_for_whatsapp?doctype={doctype}&name={name}&key={key}".format(
