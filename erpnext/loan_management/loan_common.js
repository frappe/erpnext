...
frappe.ui.form.on(cur_frm.doctype, {
	refresh: function (frm) {
		if (
			["Loan Disbursement", "Loan Repayment", "Loan Interest Accrual", "Loan Write Off"].includes(
				frm.doc.doctype
			) &&
			frm.doc.docstatus > 0
		) {
	...
	applicant: function (frm) {
		...
		if (frm.doc.applicant) {
			frappe.model.with_doc(frm.doc.applicant_type, frm.doc.applicant, function () {
				...
				frm.set_value("applicant_name", applicant.employee_name || applicant.member_name);
			});
		} else {
