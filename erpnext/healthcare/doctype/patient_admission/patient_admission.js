...
var cancel_schedule = function(frm){
	...
	var d = new frappe.ui.Dialog({
		...
		primary_action : function(){
			frappe.call({
				"method": "erpnext.healthcare.doctype.patient_admission.patient_admission.cancel_scheduled_admission",
...
var admit_patient = function(frm){
	...
	var admit_and_allocate = function(date_in,time_in,bed_number,facility_type,facility_name,expected_discharge){
		frappe.call({
			"method": "erpnext.healthcare.doctype.patient_admission.patient_admission.admit_and_allocate_patient",
...
var allocate_patient = function(frm){
	...
	var allocate = function(date_in,time_in,bed_number,facility_type,facility_name,expected_discharge){
		frappe.call({
			"method": "erpnext.healthcare.doctype.patient_admission.patient_admission.allocate_facility",
			"args": {patient: doc.patient, admission: doc.name,
				date_in:date_in, time_in:time_in, bed: bed_number,
				 facility_type: facility_type, facility: facility_name,
...
var facility_transfer_allocate = function(frm){
	...
	var transfer_and_allocate = function(bed_number,facility_type,facility_name,expected_discharge){
		frappe.call({
			"method": "erpnext.healthcare.doctype.patient_admission.patient_admission.facility_transfer_allocation",
...
var queue_discharge_patient = function(frm){
	...
	var d = new frappe.ui.Dialog({
		...
		primary_action : function(){
			frappe.call({
				"method": "erpnext.healthcare.doctype.patient_admission.patient_admission.queue_discharge_patient",
				"args": {patient: doc.patient, admission: doc.name},
				callback: function(r){
					cur_frm.reload_doc();
				}
...
var discharge_patient = function(frm){
	...
	var d = new frappe.ui.Dialog({
		...
		primary_action : function(){
			frappe.call({
				"method": "erpnext.healthcare.doctype.patient_admission.patient_admission.discharge_patient",
				"args": {patient: doc.patient, admission: doc.name},
				callback: function(r){
					cur_frm.reload_doc();
				}
...
var btn_create_consultation = function(frm){
	var doc = frm.doc;
	frappe.call({
		method:"erpnext.healthcare.doctype.patient_admission.patient_admission.create_consultation",
		args: {admission:doc.name},
...
var btn_create_inv_for_facility_used = function(frm){
	var doc = frm.doc;
	frappe.call({
		method:"erpnext.healthcare.doctype.patient_admission.patient_admission.create_inv_for_facility_used",
		args: {admission:doc.name},
		callback: function(data){
			if(!data.exc){
				var doclist = frappe.model.sync(data.message);
...
var btn_discharge_summary = function(frm){
	var doc = frm.doc;
	frappe.call({
		method:"erpnext.healthcare.doctype.patient_admission.patient_admission.create_discharge_summary",
		args: {admission:doc.name},
		callback: function(data){
			if(!data.exc){
