...
def get_patient_detail(patient, company=None):
	patient_dict = frappe.db.sql(("""select * from tabPatient where name='{0}'""").format(patient), as_dict=1)
	if not patient_dict:
		frappe.throw("Patient not found")
	vital_sign = frappe.db.sql(("""select * from `tabVital Signs` where patient='{0}' order by signs_date desc limit 1""").format(patient), as_dict=1)
