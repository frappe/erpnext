...
class TestFeeValidity(unittest.TestCase):
	def test_fee_validity(self):
		...
		appointment = create_appointment(patient, physician, getdate("21/01/2017"))
		...
		appointment = create_appointment(patient, physician, getdate("25/01/2017"))
		invoice = frappe.db.get_value("Patient Appointment", appointment.name, "sales_invoice")
		self.assertTrue(invoice)
		appointment = create_appointment(patient, physician, getdate("29/01/2017"))
		invoice = frappe.db.get_value("Patient Appointment", appointment.name, "sales_invoice")
		self.assertEqual(invoice, None)
