...
frappe.medical.ServiceDesk = Class.extend({
	...
	get_data: function (menu_active) {
		...
		return frappe.call({
			method: "erpnext.healthcare.page.service_desk.service_desk.get_patient_services_info",
	...
	create_invoice: function (div_id, prescriptions, doc_ids) {
		...
		if(div_id === "drug"){
			frappe.call({
				method: "erpnext.healthcare.doctype.consultation.consultation.create_drug_invoice",
			...
			frappe.call({
				method: "erpnext.healthcare.doctype.appointment.appointment.create_invoice",
	...
	create_document: function (doc, line, template, invoice) {
		...
		if (doc === "Lab Test") {
			frappe.call({
				method: "erpnext.healthcare.doctype.lab_test.lab_test.create_lab_test_from_desk",
