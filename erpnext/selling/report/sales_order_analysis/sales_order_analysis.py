...
def get_data(filters):
	...
	query = (
		...
		.select(
			...
			so.customer,
			soi.item_code,
			delay_days.as_("delay_days"),
			Case().when(so.status.isin(["Completed", "To Bill"]), 0).else_(delay_days).as_("delay"),
			soi.qty,
			soi.delivered_qty,
			(soi.qty - soi.delivered_qty).as_("pending_qty"),
			Sum(IfNull(sii.qty, 0)).as_("billed_qty"),
			soi.base_amount.as_("amount"),
			(soi.delivered_qty * soi.base_rate).as_("delivered_qty_amount"),
			(soi.billed_amt * IfNull(so.conversion_rate, 1)).as_("billed_amount"),
			(soi.base_amount - (soi.billed_amt * IfNull(so.conversion_rate, 1))).as_("pending_amount"),
	...
	if show_party_name():
		...
		query = (
			...
			.on(customer.name == so.customer)
			.select(customer.customer_name.as_("customer_name"))
		)
