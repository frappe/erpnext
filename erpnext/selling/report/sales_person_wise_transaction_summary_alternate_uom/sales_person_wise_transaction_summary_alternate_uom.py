	...
	for d in entries:
		if d.stock_qty > 0 or filters.get('show_return_entries', 0):
			data.append([
				...
				d.stock_qty, 
				d.alternate_qty, d.alternate_uom, d.weight, d.weight_uom,
	...
	entries = frappe.db.sql("""
		...
		CASE 
			...
			(  Select uom from `tabUOM Conversion Detail` 
			 where parent =  dt_item.item_code and is_alternate_uom = 1)
			ELSE (  Select uom from `tabUOM Conversion Detail` 
			 where parent =  dt_item.item_code and is_alternate_uom = 1)
			End as alternate_uom,	

		CASE 
			WHEN dt.status = "Closed" THEN 
			 (Select   dt_item.%s * weight_per_unit from `tabItem` where item_code = dt_item.item_code)
            ELSE ( Select dt_item.stock_qty * weight_per_unit from `tabItem` where item_code = dt_item.item_code)
			End as weight,
				

		CASE 
			WHEN dt.status = "Closed" THEN 
            (Select weight_uom from `tabItem` where item_code = dt_item.item_code )
			ELSE (Select weight_uom from `tabItem` where item_code = dt_item.item_code )
            End as weight_uom, 
		...
		""" %(date_field, qty_field,qty_field,qty_field, qty_field, qty_field, filters["doc_type"], filters["doc_type"], '%s', conditions),
