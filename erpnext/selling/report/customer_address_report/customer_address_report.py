...
field_map = {
	...
	"Address": [ "address_title", "address_line1", "address_line2", "city", "state", "pincode", "country", "fax", "tax_category", "gstin", "gst_state", "is_primary_address", "is_shipping_address" ]
}
...
def get_columns(filters):
	...
	return [
		...
		"Party GSTIN",
		"GST State",
		"Is Billing Address:Check",
		"Is Shipping Address:Check",
		"First Name",
		"Last Name",
		"Phone",
		"Mobile No",
...
def get_party_addresses_and_contact(party_type, party, party_group, filters):
	...
	for party, details in iteritems(party_details):
		party_detail_lst = []
		if fil.get("party_type") == "Customer":
			party_detail_lst = frappe.get_list(party_type, filters = {"name": party}, fields=["customer_type","territory", "default_currency", "default_price_list",
										"customer_primary_contact", "payment_terms", "industry", "market_segment"], as_list=True)

		if fil.get("party_type") == "Supplier":
			party_detail_lst = frappe.get_list(party_type, filters={"name": party}, fields=["supplier_type", "territory", "default_currency",
									"default_price_list", "primary_contact", "payment_terms", "industry", "market_segment"], as_list=True)
		addresses = details.get("address", [])
		contacts = details.get("contact", [])
		if not any([addresses, contacts]):
			result = [party]
			result.append(party_groups[party])
			result.extend(list(party_detail_lst[0]))
