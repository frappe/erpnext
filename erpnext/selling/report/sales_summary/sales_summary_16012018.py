...
def get_data():
	...
	where sii.parent=si.name and (DAY(si.posting_date) = DAY(NOW()- INTERVAL 1 DAY) and MONTH(si.posting_date) = MONTH(NOW()- INTERVAL 1 DAY) and YEAR(si.posting_date) = YEAR(NOW()- INTERVAL 1 DAY)) and si.is_return = 0 
	...
	from `tabSales Invoice` si,
	`tabSales Invoice Item` sii
	where sii.parent=si.name and (DAY(si.posting_date) = DAY(NOW()- INTERVAL 7 DAY) and MONTH(si.posting_date) = MONTH(NOW()- INTERVAL 7 DAY) and YEAR(si.posting_date) = YEAR(NOW()- INTERVAL 7 DAY)) and si.is_return = 0 
	group by sii.item_group,si.sales_channel
	""")
	...
	from `tabSales Invoice` si,
	`tabSales Invoice Item` sii
	where sii.parent=si.name and (MONTH(si.posting_date) = MONTH(NOW()) and YEAR(si.posting_date) = YEAR(NOW())) and si.is_return = 0 
	group by sii.item_group,si.sales_channel
	""")
