...
def get_data():
	...
	where sii.parent=si.name and si.creation > NOW()- INTERVAL 1 DAY and si.is_return = 0 
	...
	from `tabSales Invoice` si,
	`tabSales Invoice Item` sii
	where sii.parent=si.name and si.creation > NOW()- INTERVAL 7 DAY and si.is_return = 0 
	group by sii.item_group,si.sales_channel
	""")
	...
	from `tabSales Invoice` si,
	`tabSales Invoice Item` sii
	where sii.parent=si.name and si.creation > NOW()- INTERVAL 30 DAY and si.is_return = 0 
	group by sii.item_group,si.sales_channel
	""")
