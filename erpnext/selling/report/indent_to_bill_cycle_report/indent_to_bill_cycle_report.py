...
def get_columns(filters):
	columns = [
		...
		{
			"label": _("Purchase Receipt Accepted Quantity"),
			"fieldname": "purchase_receipt_accepted_quantity",
			"fieldtype": "Float",
			"width": 140
		},
		{
			"label": _("Purchase Receipt Amount"),
			"fieldname": "purchase_receipt_amt",
			"fieldtype": "Float",
			"width": 140
		},
		{
		...
		{
			"label": _("Purchase Invoice Quantity"),
			"fieldname": "purchase_invoice_quantity",
			"fieldtype": "Float",
			"width": 140
		},
		{
...
def get_data(filters):
	...
	for po in purchase_order_entry:
		...
		procurement_detail = {
			...
			"actual_delivery_date": po.pr_posting_date,#pr_records.get(po.name),
			"purchase_receipt": po.pr_name,
			"purchase_receipt_date": po.pr_posting_date,
			"purchase_receipt_accepted_quantity": po.pri_accepted_qty,
			"purchase_receipt_amt": po.pri_item_amount,
			"purchase_invoice": po.pi_name,
			"purchase_invoice_date": po.pi_posting_date,
			"purchase_invoice_quantity": po.pii_qty ,
			"purchase_invoice_amt": po.pri_item_amount
		}
...
def get_mapped_pi_records():
	return frappe._dict(frappe.db.sql("""
		...
		WHERE
			pi_item.docstatus = 1
			AND po.status not in ("Closed","Draft","Cancelled")
...
def get_mapped_pr_records():
	return frappe._dict(frappe.db.sql("""
		...
		WHERE
			...
			AND pr.status not in  ("Closed","Draft","Cancelled")
		"""))
...
def get_po_entries(conditions):
	return frappe.db.sql("""
		SELECT
			...
			parent.name,
			pr.name as pr_name,
			pr.posting_date as pr_posting_date,
			pri.qty as pri_accepted_qty,
			pri.amount as pri_item_amount,
			pri.item_code as pri_item_code,
			pii.qty as pii_qty,
			pi.name as pi_name,
		...
		`tabPurchase Invoice Item` pii ,`tabPurchase Invoice` pi
		WHERE
			parent.docstatus = 1
			AND parent.name = child.parent
			AND parent.status not in  ("Closed","Cancelled")
			AND child.material_request=pri.material_request 
			AND child.item_code=pri.item_code 
			AND parent.name=pii.purchase_order 
			AND child.item_code=pii.item_code 
