...
def get_columns(filters, period_list, partner_doctype):
	...
	if filters.get("target_on") in ['Stock Qty', 'Contents Qty']:
...
def prepare_data(filters, sales_users_data, actual_data, date_field, period_list, sales_field):
	...
	target_qty_amt_field = get_target_qty_amt_field(filters.get("target_on"))
	qty_or_amount_field = get_transaction_qty_amt_field(filters.get("target_on"))
...
def get_actual_data(filters, item_groups, sales_users_or_territory_data, date_field, sales_field):
	...
	return frappe.db.sql(""" SELECT `tab{child_doc}`.item_group,
			`tab{child_doc}`.stock_qty, `tab{child_doc}`.alt_uom_qty, `tab{child_doc}`.base_net_amount,
		...
		WHERE
			...
			and `tab{parent_doc}`.{date_field} between %s and %s"""  # nosec
			.format(
				cond = cond,
...
def get_target_qty_amt_field(target_on):
	if target_on == "Stock Qty":
		return "target_qty"
	elif target_on == "Contents Qty":
		return "target_alt_uom_qty"
	else:
		return "target_amount"


def get_transaction_qty_amt_field(target_on):
	if target_on == "Stock Qty":
		return "stock_qty"
	elif target_on == "Contents Qty":
		return "alt_uom_qty"
