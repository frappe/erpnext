...
class DocType(TransactionBase):
	...
	def get_other_charges(self,obj,default = 0):
		...
		if not getlist(obj.doclist, 'other_charges'):
			...
			idx = 0
			other_charge = webnotes.conn.sql("select t1.charge_type,t1.row_id,t1.description,t1.account_head,t1.rate,t1.tax_amount,t1.included_in_print_rate, t1.cost_center from `tabRV Tax Detail` t1, `tabOther Charges` t2 where t1.parent = t2.name and t2.company = '%s' and %s order by t1.idx" % (obj.doc.company, add_cond), as_dict = 1)
