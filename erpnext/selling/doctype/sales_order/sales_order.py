...
class SalesOrder(SellingController):
	...
	def validate_po(self):
		...
		if self.po_date and not self.skip_delivery_note_creation:
			for d in self.get("items"):
				if d.delivery_date and getdate(self.po_date) > getdate(d.delivery_date):
					frappe.throw(_("Row #{0}: Expected Delivery Date cannot be before Purchase Order Date")
						.format(d.idx))

		if self.po_no and self.customer and not self.skip_delivery_note_creation:
	...
	def validate_delivery_date(self):
		if self.order_type == 'Sales' and not self.skip_delivery_note_creation:
...
def get_events(start, end, filters=None):
	...
	data = frappe.db.sql("""
		...
		where `tabSales Order`.name = `tabSales Order Item`.parent
			and `tabSales Order`.skip_delivery_note_creation = 0
