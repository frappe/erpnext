...
erpnext.SalesFunnel = class SalesFunnel {
	...
	setup(wrapper) {
		...
		this.elements.funnel_wrapper = $('<div class="funnel-wrapper text-center" style="margin-top:50px !important"></div>')
			.appendTo(this.elements.layout);
		...
		if (me.options.chart == 'sales_funnel') {
			...
			frappe.call({
				...
				callback: function (r) {
					debugger
					me.options.data = r.message;
					if (r.message.datasets == 0 || r.message == "empty") {
						const $parent = me.elements.funnel_wrapper;
						$parent.html(__('No data for this period'));
					} else {
						me.render_pipeline();
	...
	render_opp_by_lead_source() {
		...
		this.chart = new frappe.Chart(parent, {
			title: 'Sales Opportunities by Source',
			height: 400,
			data: chart_data,
			type: 'bar',
			barOptions: {
	...
	render_pipeline() {
		...
		const chart_data = me.options.data ? me.options.data : null;
		debugger

		const parent = me.elements.funnel_wrapper[0];
		const chart = new frappe.Chart(parent, {
