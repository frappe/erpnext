...
class Payment {
	...
	set_primary_action() {
		...
		this.dialog.set_primary_action(__("Submit"), function() {
			...
			//var out_amt=me.dialog.field_dict.outstanotnding_amount.get_value();
			//var out_amt=me.dialog.get_value();
			var pos_data=get_Post_Data();
			/**console.log("outstanding_amount---------------------------------------",me.frm.doc.outstanding_amount);
			console.log("pos_data---------------------------------------",pos_data);
			console.log("pos_profile_name---------------------------------------",me.frm.parent);**/
			
			var out_amt=me.frm.doc.outstanding_amount;
			var write_of_amt_from_pos=me.frm.doc.write_off_amount;
			var outstanding_amount_has_to_be_0=pos_data[0].outstanding_amount_has_to_be_0;
	var  write_off_amount_can_not_be_more_than_positive_or_negative=pos_data[0].write_off_amount_can_not_be_more_than_positive_or_negative;
			var write_off__amount=pos_data[0].write_off__amount;
			var flag=0;
...
function get_Post_Data() {
    var pos_data = [];
    frappe.call({
        method: "erpnext.selling.page.point_of_sale.point_of_sale.get_pos_data",
        async: false,
        callback: function(r) {
            console.log("reportData::" + JSON.stringify(r.message));
	var outstanding_amount_has_to_be_0=r.message[0].outstanding_amount_has_to_be_0;
	var  write_off_amount_can_not_be_more_than_positive_or_negative=r.message[0].write_off_amount_can_not_be_more_than_positive_or_negative;
	var write_off__amount=r.message[0].write_off__amount;
	console.log("write_off_amount::"+write_off__amount);
            pos_data.push(
			{"outstanding_amount_has_to_be_0":outstanding_amount_has_to_be_0,
			"write_off_amount_can_not_be_more_than_positive_or_negative":write_off_amount_can_not_be_more_than_positive_or_negative,
			"write_off__amount":write_off__amount	
					}
