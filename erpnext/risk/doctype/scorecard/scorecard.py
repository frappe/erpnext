...
from erpnext.risk.doctype.scorecard_period.scorecard_period import make_scorecard
...
class Scorecard(Document):
	...
	def calculate_total_score(self):
		...
		for scp in scorecards:
			my_sc = frappe.get_doc("Scorecard Period", scp.name)
		...
		if total_max_score > 0:
			self.party_score = round(100.0 * (total_score / total_max_score), 1)
		else:
			self.party_score = 100

	def update_standing(self):
		# Get the setup document

		for standing in self.standings:
			if (not standing.min_grade or (standing.min_grade <= self.party_score)) and (
				not standing.max_grade or (standing.max_grade > self.party_score)
			):
				self.status = standing.standing_name
				self.indicator_color = standing.standing_color
				self.notify_party = standing.notify_party
				self.notify_employee = standing.notify_employee
				self.employee_link = standing.employee_link

				# Update party standing info
				for fieldname in ("prevent_pos", "prevent_rfqs", "warn_rfqs", "warn_pos"):
					self.set(fieldname, standing.get(fieldname))
					frappe.db.set_value(self.party_type, self.party, fieldname, self.get(fieldname))
...
def get_timeline_data(doctype, name):
	...
	for sc in scorecards:
		start_date, end_date, total_score = frappe.db.get_value(
			"Scorecard Period", sc.name, ["start_date", "end_date", "total_score"]
		)
...
def refresh_scorecards():
	...
	for sc in scorecards:
		...
		if make_all_scorecards(sc.name) > 0:
			...
			frappe.get_doc("Scorecard", sc.name).save()


@frappe.whitelist()
def make_all_scorecards(docname):

	sc = frappe.get_doc("Scorecard", docname)
	party = frappe.get_doc(sc.party_type, sc.party)

	start_date = getdate(party.creation)
	...
	while (start_date < todays) and (end_date <= todays):
		...
		if len(scorecards) == 0:
			period_card = make_scorecard(docname, None)
	...
	if scp_count > 0:
		frappe.msgprint(
			_("Created {0} scorecards for {1} between:").format(scp_count, sc.party)
			+ " "
...
def make_default_records():
	...
	for d in install_variable_docs:
		try:
			d["doctype"] = "Scorecard Variable"
			frappe.get_doc(d).insert()
		except frappe.NameError:
			pass
	for d in install_standing_docs:
		try:
			d["doctype"] = "Scorecard Standing"
			frappe.get_doc(d).insert()
		except frappe.NameError:
			pass
