...
def get_columns(filters):
	...
	if filters.purpose == "Material Issue":
		...
		cols.append(("Issued Equipment Name")+":Data:170")
...
def get_data(filters):
	...
	query = (qb.from_(se)
				...
				.select(se.posting_date,sed.item_code,sed.item_name,i.item_group,i.item_sub_group,sed.uom,sed.qty,sed.valuation_rate,sed.amount,sed.s_warehouse,se.name)
	...
	if filters.purpose == 'Material Transfer':
		...
		query = (query
				.select(sed.cost_center, sed.issue_to, sed.issued_employee_name, sed.issued_equipment_name)
