...
def get_columns():
	...
	columns = [
		...
		_("Purchase UOM Conversion Factor")+":Float:90",
		_("Purchase UOM Qty")+":Float:90",
		_("Sales UOM")+":Link/UOM:90",
		_("Sales UOM Conversion Factor")+":Float:90",
		_("Sales UOM Qty")+":Float:90",
	...
	return frappe.db.sql("""
		select
            ...
            (SELECT uomc.conversion_factor
             ...
             WHERE uomc.parent = sle.item_code
                AND itm.name = sle.item_code
                AND uomc.uom = itm.sales_uom) sales_uom_conversion_factor
		from
...
def get_item_details(items, sle, filters):
	...
	if items:
		for item in frappe.db.sql("""
			...
			from `tabItem` itm
			where itm.name in ({0}) and ifnull(disabled, 0) = 0
