...
class TrackedFIFO:
	...
	def __init__(self, fifo, ledger_entries, candidates):
		...
		self.entries = TrackedFIFOEntries(fifo, ledger_entries, candidates)
	...
	def get_summary_query(self, query, serial):
		...
		progress_query = frappe.qb.from_(events).select(
			...
			.over(events.name, events.warehouse, events.posting_datetime, events.creation, events.entry_name)
			.as_("row_qty"),
		)
		if not serial:
		...
		return (
			...
			.select(
				progress.name,
				progress.warehouse,
				Count(Function("row", progress.posting_datetime, progress.creation, progress.entry_name))
				.distinct()
				.as_("row_count"),
	...
	def get_dates_query(query):
		...
		order = Function("row", events.posting_datetime, events.creation, events.entry_name)
		return (
			frappe.qb.from_(events)
