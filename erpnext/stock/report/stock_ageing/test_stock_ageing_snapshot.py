...
from erpnext.stock.report.stock_report_snapshot import LIVE_TABLES, StockReportSnapshot, arrow_schema
...
class TestStockAgeingSnapshot(ERPNextTestSuite):
	...
	def test_reused_voucher_uses_replay(self):
		rows = self.make_rows(0)
		for row in rows:
			row.update(name="Item", warehouse="Warehouse")
		rows[1].voucher_no = rows[0].voucher_no
		self.assertIsNone(self.calculate(rows))

	def test_tied_timestamps_are_ordered_by_entry_name(self):
		rows = self.make_rows(0)
		for row in rows:
			row.update(name="Item", warehouse="Warehouse")
		rows[1].update(posting_datetime=rows[0].posting_datetime, creation=rows[0].creation)
		self.assertEqual(self.calculate(rows), self.replay(rows))
	...
	def test_tied_tracked_timestamps_are_ordered_by_entry_name(self):
		for serial in (False, True):
			with self.subTest(serial=serial):
				rows = self.tracked_rows([(1, 100, "A"), (1, 120, "B"), (-1, -100, "A")], serial)
				rows[1].update(posting_datetime=rows[0].posting_datetime, creation=rows[0].creation)
				self.assert_tracked_matches(rows, batchwise={"A": 1, "B": 1})
	...
	def count_split_parameters(self, rows):
		...
		def record(query):
			...
			if LAYER_KEYS in sql or RESOLVED_KEYS in sql:
	...
	def calculate(
		...
		fifo = self.make_fifo(rows, method, batchwise, SnapshotFIFOSlots)
		with duckdb.connect(":memory:") as conn:
			...
			snapshot.conn, snapshot.filters, snapshot.temp_directory = conn, {}, None
			snapshot.tables = self.live_tables(batchwise, serial_bundles, batch_bundles)
			frappe.local.stock_report_snapshot = snapshot
			ledger = frappe.qb.DocType("Stock Ledger Entry")
			fields = [ledger[field] for field in DETAIL_FIELDS if field != "name"]
			query = frappe.qb.from_(ledger).select(ledger.item_code.as_("name"), *fields)
			with patch.object(
				fifo,
				...
				with patch.object(
					...
					try:
						...
						frappe.local.stock_report_snapshot = None
		return None
	...
	def live_tables(self, batchwise, serial_bundles, batch_bundles):
		flags, entries = dict(batchwise or {}), []
		for parent, serials in (serial_bundles or {}).items():
			for index, serial in enumerate(serials, 1):
				entries.append({"parent": parent, "idx": index, "serial_no": serial, "docstatus": 1})
		for parent, batches in (batch_bundles or {}).items():
			for index, (batch, flag, qty, value) in enumerate(batches, 1):
				entries.append(
					{
						"parent": parent,
						"idx": index,
				...
				flags[batch] = flag
		batches = [{"name": name, "use_batchwise_valuation": flag} for name, flag in flags.items()]
		return {
			"tabSerial and Batch Entry": self.live_table("Serial and Batch Entry", entries),
			"tabBatch": self.live_table("Batch", batches),
		}

	@staticmethod
	def live_table(doctype, rows):
		return pa.Table.from_pylist(rows, schema=arrow_schema(LIVE_TABLES[doctype].fields))

	def make_fifo(self, rows, method, batchwise=None, slots=FIFOSlots):
		fifo = slots(frappe._dict(company="_Test Company", to_date="2026-12-31"), deepcopy(rows))
