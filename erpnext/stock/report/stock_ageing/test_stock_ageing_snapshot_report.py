	...
	def test_unfiltered_valuation_prefetch_uses_report_ledger_scope(self):
		...
		for report_filters, methods in cases:
			with patch.object(StockReportSnapshot, "get_connection", return_value=self.connect(table)):
				with StockReportSnapshot("Stock Ageing", report_filters):
					self.assert_prefetched_valuation_methods(report_filters, methods)

	def assert_prefetched_valuation_methods(self, filters, expected):
		with patch("erpnext.stock.utils.get_valuation_method", return_value="Moving Average") as default:
			slots = stock_ageing.FIFOSlots(filters)
...
class FIFOSlotsOnSnapshot(SnapshotFIFOSlots):
	"""FIFOSlots for the live tests: entries read from the database come from a snapshot."""
...
TestStockAgeingOnSnapshot = on_snapshot(live_tests.TestStockAgeing, FIFOSlots=FIFOSlotsOnSnapshot)
