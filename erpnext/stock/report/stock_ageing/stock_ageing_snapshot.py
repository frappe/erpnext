...
from erpnext.stock.report.stock_ageing.stock_ageing import DETAIL_FIELDS, FIFOSlots
...
class SnapshotFIFO:
	...
	def running_sum(value, table):
		return (
			...
			.orderby(table.posting_datetime, table.creation, table.entry_name)
...
class SnapshotFIFOSlots(FIFOSlots):
	...
	def _generate_from_snapshot(self, serial_bundles: dict, batch_bundles: dict) -> bool:
		...
		if result.replay_query is not None:
			for row in run_stock_query(result.replay_query, as_dict=True, as_iterator=True):
