...
def get_conditions(filters):
	...
	conditions += filters.get('id') and " AND dn.name = '%s' " % filters.get('id') or ""
...
def set_address_details(row, special_characters):
	...
	if row.get('company_address'):
		address_line1, address_line2, city, pincode, state = db.get_value("Address", row.get('company_address'), ['address_line1', 'address_line2', 'city', 'pincode', 'state'])
	...
	if row.get('shipping_address_name'):
		address_line1, address_line2, city, pincode, state = db.get_value("Address", row.get('shipping_address_name'), ['address_line1', 'address_line2', 'city', 'pincode', 'state'])
...
def set_taxes(row, filters):
	taxes = db.get_list("Sales Taxes and Charges", filters={'parent': row.dn_id}, fields=('item_wise_tax_detail', 'account_head'))
