...
def validate_serial_no(sle, item_det):
	...
	if not serial_nos:
		if not sle.is_cancelled and (cint(sle.actual_qty) < 0 or not item_det.serial_no_series):
			frappe.throw(_("Serial Nos Required for Serialized Item {0}")
					.format(sle.item_code), SerialNoRequiredError)
		return

	if item_det.has_serial_no == 0:
		frappe.throw(_("Item {0} is not setup for Serial Nos. Serial No field must be blank")
	...
	# SLE is not cancelled and serial nos are present.
	if cint(sle.actual_qty) != flt(sle.actual_qty):
		frappe.throw(_("Serial No {0} quantity {1} cannot be a fraction")
				.format(sle.item_code, sle.actual_qty))

	if len(serial_nos) != abs(cint(sle.actual_qty)):
		frappe.throw(_("{0} Serial Numbers required for Item {1}. You have provided {2}.")
				.format(abs(sle.actual_qty), sle.item_code, len(serial_nos)), SerialNoQtyError)

	if len(serial_nos) != len(set(serial_nos)):
		frappe.throw(_("Duplicate Serial No entered for Item {0}")
				.format(sle.item_code), SerialNoDuplicateError)
	...
	for serial_no in serial_nos:
		...
		if sr.item_code != sle.item_code and not allow_serial_nos_with_different_item(serial_no, sle):
			frappe.throw(_("Serial No {0} does not belong to Item {1}")
					.format(serial_no, sle.item_code), SerialNoItemError)

		if cint(sle.actual_qty) > 0 and does_serial_no_exist(sr, sle):
		...
		if sr.warehouse != sle.warehouse:
			frappe.throw(_("Serial No {0} does not belong to Warehouse {1}")
					.format(serial_no, sle.warehouse), SerialNoWarehouseError)

		if not sr.purchase_document_no:
			frappe.throw(_("Serial No {0} not in stock")
					.format(serial_no), SerialNoNotExistsError)

		if sle.voucher_type not in ("Delivery Note", "Sales Invoice"):
			continue
...
def does_serial_no_exist(sn, sle):
