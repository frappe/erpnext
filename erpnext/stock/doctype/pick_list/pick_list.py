...
@frappe.whitelist()
def create_delivery(source_name: str, target_doc: str | Document | None = None):
	pick_list = frappe.get_doc("Pick List", source_name)
	target = frappe.flags.args.get("target")
	...
	if not all(item.sales_order for item in pick_list.locations):
		documents.append(create_delivery_wo_so(pick_list, target))
	...
	if len(documents) == 1:
		...
		doc_list = [get_link_to_form("Delivery Note", p.name) for p in documents]
...
def create_dn_for_pick_lists(
	...
	if not sales_order_arg and not all(item.sales_order for item in pick_list.locations):
		...
		delivery_note = create_delivery_wo_so(pick_list, "Delivery Note", target_doc=delivery_note)
...
def create_delivery_with_so(sales_dict, pick_list, target):
	...
	for key in sales_dict:
		target_doc = create_delivery_from_so(pick_list, sales_dict[key], target, None)
		if target_doc:
			target_doc.flags.ignore_mandatory = True
			...
			if target == "Sales Invoice":
				target_doc.update_stock = 1
			target_doc.save()
			target_doc_list.append(target_doc)
...
def add_product_bundles_to_target(pick_list: "PickList", target_doc, item_mapper, sales_order=None) -> None:
