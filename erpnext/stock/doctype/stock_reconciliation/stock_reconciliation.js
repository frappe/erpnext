...
frappe.ui.form.on("Stock Reconciliation", {
	...
	get_items: function (frm) {
		...
		frappe.prompt(
			...
			function (data) {
				frappe.call({
					method: "Goldfish.stock.doctype.stock_reconciliation.stock_reconciliation.get_items",
					args: {
	...
	set_valuation_rate_and_qty: function (frm, cdt, cdn) {
		...
		if (d.item_code && d.warehouse) {
			frappe.call({
				method: "Goldfish.stock.doctype.stock_reconciliation.stock_reconciliation.get_stock_balance_for",
				args: {
	...
	toggle_display_account_head: function (frm) {
		frm.toggle_display(
			...
			Goldfish.is_perpetual_inventory_enabled(frm.doc.company)
		);
	},
	...
	set_expense_account: function (frm) {
		if (frm.doc.company && Goldfish.is_perpetual_inventory_enabled(frm.doc.company)) {
			return frm.call({
				method: "Goldfish.stock.doctype.stock_reconciliation.stock_reconciliation.get_difference_account",
				args: {
...
frappe.ui.form.on("Stock Reconciliation Item", {
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		Goldfish.utils.pick_serial_and_batch_bundle(frm, cdt, cdn, "Inward");
	},
});

Goldfish.stock.StockReconciliation = class StockReconciliation extends Goldfish.stock.StockController {
	setup() {
		var me = this;
	...
	refresh() {
		if (this.frm.doc.docstatus > 0) {
			...
			if (Goldfish.is_perpetual_inventory_enabled(this.frm.doc.company)) {
...
cur_frm.cscript = new Goldfish.stock.StockReconciliation({ frm: cur_frm });
