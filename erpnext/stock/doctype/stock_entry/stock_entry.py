...
class StockEntry(StockController):
	...
	def validate_finished_goods(self):
		...
		if self.purpose == "Manufacture":
			# Allow multiple finished goods for manufacturing
			# Multiple finished goods are now supported
			pass
	...
	def update_stock_ledger(self):
		sl_entries = []
		finished_items = self.get_finished_item_rows()

		# make sl entries for source warehouse first
		self.get_sle_for_source_warehouse(sl_entries, finished_items)

		# SLE for target warehouse
		self.get_sle_for_target_warehouse(sl_entries, finished_items)
	...
	def get_finished_item_rows(self):
		finished_item_rows = []
		if self.purpose in ("Manufacture", "Repack"):
			for d in self.get("items"):
				if d.is_finished_item:
					finished_item_rows.append(d)
		
		return finished_item_rows
	...
	def get_sle_for_source_warehouse(self, sl_entries, finished_items):
		for d in self.get("items"):
			if cstr(d.s_warehouse):
				...
				if cstr(d.t_warehouse):
					...
					# For multiple finished items, find the first one that's different
					for finished_item_row in finished_items:
						if (
	...
	def get_sle_for_target_warehouse(self, sl_entries, finished_items):
		for d in self.get("items"):
			if cstr(d.t_warehouse):
				sle = self.get_sl_entries(
					d,
				...
				# Check if this item is a finished item or has source warehouse
				is_finished_item = any(fg.name == d.name for fg in finished_items)
				if cstr(d.s_warehouse) or is_finished_item:
