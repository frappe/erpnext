	...
	get_indicator: function (doc) {
		...
		if (doc.status == "Stopped") {
			...
			if (
				...
				flt(doc.per_returned, precision) < 100 &&
				flt(doc.per_returned, precision) > 0
			) {
				return [__("Partially Returned"), "gray", "per_returned,<,100"];
			} else if (doc.material_request_type == "Purchase" && flt(doc.per_returned, precision) == 100) {
				return [__("Returned"), "gray", "per_returned,=,100"];
			} else if (
				doc.material_request_type == "Purchase" &&
