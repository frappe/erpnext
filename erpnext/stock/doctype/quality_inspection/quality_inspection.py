		...
		if self.reference_type == 'Job Card':
			...
			if self.reference_type and self.reference_name:
				...
				""".format(
						...
						quality_inspection=args[0], reference_name=args[2], item_code=args[3]
					)
				}, args)
				...
				frappe.db.multisql({
					'mariadb': """SELECT 1 FROM DUAL WHERE false""",
					'postgres': """
						UPDATE
							`tab{parent_doc}` t2
						SET
				...
				""".format(
						parent_doc=self.reference_type, child_doc=doctype, conditions=conditions,
						modified=args[1], reference_name=args[2], item_code=args[3]
					)
				})
