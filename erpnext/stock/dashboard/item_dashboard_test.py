...
def get_data(item_code=None, status=None, item_group=None,
	...
	try:
		...
		if DatabaseQuery('Serial No State', user=frappe.session.user).build_match_conditions():
			filters.append(['status', 'in', [w.name for w in frappe.get_list('Serial No State')]])
