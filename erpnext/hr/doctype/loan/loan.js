...
frappe.ui.form.on('Loan', {
	...
	refresh: function (frm) {
		...
		if (frm.doc.disbursement_date && !frm.doc.repayment_schedule[0].payment_date) {
	...
	make_repayment_entry: function(frm) {
		...
		$.each(me.frm.doc.repayment_schedule || [], function(i, row) {
			if (row.payment_date <= month_end && row.payment_date >= month_start)
				payments.push({
