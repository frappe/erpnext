...
def get_benefit_component_amount(employee, start_date, end_date, struct_row, sal_struct, period_lenght, frequency):
	...
	if period_factor:
		...
		if benefit_application_name:
			...
			return get_benefit_amount(benefit_application, struct_row, period_factor, period_lenght)
		...
		# TODO: Check if there is benefit claim for employee then pro-rata devid the rest of amount (Late Benefit Application)
			...
			if component_max > 0:
				...
				return get_amount(period_factor, benefit_amount, period_lenght)
	return False
...
def get_benefit_amount(application, struct_row, period_factor, period_lenght):
	amount = 0
	for employee_benefit in application.employee_benefits:
		if employee_benefit.earning_component == struct_row.salary_component:
			amount += get_amount(period_factor, employee_benefit.amount, period_lenght)
	return amount if amount > 0 else False

def get_amount(period_factor, amount, period_lenght):
	amount_per_day = amount / period_factor
	total_amount = amount_per_day * period_lenght
	return total_amount
