...
class DailyWorkSummarySetting(Document):
	def validate(self):
		if self.users:
			if not frappe.flags.in_test and not frappe.db.get_value('Email Account', dict(enable_incoming=1, default_incoming=1)):
...
def trigger_emails():
	...
	for d in settings:
		...
		if is_current_hour(setting_doc.send_emails_at) and not is_holiday_today(setting_doc.holiday_list) and setting_doc.enabled:
			...
			if emails:
				daily_work_summary = frappe.get_doc(dict(doctype='Daily Work Summary', setting=setting_doc.name)).insert()
