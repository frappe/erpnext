...
from erpnext.hr.doctype.employee_loan.employee_loan import get_emi_amount, check_repayment_method
...
class EmployeeLoanApplication(Document):
	...
	def get_emi_details(self):
		...
		if self.repayment_method == "Repay Fixed Amount per Period":
			...
			self.repayment_periods = math.ceil((math.log(self.emi_amount) - math.log(self.emi_amount - 
									(self.loan_amount*monthly_interest_rate)))/(math.log(1+monthly_interest_rate)))

		self.total_payable_amount = self.emi_amount * self.repayment_periods
		self.total_payable_interest = self.total_payable_amount - self.loan_amount

		# N = math.ceil(math.log((emi / (emi - P*R)), (1 + R)))