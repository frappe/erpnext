...
def make_bank_entry(dt, dn):
	...
	default_bank_cash_account = get_account(expense_claim.company)
	amount, exchange_rate = get_amount_and_exchange_rate(default_bank_cash_account.account_currency, expense_claim)

	payable_account_currency = get_account_currency(expense_claim.payable_account)
	payable_amount, payable_acc_exchange_rate = get_amount_and_exchange_rate(payable_account_currency, expense_claim)
	...
	je.multi_currency = 1 if payable_account_currency != default_bank_cash_account.account_currency else 0

	je.append('accounts', {
		'account': expense_claim.payable_account,
		'account_currency': payable_account_currency,
		'exchange_rate': flt(payable_acc_exchange_rate),
...
def get_amount_and_exchange_rate(account_currency, doc):
	if account_currency != doc.currency:
		amount = flt(doc.outstanding_amount) * flt(doc.conversion_rate)
		exchange_rate = 1
	else:
...
def get_account(company):
	...
	default_bank_cash_account = get_default_bank_cash_account(company, 'Bank')
	if not default_bank_cash_account:
		default_bank_cash_account = get_default_bank_cash_account(company, 'Cash')

	if not default_bank_cash_account:
		frappe.throw(_('Please set a default Cash or Bank account in Company {}').format(
			get_link_to_form('Company', company)))

	return default_bank_cash_account
...
def get_expense_claim_account(item_code, company):
	...
	if not account:
		frappe.throw(_('Set the Default Expense Account for the {0} {1}').format(
			frappe.bold('Item'), get_link_to_form('Item', item_code)))
