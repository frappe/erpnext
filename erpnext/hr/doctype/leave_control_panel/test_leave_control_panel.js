...
QUnit.test("Test: Leave control panel [HR]", function (assert) {
	...
	frappe.run_serially([
		...
		() => cur_frm.set_value("to_date", frappe.datetime.add_months(today_date, 1)),	//for three months
		...
		() => {
			...
			assert.equal(leave_allocated.length, 2,
				'leave is successfully allocated to both the employee');
		},
		() => done()
	]);
});