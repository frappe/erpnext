...
class TestAttendanceStatus(unittest.TestCase):
	...
	def test_duplicate_attendance_status_is_present_with_same_default_property(self):
		att_status = create_attendace_status("Present Status 1", "T", is_present=1, applicable_for_employee_checkins=1)

		att_status_2 = create_attendace_status("Present Status 2", "T", is_present=1, applicable_for_employee_checkins=1)
	...
	def test_duplicate_attendance_status_is_half_day_with_same_default_property(self):
		att_status = create_attendace_status("Half day Status 1", "T", is_half_day=1, applicable_for_employee_checkins=1,
			applicable_for_leave_application=1, applicable_for_attendace_request=1)

		att_status_2 = create_attendace_status("Half day Status 3", "T", is_half_day=1, applicable_for_employee_checkins=1)
		self.assertRaises(frappe.ValidationError, att_status_2.save)

		att_status_2 = create_attendace_status("Half day Status 4", "T", is_half_day=1, applicable_for_attendace_request=1)
		self.assertRaises(frappe.ValidationError, att_status_2.save)

		att_status_2 = create_attendace_status("Half day Status 5", "T", is_half_day=1, applicable_for_leave_application=1)
		self.assertRaises(frappe.ValidationError, att_status_2.save)

	def test_duplicate_attendance_status_is_leave_with_same_default_property(self):
		att_status = create_attendace_status("On Leave Status 1", "T", is_leave=1, applicable_for_leave_application=1)
	...
	def tearDown(self):
		frappe.db.sql("""DELETE FROM `tabAttendance Status` WHERE name not in ('Present', 'Absent', 'On Leave')""")
...
def create_attendace_status(name, abbr, **args):
	...
	attendacne_status.applicable_for_attendace_request = args.applicable_for_attendace_request or 0
