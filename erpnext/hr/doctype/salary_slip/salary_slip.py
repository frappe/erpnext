	...
	def calculate_lwp(self, holidays, working_days):
		...
		for d in range(working_days):
			...
			leave = frappe.db.sql("""
				...
				and CASE WHEN t2.include_holiday != 1 THEN %(dt)s not in ('{0}') and %(dt)s between from_date and to_date and ifnull(salary_slip, '') = ''
				WHEN t2.include_holiday THEN %(dt)s between from_date and to_date and ifnull(salary_slip, '') = ''
				END
