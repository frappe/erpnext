...
class SalarySlip(TransactionBase):
	...
	def set_time_sheet(self):
		if self.salary_slip_based_on_timesheet and not self.get('timesheets'):
			...
			timesheets = frappe.db.sql(""" select * from `tabTime Sheet` where employee = %(employee)s and (status = 'Submitted' or
				status = 'Billed') and %(start_date)s between start_date and end_date and %(end_date)s between start_date and end_date""", 
	...
	def add_earning_for_hourly_wages(self, earning_type):
		...
		for data in self.earnings:
			if data.e_type == earning_type:
				data.e_modified_amount = self.hour_rate * self.total_working_hours
				default_type = True
				break

		if not default_type:
			earnings = self.append('earnings', {})
			earnings.e_type = earning_type
			earnings.e_modified_amount = self.hour_rate * self.total_working_hours
