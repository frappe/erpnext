...
def get_salary_structure_assignment(employee, salary_structure, on_date):
	if not employee or not salary_structure or not on_date:
		return None

	salary_structure = frappe.db.sql("""
		select name
		from `tabSalary Structure Assignment`
		where employee = %(employee)s and salary_structure = %(salary_structure)s and docstatus = 1
		and %(on_date)s >= from_date
		order by from_date desc
		limit 1
	""", {
