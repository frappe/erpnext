...
test_records = [
	dict(doctype="Item", item_code="Food Item 1", item_group="Products", is_stock_item=0),
	dict(doctype="Item", item_code="Food Item 2", item_group="Products", is_stock_item=0),
	dict(doctype="Item", item_code="Food Item 3", item_group="Products", is_stock_item=0),
	dict(doctype="Item", item_code="Food Item 4", item_group="Products", is_stock_item=0),
	dict(
		doctype="Restaurant Menu",
		restaurant="Test Restaurant 1",
		name="Test Restaurant 1 Menu 1",
		items=[
...
class TestRestaurantMenu(unittest.TestCase):
	def test_price_list_creation_and_editing(self):
		...
		self.assertEqual(
			frappe.db.get_value(
				"Item Price",
				dict(price_list="Test Restaurant 1 Menu 1", item_code="Food Item 1"),
				"price_list_rate",
			),
			400,
		)
		self.assertEqual(
			frappe.db.get_value(
				"Item Price",
				dict(price_list="Test Restaurant 1 Menu 2", item_code="Food Item 1"),
				"price_list_rate",
			),
			450,
		)
