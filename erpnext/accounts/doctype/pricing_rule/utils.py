...
def get_qty_and_rate_for_mixed_conditions(doc, pr_doc, args):
	...
	if items and doc.get("items"):
		for row in doc.get('items'):
			if row.get(apply_on) not in items:
				continue
...
def apply_pricing_rule_on_transaction(doc):
	...
	if pricing_rules:
		...
		for d in pricing_rules:
			no_coupon = hasattr(doc,"coupon_code") and doc.coupon_code is None
			if no_coupon and d.coupon_code_based:
				continue
