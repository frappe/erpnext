...
def get_items_list(pos_profile, company):
	...
	query = (
		...
		.on((item_default.parent == i.name) & (item_default.company == company))
		.left_join(c)
		.on((i.name == c.parent) & (i.sales_uom == c.uom))
		.select(
			i.name,
			i.item_code,
			i.item_name,
		...
		.where((i.disabled == 0) & (i.has_variants == 0) & (i.is_sales_item == 1) & (i.is_fixed_asset == 0))
	)
	if args_list:
		query = query.where(i.item_group.isin(args_list))
