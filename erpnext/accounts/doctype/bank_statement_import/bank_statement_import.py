...
def get_transaction_reference(txn_data: dict) -> str:
	...
	# Rejoin customer_reference that overflowed into extra_details on a single-line :61:.
	# The mt940 library caps customer_reference at 16 chars; only apply the rejoin when
	# the captured value is exactly at that cap (indicating the parser split a longer
	# reference). Below the cap, extra_details is genuine supplementary information.
