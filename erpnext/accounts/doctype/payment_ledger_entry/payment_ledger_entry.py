		...
		if entry.get('against_voucher_type') and entry.get('against_voucher'):
			if entry.get('against_voucher') != entry.get('voucher_no'):
				...
				ple = frappe.get_doc({
					...
					'posting_date': now(),
					'amount': abs(entry.debit-entry.credit) * -1 if cancel else abs(entry.debit-entry.credit),
					'is_cancelled': cancel
				})
...
def get_active_payments(inv_type, inv_no):
	...
	active_payments = (
		...
		.where((pledger.against_voucher_type == inv_type) & (pledger.against_voucher_no == inv_no) & (pledger.is_cancelled == False))
		.run(as_dict=True)
	)
	...
	if payment_ledger_entry:
		(
			...
			.where(
				...
				& (pl.against_voucher_type.isnull() if payment_ledger_entry.against_voucher_type is None else pl.against_voucher_type == payment_ledger_entry.against_voucher_type)
				& (pl.against_voucher_no.isnull() if payment_ledger_entry.against_voucher_no is None else pl.against_voucher_no == payment_ledger_entry.against_voucher_no)
