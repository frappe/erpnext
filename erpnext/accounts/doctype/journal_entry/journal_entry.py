    ...
    def before_save(self):
        if self.voucher_type in ["Depreciation Entry", "Exchange Rate Revaluation"]:
            ...
            self.exchange_rate = get_exchange_rate(
                self.transaction_currency, self.default_company_currency, self.posting_date
            )
            for row in self.accounts:
                ...
                row.local_currency_exchange_rate = get_exchange_rate(
                    self.default_company_currency, row.local_currency, self.posting_date
                )
                ...
                if row.transaction_debit and self.exchange_rate and row.local_currency_exchange_rate:
                    row.debit_in_local_currency = (
                        row.transaction_debit * self.exchange_rate
                    ) * row.local_currency_exchange_rate
                if row.transaction_credit and self.exchange_rate and row.local_currency_exchange_rate:
                    row.credit_in_local_currency = (
                        row.transaction_credit * self.exchange_rate
                    ) * row.local_currency_exchange_rate
...
def get_transactions(doc, child, transaction_type):
    ...
    if "account" in child_doc and "exchange_rate" in child_doc and transaction_type == "debit":
        if (
            child_doc["transaction_debit"]
            and child_doc["transaction_exchange_rate"]
            and child_doc["exchange_rate"]
            and child_doc["local_currency_exchange_rate"]
        ):
    ...
    if "account" in child_doc and "exchange_rate" in child_doc and transaction_type == "credit":
        ...
        return {
            ...
            "credit_in_local_currency": credit_in_local_currency,
        }