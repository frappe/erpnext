...
class PaymentManager(Document):
	def autoname(self):
		...
		if not frappe.db.exists("Bank Account Mapper", mapper_name):
			mapper = frappe.new_doc("Bank Account Mapper")
	...
	def get_statement_headers(self):
		...
		mapper_doc = frappe.get_doc("Bank Account Mapper", self.bank_data_mapper)
	...
	def populate_payment_entries(self):
		...
		date_format = frappe.get_value("Bank Account Mapper", self.bank_data_mapper, "date_format")
		if (date_format is None):
			date_format = '%Y-%m-%d'
		if self.bank_data_mapper:
			mapped_items = frappe.get_doc("Bank Account Mapper", self.bank_data_mapper).mapped_items
