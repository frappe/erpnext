...
frappe.ui.form.on('Personal Account', {
...
frappe.ui.form.on('Personal Account', {
	...
	party: function(frm) {
		if(frm.doc.payment_type && frm.doc.party_type && frm.doc.party) {
			...
			return frappe.call({
				method: "erpnext.accounts.doctype.personal_account.personal_account.get_party_details",
				args: {
					company: frm.doc.company,
	...
	set_account_currency_and_balance: function(frm, account, currency_field,
		...
		frappe.call({
			method: "erpnext.accounts.doctype.personal_account.personal_account.get_account_details",
			args: {
				"account": account,
				"date": frm.doc.posting_date
			},
	...
	get_outstanding_documents: function(frm) {
		...
		return  frappe.call({
			method: 'erpnext.accounts.doctype.personal_account.personal_account.get_outstanding_reference_documents',
			args: {
				args: {
	...
	set_deductions_entry: function(frm, account) {
		if(frm.doc.difference_amount) {
			frappe.call({
				method: "erpnext.accounts.doctype.personal_account.personal_account.get_company_defaults",
				args: {
					company: frm.doc.company
				},
				callback: function(r, rt) {
...
frappe.ui.form.on('Payment Entry Reference', {
	...
	reference_name: function(frm, cdt, cdn) {
		var row = locals[cdt][cdn];
		return frappe.call({
			method: "erpnext.accounts.doctype.personal_account.personal_account.get_reference_details",
			args: {
