...
class TestOpeningInvoiceCreationTool(ERPNextTestSuite):
	def make_invoices(
		...
		doc = frappe.get_doc({"doctype": "Opening Invoice Creation Tool", **args}).insert()
	...
	def test_import_run_keeps_results_on_source_rows(self):
		...
		self.assertTrue(all(row.result_status == "Success" and row.reference_name for row in doc.invoices))
	...
	def test_run_requires_parent_write_permission(self):
		doc = self.make_invoices(company="_Test Opening Invoice Company", return_doc=True)
		frappe.set_user("Guest")
		try:
			self.assertRaises(frappe.PermissionError, doc.make_invoices)
		finally:
	...
	def test_opening_invoice_requires_temporary_account_type(self):
		...
		failed_rows = frappe.get_all(
			...
			fields=["name", "exception"],
		)
		self.assertEqual(doc.status, "Partial Success")
		self.assertEqual(failed_rows[0].name, doc.invoices[0].name)
		self.assertTrue(failed_rows[0].exception)
		self.assertEqual(doc.invoices[1].result_status, "Success")
	...
	def test_opening_sales_invoice_creation_with_missing_debit_account(self):
		...
		failed_rows = frappe.get_all(
			"Opening Invoice Creation Tool Item",
			filters={"parent": doc.name, "result_status": "Failed"},
			fields=["exception"],
		)
		self.assertEqual(doc.status, "Error")
		self.assertEqual(len(failed_rows), 2)
		self.assertTrue(all("AccountMissingError" in row.exception for row in failed_rows))
	...
	def test_large_run_queues_saved_run_name(self):
		rows = [{"party": "_Test Customer"} for _ in range(50)]
		doc = self.make_invoices(company="_Test Opening Invoice Company", invoices=rows, return_doc=True)

		with patch(
			"erpnext.accounts.doctype.opening_invoice_creation_tool.opening_invoice_creation_tool.enqueue"
		) as enqueue_job:
			doc.make_invoices()

		doc.reload()
		self.assertEqual(doc.status, "In Progress")
		self.assertEqual(enqueue_job.call_args.kwargs["run_name"], doc.name)
