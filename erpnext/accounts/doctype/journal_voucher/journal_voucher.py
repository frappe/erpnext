...
def get_payment_entry_from_sales_invoice(sales_invoice):
	...
	# credit customer
	jv["entries"][0].account = si.debit_to
	jv["entries"][0].balance = get_balance_on(si.debit_to)
	jv["entries"][0].credit = si.outstanding_amount
	jv["entries"][0].against_invoice = si.name

	# debit bank
	jv["entries"][1].debit = si.outstanding_amount
...
def get_payment_entry_from_purchase_invoice(purchase_invoice):
	...
	# credit supplier
	jv["entries"][0].account = pi.credit_to
	jv["entries"][0].balance = get_balance_on(pi.credit_to)
	jv["entries"][0].debit = pi.outstanding_amount
	jv["entries"][0].against_voucher = pi.name

	# credit bank
	jv["entries"][1].credit = pi.outstanding_amount
