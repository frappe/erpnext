...
class PurchaseInvoice(BuyingController):
	...
	def set_expense_account(self, for_validate=False):
		...
		for item in self.get("items"):
			...
			elif item.is_fixed_asset and is_cwip_accounting_disabled(self.company, asset_category):
				if not item.asset:
	...
	def check_asset_cwip_enabled(self):
		...
		for item in self.get("items"):
			asset_category = frappe.db.get_value("Item", item.item_code, "asset_category")
			if item.is_fixed_asset and not is_cwip_accounting_disabled(self.company, asset_category):
				return 1
	...
	def make_item_gl_entries(self, gl_entries):
		...
		for item in self.get("items"):
			if flt(item.base_net_amount):
				...
				elif not item.is_fixed_asset or (item.is_fixed_asset and is_cwip_accounting_disabled(self.company,
					asset_category)):
