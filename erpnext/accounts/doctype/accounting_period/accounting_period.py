...
class AccountingPeriod(Document):
	...
	def validate_overlap(self):
		accounting_period = frappe.qb.DocType("Accounting Period")
		existing_accounting_period = (
			frappe.qb.from_(accounting_period)
			.select(accounting_period.name)
			.where(
				(
					(self.start_date >= accounting_period.start_date)
					& (self.start_date <= accounting_period.end_date)
				)
				| (
					(self.end_date >= accounting_period.start_date)
					& (self.end_date <= accounting_period.end_date)
				)
				| accounting_period.start_date.between(self.start_date, self.end_date)
				| accounting_period.end_date.between(self.start_date, self.end_date)
			)
			.where((accounting_period.name != self.name) & (accounting_period.company == self.company))
		).run(as_dict=True)
