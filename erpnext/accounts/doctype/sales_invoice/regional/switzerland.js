frappe.ui.form.on('Sales Invoice', {
  onload: function(frm) {
    ...
    var iso7064={compute:function(rawValue) {const value=stringifyInput(rawValue);if(!value.match(FORMAT)){throw new Error('Invalid data format; expecting: \''+FORMAT+'\', found: \''+value+'\'')}return mod97(value)},computeWithoutCheck:function(rawValue){return mod97(rawValue)}};
    var iso11649={generate:function(rawValue){const value=stringifyInput(rawValue);if(!value.match(FORMAT_RF_BODY)){throw new Error('Invalid Creditor Reference format; expecting: \''+FORMAT_RF_BODY+'\', found: \''+value+'\'')}return FORMAT_RF_HEAD+('0'+(98-iso7064.computeWithoutCheck(value+FORMAT_RF_HEAD+'00'))).slice(-2)+value},validate:function(rawValue){const value=stringifyInput(rawValue);if(!value.match(FORMAT_RF)){throw new Error('Invalid Creditor Reference format; expecting: \''+FORMAT_RF+'\', found: \''+value+'\'')}return iso7064.computeWithoutCheck(value.substring(4,value.length)+value.substring(0,4))===1}};

    function stringifyInput(rawValue,valueName='rawValue') {
      if(rawValue!==null&&rawValue!==undefined) {
        switch(typeof rawValue){
          case 'string':return rawValue.toUpperCase().replace(/[^0-9A-Z]/g,'');
          default:throw new Error('Expecting '+valueName+' of type \'string\', found: \''+(typeof rawValue)+'\'')
        }
      }
