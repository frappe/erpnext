...
def get_outstanding_invoices(
	party_type, party, account, condition=None, filters=None, filter_criterion=None
):
	...
	outstanding_query = (
		...
		.on(
			(AliasedQuery("invoices").voucher_type == AliasedQuery("payments").against_voucher_type)
			& (AliasedQuery("invoices").voucher_no == AliasedQuery("payments").against_voucher_no)
		)
		.select(
			Table("invoices").voucher_type,
			...
			(Table("invoices").invoice_amount - IfNull(Table("payments").payment_amount, 0)).as_(
				"outstanding_amount"
			),
		)
		.having(qb.Field("outstanding_amount") > 0)
	)
