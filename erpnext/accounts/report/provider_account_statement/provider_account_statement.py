...
def return_data(filters):
	...
	for purchase_order in purchase_orders:
		row = [purchase_order.posting_date, purchase_order.supplier, "Purchase Invoice", purchase_order.name, purchase_order.grand_total, purchase_order.total_advance, purchase_order.outstanding_amount, purchase_order.currency]
		data.append(row)
	...
	for supplier_document in supplier_documents:
		...
		row = [supplier_document.posting_date, supplier_document.supplier, "Supplier Documents", supplier_document.name, supplier_document.total, total_advance, supplier_document.outstanding_amount, supplier_document.currency]
		data.append(row)
	...
	for supplier_retention in supplier_retentions:
		row = [supplier_retention.posting_date, supplier_retention.supplier, "Supplier Retention", supplier_retention.name, supplier_retention.total_references, 0, 0, supplier_retention.currency]
		data.append(row)
	...
	for payment_entry in payment_entries:
		row = [payment_entry.posting_date, payment_entry.party, "Payment Entry", payment_entry.name, payment_entry.total_allocated_amount, payment_entry.paid_amount, payment_entry.unallocated_amount, payment_entry.paid_to_account_currency]
		data.append(row)
	
	condition_credit_note = return_filters(filters)
	...
	for credit in credits:
		...
		row = [credit.posting_date, credit.supplier, "Credit Note CXP", credit.name, credit.total, total_advance, credit.outstanding_amount, credit.currency]
		data.append(row)
