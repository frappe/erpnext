    ...
    for assets in ps:
        ...
        for accdepr in frappe.get_doc("Fixed Asset Account", fixed_asset_name).depreciation:
           if get_fiscal_year(fiscal_year = accdepr.fiscal_year) == get_fiscal_year(date = day_before_start):
               depr = accdepr.total_accumulated_depreciation
        ...
        if totalsales == 0:
            ...
            print opening_balance, depronopening
        ...
        if (totalsales <= opening_balance or totalsales == 0):
            purchases_sql = frappe.db.sql("""select * from `tabFixed Asset Account` pur where pur.fixed_asset_name=%s and purchase_date>=%s and purchase_date<=%s""", (fixed_asset_name, finyrfrom, finyrto), as_dict=True)
        ...
        if (totalsales == opening_balance):
            deprwrittenback = depronopening + depr
        elif (totalsales == (opening_balance + totalpurchase)):
            deprwrittenback = depr + depronopening + depronpurchases
        elif (totalsales < opening_balance):
            sales_sql = frappe.db.sql("""select * from `tabFixed Asset Sale` where docstatus = 1 and fixed_asset_account=%s and posting_date>=%s and posting_date<=%s""", (fixed_asset_name, finyrfrom, finyrto), as_dict=True)
            value = totalsales
            factor = deprtilllastyr / opening_balance
        ...
        if depr < opening_balance and opening_balance - depr >= depronopening:
	        row = [fixed_asset_name,
	               ...
	               flt(((depr + depronopening + depronpurchases) - 	deprwrittenback),2)]
	elif depr < opening_balance and opening_balance - depr < depronopening:
	        row = [fixed_asset_name,
	               fixed_asset_account,
	               rateofdepr,
	               flt(opening_balance,2),
	               ...
	               flt(((opening_balance + totalpurchase) - totalsales),2),
	               flt(depr,2),
	               flt(opening_balance - depr,2),
	               flt(depronpurchases,2),
	               flt(deprwrittenback,2),
	               flt(((depr + (opening_balance - depr) + depronpurchases) - 	deprwrittenback),2)]
	elif depr == opening_balance:
	        row = [fixed_asset_name,
	               fixed_asset_account,
	               rateofdepr,
	               flt(opening_balance,2),
	               ...
	               flt(depronpurchases,2),
	               flt(deprwrittenback,2),
	               flt(((depr + 0 + depronpurchases) - 	deprwrittenback),2)]
