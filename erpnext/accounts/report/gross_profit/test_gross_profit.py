...
class TestGrossProfit(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		cls.income = flt(
			abs(get_balance_on(account=frappe.get_cached_value("Company", test_company, "default_income_account"))), 2)
		cls.cogs = flt(
			get_balance_on(account=frappe.get_cached_value("Company", test_company, "default_expense_account")), 2)
	...
	def test_gross_profit_selling_amount(self):
		self.assertEqual(self.income, self.totals[-5], "Incorrect selling amount got {0} but expected {1}".format(
			frappe.format(self.totals[-5], "Currency"), frappe.format(self.income, "Currency")))

	def test_gross_profit_buying_amount(self):
		self.assertEqual(self.cogs, self.totals[-4], "Incorrect buying amount got {0} but expected {1}".format(
			frappe.format(self.totals[-4], "Currency"), frappe.format(self.cogs, "Currency")))
