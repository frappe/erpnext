...
function hide_filters(report) {
	report.page.page_form[0]
		.querySelectorAll(".form-group.frappe-control")
		.forEach(function setHidden(field) {
...
erpnext.TaxDetail = class TaxDetail {
	...
	setAppliedFilters(filters) {
		if (this.qr.datatable) {
			Array.from(this.qr.datatable.header.querySelectorAll(".dt-filter")).map(function setFilters(
				input
			) {
	...
	create_controls() {
		...
		controls["delete_section"] = this.qr.page.add_field({
			...
			click: () => {
				...
				if (cur_section) {
					frappe.confirm(
						__("Are you sure you want to delete section") + " " + cur_section + "?",
						() => {
		...
		controls["delete_component"] = this.qr.page.add_field({
			...
			click: () => {
				...
				if (component) {
					frappe.confirm(
						__("Are you sure you want to delete component") + " " + component + "?",
						() => {
	...
	show_help() {
		...
		this.qr.$report_footer.append(
			'<div class="col-md-12"><strong>' +
				__("Help") +
				`: </strong>${help}<a href="https://www.casesolved.co.uk"> Case Solved</a></div>`
		);
	}
};
