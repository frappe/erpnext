...
def execute(filters=None):
	...
	columns = [_("Date") + "::240", _("RTN") + "::240", _("Name or Social reason") + "::240", _("Transaction Type") + "::240", _("Serie") + "::240", _("Document Number") + "::240", _("CAI") + "::240", _("Exempts Sales") + ":Currency:120", _("Exonerated") + ":Currency:120", _("Taxed Sales 15%") + ":Currency:120", _("I.S.V 15%") + ":Currency:120", _("Taxed Sales 18%") + ":Currency:120", _("I.S.V 18%") + ":Currency:120", _("Partial Discount") + ":Currency:120" ,_("Discount Amount") + ":Currency:120", _("Total") + ":Currency:120"]
...
def return_data(filters):
	...
	for salary_slip in salary_slips:
		...
		if serie_number == serie and salary_slip.status != "Return":	
			...
			row = [posting_date, rtn, salary_slip.client_name, type_transaction, serie_number,salary_slip.name, salary_slip.cai, exempt, total_exonerated, taxed_sales15, isv15, taxed_sales18, isv18, partial_discount, discount_amount, grand_total]
			data.append(row)
	...
	for salary_slip in salary_slips:
		...
		if serie_number == serie and salary_slip.status == "Return":	
			...
			row = [posting_date, rtn, salary_slip.client_name, type_transaction, serie_number,salary_slip.name, salary_slip.cai, total_exempt, total_exonerated, taxed_sales15, isv15, taxed_sales18, isv18, 0, 0, grand_total]
			data.append(row)
	...
	for debit_note in debit_notes:
		...
		if serie_number == serie:
			...
			row = [posting_date,rtn,debit_note.customer, type_transaction, serie_number, debit_note.name, debit_note.cai, total_exempt, total_exonerated, taxed_sales15, isv15, taxed_sales18, isv18, 0, 0, grand_total]
			data.append(row)
	...
	for credit_note in credit_notes:
		...
		if serie_number == serie:
			...
			row = [posting_date,rtn,credit_note.customer, type_transaction, serie_number, credit_note.name, credit_note.cai, total_exempt, total_exonerated, taxed_sales15, isv15, taxed_sales18, isv18, 0, 0, grand_total]
			data.append(row)
	...
	for customer_retetention in customer_retentions:
		...
		if serie_number == serie:
			...
			row = [customer_retetention.posting_date, customer_retetention.rtn,customer_retetention.customer, type_transaction, serie_number, customer_retetention.name, customer_retetention.cai, total_exempt, total_exonerated, taxed_sales15, isv15, taxed_sales18, isv18, 0, 0, grand_total]
			data.append(row)
