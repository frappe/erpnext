...
def return_data(filters):
	...
	for entry in entries:		
		if verificate_serial(filters, entry.naming_series, entry.paid_amount, entry.unallocated_amount):
			row = [entry.name, filters.get("serial"), entry.party, entry.company, entry.paid_amount, entry.mode_of_payment, entry.reason_payment]
			data.append(row)

	return data

def verificate_serial(filters, naming_series, paid_amount, unallocated_amount):
	...
	if serialString == filters.get("secuence"):
		if paid_amount == unallocated_amount:
			is_serial = True
