...
from erpnext.accounts.report.cash_basis_financial_statements import get_cash_basis_data
...
def get_provisional_profit_loss(asset, liability, equity, period_list, company, currency=None):
    """
    Calculate provisional profit/loss as the balancing item:
    Provisional P/L = Total Assets - (Total Liabilities + Total Equity)

    Under cash basis this reflects only cash-settled transactions so
    AR outstanding is excluded from assets and AP outstanding from liabilities.
    """
...
def check_opening_balance(asset, liability, equity):
    """Check if the previous fiscal year balance sheet was closed (opening_balance should be 0)."""
    opening_balance = 0
...
def get_report_summary(
    period_list, asset, liability, equity, provisional_profit_loss, currency, filters, consolidated=False
):
    net_asset, net_liability, net_equity, net_provisional = 0.0, 0.0, 0.0, 0.0
    ...
    for period in period_list:
        ...
        if provisional_profit_loss:
            net_provisional += provisional_profit_loss.get(key)
    ...
    return [
        ...
        {
            "value": net_provisional,
            "label": _("Provisional Profit / Loss (Credit)"),
            "indicator": "Green" if net_provisional > 0 else "Red",
            "datatype": "Currency",
            "currency": currency,
        },
