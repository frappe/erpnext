...
def execute():
	for organization in frappe.db.sql("select name, expenses_included_in_valuation from tabOrganization", as_dict=1):
		...
		""", (organization.name, frozen_date), as_dict=1)
		...
		for pi in pi_list:
			...
			gle_for_expenses_included_in_valuation = frappe.db.sql("""select name from `tabGL Entry`
				...
				(pi.name, organization.expenses_included_in_valuation))
...
def get_frozen_date(organization, account):
	...
	last_adjustment_entry = frappe.db.sql("""select posting_date from `tabGL Entry`
		where account=%s and organization=%s and voucher_type = 'Journal Entry'
		order by posting_date desc limit 1""", (account, organization))
	...
	last_closing_entry = frappe.db.sql("""select posting_date from `tabGL Entry`
		where organization=%s and voucher_type = 'Period Closing Voucher'
		order by posting_date desc limit 1""", organization)
