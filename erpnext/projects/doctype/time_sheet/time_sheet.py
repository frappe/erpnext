	...
	def validate_mandatory_fields(self):
		...
		for data in self.timesheets:
			if not data.from_time and not data.to_time:
				frappe.throw(_("Row {0}: From Time and To Time is mandataory.").format(data.idx))
				
			if not data.activity_type and self.employee:
				frappe.throw(_("Row {0}: Activity Type is mandataory.").format(data.idx))
...
def make_sales_invoice(source_name, target=None):
	if not cint(frappe.defaults.get_global_default("time_sheet_for_sales_invoice")):
		frappe.throw(_("User can not make sales invoice from timesheet, for more detail check Account Settings."))
...
def make_salary_slip(source_name, target_doc=None):
	if not cint(frappe.defaults.get_global_default("time_sheet_for_salary_slip")):
		frappe.throw(_("User can not make salary slip from timesheet, for more detail check HR Settings."))
...
def get_activity_cost(employee=None, activity_type=None):
	rate = frappe.db.get_values("Activity Cost", {"employee": employee,
		"activity_type": activity_type}, ["costing_rate", "billing_rate", "wage_rate"], as_dict=True)
	if not rate:
		rate = frappe.db.get_values("Activity Type", {"activity_type": activity_type},
			["costing_rate", "billing_rate", "wage_rate"], as_dict=True)
