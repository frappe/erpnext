    ...
    def validate_mandatory_fields(self):
        ...
        for data in self.time_logs:
            ...
            if self.production_order and flt(pending_qty) < flt(data.completed_qty) and flt(pending_qty) > 0:
                frappe.throw(
                    _("Row {0}: Completed Qty cannot be more than {1} for operation {2}").format(data.idx, pending_qty,
                                                                                                 data.operation),
    ...
    def update_cost(self):
        for data in self.time_logs:
            if data.activity_type and data.billable:
                ...
                if rate:
                    data.billing_rate = flt(rate.get('billing_rate')) if flt(
                        data.billing_rate) == 0 else data.billing_rate
                    data.costing_rate = flt(rate.get('costing_rate')) if flt(
                        data.costing_rate) == 0 else data.costing_rate
...
def get_timesheet_data(name, project):
    if project and project != '':
        ...
        data = frappe.get_all('Timesheet',
                              fields=["(total_billable_amount - total_billed_amount) as billing_amt",
                                      "total_billable_hours as billing_hours"], filters={'name': name})
...
def get_activity_cost(employee=None, activity_type=None):
    rate = frappe.db.get_values("Activity Cost", {"employee": employee,
                                                  "activity_type": activity_type}, ["costing_rate", "billing_rate"],
                                as_dict=True)
    if not rate:
...
@frappe.whitelist()
def get_barcode(fields, doctype = None, filters=None):
    if not frappe.db.exists('Bar Code', {'name': fields}):
        return
