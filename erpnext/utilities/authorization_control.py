...
def get_appr_user_role(det, doctype_name, total, based_on, condition, master_name, company):
	...
	if det:
		...
		if not app_dtl:
			app_dtl = frappe.db.sql(
                ...
                and based_on = {} and ifnull(company,'') = '' {}""".format("%s", "%s", "%s", "%s", condition),
...
def validate_auth_rule(doctype_name, total, based_on, cond, company, master_name=""):
	chk = 1
	add_cond1, add_cond2 = "", ""
	...
	if chk == 1:
		...
		get_appr_user_role(appr, doctype_name, total, based_on, cond + add_cond2, master_name, company)


def bifurcate_based_on_type(doctype_name, total, av_dis, based_on, doc_obj, val, company):
	add_cond = ""
	auth_value = av_dis
	...
	if based_on == "Itemwise Discount":
		...
		validate_auth_rule(doctype_name, auth_value, based_on, add_cond, company)


def validate_approving_authority(doctype_name, company, total, doc_obj=""):
	...
	for d in based_on:
		bifurcate_based_on_type(doctype_name, total, av_dis, d, doc_obj, 1, company)
	...
	for d in based_on:
		bifurcate_based_on_type(doctype_name, total, av_dis, d, doc_obj, 2, company)
	...
	for g in final_based_on:
		bifurcate_based_on_type(doctype_name, total, av_dis, g, doc_obj, 0, company)


def get_value_based_rule(doctype_name, employee, total_claimed_amount, company):
	val_lst = []
	val = frappe.db.sql(
		"""select value from `tabAuthorization Rule`
