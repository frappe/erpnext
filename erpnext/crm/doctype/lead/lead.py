...
class Lead(SellingController, CRMNote):
    ...
    if TYPE_CHECKING:
        ...
        source: DF.Link | None
        state: DF.Data | None
        status: DF.Literal["Lead", "Open", "Replied", "Opportunity", "Hold", "Quotation", "Lost Quotation", "Interested", "Converted", "Do Not Contact", "Completed"]
    ...
    def link_to_contact(self):
        ...
        if self.contact_doc:
            self.contact_doc.append(
                "links",
                {"link_doctype": "Lead", "link_name": self.name, "link_title": self.lead_name}
            )
    ...
    def send_mail(self):
        ...
        user_group = frappe.get_all(
            "User Group Member",
            filters={"parent": "Test User Group"},
            fields=["user"]
        )
        if user_group:
            recipients = [u.user for u in user_group if u.user]
            if email_template:
                email_template = frappe.get_doc("Email Template", "Lead Welcome (Team Notification)")
                frappe.sendmail(
    ...
    def remove_link_from_prospect(self):
        ...
        for d in prospects:
            ...
            if len(prospect.get("leads")) == 1:
                ...
                to_remove = None
                for row in prospect.get("leads"):
                    if row.lead == self.name:
                        to_remove = row
    ...
    def has_opportunity(self):
        return frappe.db.get_value(
            "Opportunity",
            {"party_name": self.name, "status": ["!=", "Lost"]}
        )
    ...
    def has_quotation(self):
        return frappe.db.get_value(
            "Quotation",
            {"party_name": self.name, "docstatus": 1, "status": ["!=", "Lost"]}
        )
...
@frappe.whitelist()
def get_lead_emails(doctype, docname):
    ...
    emails = frappe.get_all("Communication", 
        ...
        fields=['name', 'subject', 'content', 'sender', 'creation', 'sent_or_received', 'recipients', 'cc'],
        order_by='creation desc'
    )
    print(emails)
    return emails