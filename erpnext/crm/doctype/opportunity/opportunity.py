...
class Opportunity(TransactionBase, CRMNote):
	...
	def onload(self):
		...
		opportunity_doc_contact_list = [contact for contact in self.get('__onload').get('contact_list') if contact not in ref_doc_contact_list]
		...
		opportunity_doc_addr_list = [addr for addr in self.get('__onload').get('addr_list') if addr not in ref_doc_addr_list]
