...
class calculate_taxes_and_totals(object):
	...
	def validate_item_tax_template(self):
		for item in self.doc.get("items"):
			if not (item.item_code and item.get("item_tax_template")):
				continue
			...
			if taxes := _get_item_tax_template(args, item_taxes + item_group_taxes, for_validate=True):
				if item.item_tax_template in taxes:
					continue
	...
	def _set_in_company_currency(self, doc: Document, fields: list[str]):
		"""set values in base currency"""
		for source_field in fields:
			source_value = flt(doc.get(source_field), doc.precision(source_field))
			target_field = f"base_{source_field}"
			target_value = flt(source_value * self.doc.conversion_rate, doc.precision(target_field))
	...
	def _load_item_tax_rate(self, item_tax_rate: str) -> dict:
	...
	def _get_tax_rate(self, tax, item_tax_map: dict[str, float]) -> float:
		if tax.account_head not in item_tax_map:
			return tax.rate
	...
	def calculate_taxes(self):
		...
		for n, item in enumerate(items):
			...
			for i, tax in enumerate(taxes):
				...
				tax_rate = self._get_tax_rate(tax, item_tax_map)
				current_tax_amount = self.get_current_tax_amount(item, tax, tax_rate)
				if not (self.doc.get("is_consolidated") or tax.get("dont_recompute_tax")):
				...
				if i == 0:
					...
					tax.grand_total_for_current_item = flt(
						taxes[i - 1].grand_total_for_current_item + current_tax_amount
					)
	...
	def get_current_tax_amount(self, item: Document, tax: Document, tax_rate: float) -> float:
		match tax.charge_type:
			case "Actual":
				# distribute the tax amount proportionally to each item row
				actual = flt(tax.tax_amount, tax.precision("tax_amount"))
				return item.net_amount * actual / self.doc.net_total if self.doc.net_total else 0.0
			case "On Net Total":
				return (tax_rate / 100.0) * item.net_amount
			case "On Previous Row Amount":
				return (tax_rate / 100.0) * self.doc.get("taxes")[
					cint(tax.row_id) - 1
				].tax_amount_for_current_item
			case "On Previous Row Total":
				return (tax_rate / 100.0) * self.doc.get("taxes")[
	...
	def round_off_base_values(self, tax):
		...
		if tax.account_head not in frappe.flags.round_off_applicable_accounts:
			return
	...
	def calculate_total_net_weight(self):
		if not self.doc.meta.get_field("total_net_weight"):
			return

		self.doc.total_net_weight = sum(d.total_weight for d in self.doc.items if d.total_weight)
	...
	def apply_discount_amount(self):
		if not self.doc.get("base_discount"):
	...
	def calculate_total_advance(self):
		if self.doc.docstatus.is_submitted():
			return

		total_allocated_amount = sum(
			flt(adv.allocated_amount, adv.precision("allocated_amount")) for adv in self.doc.get("advances")
		)
	...
	def is_internal_invoice(self):
		return (
			self.doc.doctype in ("Sales Invoice", "Purchase Invoice") and self.doc.is_internal_transfer()
		)
	...
	def calculate_write_off_amount(self):
		if not self.doc.get("write_off_outstanding_amount_automatically"):
			return
	...
	def calculate_margin(self, item) -> tuple[float, float]:
	...
	def set_total_amount_to_default_mop(self, total_amount_to_pay):
		if default_mode_of_payment := frappe.db.get_value(
			"POS Payment Method",
...
def get_itemised_tax_breakup_html(doc):
	...
	tax_descriptions = get_tax_descriptions(doc.taxes)
	headers = get_itemised_tax_breakup_header(f"{doc.doctype} Item", tax_descriptions)
	...
	return frappe.render_template(
		...
		dict(
			...
			tax_accounts=tax_descriptions,
			doc=doc,
		),
	)


def get_tax_descriptions(taxes: list[Document]) -> set[str]:
	return list({tax.description for tax in taxes if getattr(tax, "category", None) != "Valuation"})
...
def get_itemised_tax_breakup_data(doc):
	return get_itemised_tax(doc.taxes), get_itemised_taxable_amount(doc.items)
...
def is_last_element(idx, lst) -> bool:
	return idx == len(lst) - 1
