...
class calculate_taxes_and_totals(object):
	...
	def calculate_outstanding_amount(self):
		...
		if self.doc.doctype in ["Sales Invoice", "POS Invoice"]:
		...
		if self.doc.doctype in ["Sales Invoice", "Purchase Invoice", "POS Invoice"]:
			grand_total = self.doc.rounded_total or self.doc.grand_total
			if self.doc.party_account_currency == self.doc.currency:
				total_amount_to_pay = flt(grand_total - self.doc.total_advance, self.doc.precision("grand_total"))
			else:
				total_amount_to_pay = flt(flt(grand_total *
					self.doc.conversion_rate, self.doc.precision("grand_total")) - self.doc.total_advance, self.doc.precision("grand_total"))

			self.doc.round_floats_in(self.doc, ["paid_amount"])
			change_amount = 0

			if self.doc.doctype in ["Sales Invoice", "Purchase Invoice", "POS Invoice"] and not self.doc.get('is_return'):
			...
			self.doc.outstanding_amount = flt(
				total_amount_to_pay - flt(paid_amount) + flt(change_amount) - flt(self.doc.write_off_amount),
