...
class StockController(AccountsController):
	...
	def validate_qi_submission(self, row):
		...
		if qa_docstatus != 1:
			...
			msg = _("Row #{0}: Quality Inspection {1} is not submitted for the item: {2}").format(row.idx, link, row.item_code)
			if action == "Stop":
	...
	def validate_qi_rejection(self, row):
		...
		if qa_status == "Rejected":
			link = frappe.utils.get_link_to_form("Quality Inspection", row.quality_inspection)
			msg = _("Row #{0}: Quality Inspection {1} was rejected for item {2}").format(row.idx, link, row.item_code)
			if action == "Stop":
