...
class BaseAsset(AccountsController):
	...
	def validate_total_num_of_assets(
		self,
		num_of_assets,
		num_of_assets_already_created,
		num_of_items_in_purchase_doc,
		purchase_docname,
	):
		if (num_of_assets_already_created + num_of_assets) > num_of_items_in_purchase_doc:
			...
			frappe.throw(
				_(
					...
					frappe.bold(int(num_of_items_in_purchase_doc)),
					frappe.bold(purchase_docname),
					frappe.bold(int(num_of_assets_already_created)),
				),
	...
	def get_initial_asset_value(self):
		...
		(
			gross_purchase_amount,
			opening_accumulated_depreciation,
		) = self.get_gross_purchase_amount_and_opening_accumulated_depreciation()
	...
	def get_gross_purchase_amount_and_opening_accumulated_depreciation(self):
		if self.doctype == "Asset":
			...
			return (
				self.asset_values["gross_purchase_amount"],
				self.asset_values["opening_accumulated_depreciation"],
			)

	def get_enable_finance_books_value(self):
		self.enable_finance_books = frappe.db.get_single_value(
			"Accounts Settings", "enable_finance_books"
		)
	...
	def validate_depreciation_posting_start_date(self):
		...
		if self.depreciation_posting_start_date == self.available_for_use_date:
			frappe.throw(
				_("Depreciation Posting Date should not be equal to Available for Use Date."),
	...
	def validate_salvage_value(self):
		gross_purchase_amount = (
			self.get("gross_purchase_amount")
			if self.doctype == "Asset"
	...
	def set_missing_template_values(self, row=None):
		...
		(
			...
			asset_life_unit,
			rate_of_depr,
		) = self.fetch_template_values(row_or_doc)
		...
		if (
			row_or_doc.depreciation_method == "Written Down Value" and not row_or_doc.rate_of_depreciation
		):
	...
	def has_updated_basic_depr_details(self):
		return (
			self.has_value_changed("available_for_use_date")
			or self.has_value_changed("gross_purchase_amount")
			or self.has_value_changed("depreciation_posting_start_date")
			or self.has_value_changed("salvage_value")
	...
	def has_updated_template_details(self):
		return (
			self.has_value_changed("depreciation_template")
			or self.has_value_changed("depreciation_method")
			or self.has_value_changed("frequency_of_depreciation")
			or self.has_value_changed("asset_life_in_months")
	...
	def get_status(self):
		if self.docstatus == 0:
			...
			if self.journal_entry_for_scrap:
				...
				self.is_depreciable_asset() and self.is_not_serialized_asset() and self.get("finance_books")
			):
	...
	def cancel_movement_entries(self):
		...
		for movement in movement_entries:
			if movement.docstatus == 1:
				...
				movement_drafts.append(get_link_to_form("Asset Movement", movement.name))
	...
	def get_movement_entries(self):
		...
		movement_names = list(
			set(frappe.get_all("Asset Movement Item", filters=filters, pluck="parent"))
		)

		movement_entries = frappe.get_all(
			"Asset Movement", filters={"name": ["in", movement_names]}, fields=["name", "docstatus"]
		)
	...
	def get_filters(self):
		if self.doctype == "Asset":
			return {"asset": self.name, "serial_no": None, "docstatus": ["<", 2]}
		else:
			return {"asset": self.asset, "serial_no": self.serial_no, "docstatus": ["<", 2]}
	...
	def delete_depreciation_entries(self):
		...
		linked_entries = frappe.get_all("Depreciation Entry", filters=filters, pluck="name")
...
def get_finance_books(asset_category):
	...
	for d in asset_category_doc.finance_books:
		books.append({"finance_book": d.finance_book, "depreciation_template": d.depreciation_template})
...
def fetch_asset_tracking_details(asset):
	...
	if asset.get("serial_no"):
		...
		location, custodian, company = frappe.get_value(
			"Asset", asset.name, ["location", "custodian", "company"]
		)
...
def get_purchase_details(asset):
	...
	if asset.doctype == "Asset":
		...
		purchase_receipt, purchase_invoice = frappe.db.get_value(
			"Asset", asset.asset, ["purchase_receipt", "purchase_invoice"]
		)
...
def get_num_of_items_in_purchase_doc(asset, purchase_doctype, purchase_docname):
	...
	num_of_items_in_purchase_doc = frappe.db.get_value(
		items_doctype, {"parent": purchase_docname, "item_code": item}, "qty"
	)
...
def create_asset_revaluation(asset, asset_category, company):
	...
	asset_revaluation.update(
		{"asset": asset, "company": company, "asset_category": asset_category, "date": getdate()}
	)
...
def get_assets_that_require_maintenance():
	assets = frappe.get_all(
		"Asset",
		filters={"docstatus": 1, "maintenance_required": 1},
...
def has_pending_repairs(asset, serial_no=None):
	return frappe.db.exists(
		"Asset Repair",
		{"asset_name": asset.name, "serial_no": serial_no.name, "repair_status": "Pending"},
	)


def has_maintenance_task_due_today(parent):
	return frappe.db.exists(
		"Asset Maintenance Task", {"parent": parent.name, "next_due_date": getdate()}
	)
...
def get_asset_categories_with_cwip_accounting_enabled():
	return frappe.db.get_all("Asset Category", filters={"enable_cwip_accounting": 1}, pluck="name")
...
def get_serial_nos_that_need_to_be_booked_today(asset_categories):
	...
	return frappe.get_all(
		"Asset Serial No",
		filters={"asset": ["in", assets], "available_for_use_date": nowdate(), "booked_fixed_asset": 0},
		fields=["doctype", "name"],
	)
