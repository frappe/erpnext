...
class AccountsController(TransactionBase):
	...
	def get_args_for_get_item_details(self):
		args = {}
		for fieldname in self.meta.get_valid_columns():
			args[fieldname] = self.get(fieldname)

		if self.doctype in ["Quotation", "Sales Order", "Delivery Note", "Sales Invoice"]:
			document_type = "{} Item".format(self.doctype)
			args.update({"document_type": document_type})

		# party_name field used for customer in quotation
		if self.doctype == "Quotation" and self.quotation_to == "Customer" and args.get("party_name"):
			args.update({"customer": args.get("party_name")})

		args["ignore_pricing_rule"] = self.get('ignore_pricing_rule', 0)
	...
	def set_missing_item_details(self, for_validate=False):
		...
		if hasattr(self, "items"):
			get_item_details_args = self.get_args_for_get_item_details()

			def get_args_for_item(item):
				args = get_item_details_args.copy()
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if self.doctype in ["Purchase Invoice", "Sales Invoice"] \
						...
						item.set('is_fixed_asset', item_details.get('is_fixed_asset', 0))

					if item_details.get("pricing_rules"):
						self.apply_pricing_rule_on_items(item, item_details)
						self.set_pricing_rule_details(item, item_details)

			if self.doctype == "Purchase Invoice":
				self.set_expense_account(for_validate)

			if self.free_items:
				# add all free items together
				# and remove existing invalid free items
