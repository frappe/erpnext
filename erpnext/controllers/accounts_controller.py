...
from Goldfish.buying.utils import update_last_purchase_rate
from Goldfish.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from Goldfish.controllers.sales_and_purchase_return import validate_return
from Goldfish.exceptions import InvalidCurrency
from Goldfish.setup.utils import get_exchange_rate
from Goldfish.stock.doctype.item.item import get_uom_conv_factor
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = Goldfish.get_company_currency(self.company)
	...
	def calculate_taxes_and_totals(self):
		from Goldfish.controllers.taxes_and_totals import calculate_taxes_and_totals
	...
	def validate_due_date(self):
		...
		from Goldfish.accounts.party import validate_due_date
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center",
							self.get("cost_center") or Goldfish.get_default_cost_center(self.company),
						)
	...
	def update_against_document_in_jv(self):
		...
		if lst:
			from Goldfish.accounts.utils import reconcile_against_document
	...
	def on_cancel(self):
		from Goldfish.accounts.doctype.bank_transaction.bank_transaction import (
				...
				self.get_gl_dict(
					{
						...
						"cost_center": self.cost_center or Goldfish.get_default_cost_center(self.company),
					},
					item=self,
				)
			)
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		from Goldfish.controllers.status_updater import get_allowance_for
	...
	def create_advance_and_reconcile(self, party_link):
		...
		reconcilation_entry.cost_center = self.cost_center or Goldfish.get_default_cost_center(self.company)

		advance_entry.account = primary_account
		advance_entry.party_type = primary_party_type
		advance_entry.party = primary_party
		advance_entry.cost_center = self.cost_center or Goldfish.get_default_cost_center(self.company)
	...
	def check_conversion_rate(self):
		default_currency = Goldfish.get_company_currency(self.company)
