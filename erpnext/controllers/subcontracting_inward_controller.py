...
class SubcontractingInwardController:
	...
	def update_sre_for_submit(self):
		if self.purpose == "Subcontracting Delivery" or (
			self.purpose == "Send to Subcontractor"
			and frappe.get_cached_value("Subcontracting Order", self.subcontracting_order, "reserve_stock")
		):
			for item in self.get("items"):
				table = frappe.qb.DocType("Stock Reservation Entry")
				query = (
					...
					.where(
						...
						& (
							...
							== (
								...
								if self.purpose == "Subcontracting Delivery"
								else "Subcontracting Order"
							)
						)
						& (
							table.voucher_no
							== (self.subcontracting_inward_order or self.subcontracting_order)
						)
						& (table.voucher_detail_no == (item.scio_detail or item.sco_rm_detail))
					)
