...
class Subcontracting:
	...
	def set_consumed_qty_in_po(self):
		...
		for doctype in ["Purchase Receipt", "Purchase Invoice"]:
			consumed_items, pr_items = self.update_consumed_materials(doctype, return_consumed_items=True)
	...
	def validate_batch_no(self, row, key):
		if row.get("batch_no") and row.get("batch_no") not in self.transferred_items.get(key).get(
			"batch_no"
		):
