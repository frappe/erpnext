	...
	sl_entries = frappe.db.sql("""
		...
		WHERE
			sle.company = %(company)s
				...
				{s_conditions}
		ORDER BY
			sle.item_code,sle.posting_date asc, sle.posting_time asc, sle.creation asc
		""".format(sle_conditions=get_sle_conditions(filters),s_conditions=get_se_conditions(filters), item_conditions_sql=item_conditions_sql),
		filters, as_dict=1)
...
def get_se_conditions(filters):
	...
	if filters.get("work_order"):
		b=frappe.db.get_all("Stock Entry",{"work_order":filters.get("work_order")},['name'])
		c=[]
		d=[]
		for i in b:
			a=frappe.db.get_all("Stock Ledger Entry",{"voucher_no":i.name},["item_code","batch_no"])
			for j in a:
				c.append(j.get("item_code"))
				d.append(j.get("batch_no"))
		c=tuple(c)
		d=tuple(d)
