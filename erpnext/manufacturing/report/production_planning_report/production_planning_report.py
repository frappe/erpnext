...
class ProductionPlanReport(object):
	...
	def validate_filters(self):
		# Warehouse has already set in the work order
		for field in ["raw_material_warehouse", "include_subassembly_raw_materials"]:
			if self.filters.based_on == 'Work Order' and self.filters.get(field):
				frappe.throw(_("{0} is not required for the work orders")
	...
	def get_open_orders(self):
		...
		self.orders = frappe.db.sql(""" SELECT {fields} from {doctype}
			...
			), tuple(self.filters.docnames), as_dict=1, debug=1)
	...
	def get_raw_materials(self):
		...
		if self.filters.based_on == "Work Order":
			...
			raw_materials = frappe.get_all("Work Order Item",
				fields=["parent", "item_code", "item_name", "source_warehouse as warehouse", "required_qty"],
			...
			raw_materials = frappe.db.sql(""" SELECT bom_item.parent, bom_item.item_code,
					bom_item.item_name, {0} as required_qty
				FROM
