...
def unload_slab_from_oven(rack_name: str, slab_name: str, slab_template: str, values: str):
	...
	return {"rack": rack, "finish_results": res}
...
def finish_heating(job_card):
	...
	slabs = frappe.get_all(
		"Slab",
		filters={"current_job_card": jc.name, "status": "Heating", "docstatus": 0},
		fields=["name"],
		order_by="creation desc",
	)
	if slabs:
		move_slab_to(
			slab_number=slabs[0].name, next_stage="Cooling", job_card_number=jc.name, checkout_and_move=True
		)
