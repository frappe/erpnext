...
frappe.ui.form.on("Production Plan", {
	...
	refresh(frm) {
		...
		frm.trigger("setup_alternate_item_buttons");
	...
	setup_alternate_item_buttons(frm) {
		// The two planning tables are generated by exploding BOMs, so their items are not editable
		// in the grid. Swapping a row for a declared Item Alternative goes through this dialog,
		// which also moves any stock the plan has reserved onto the alternative item.
		if (frm.is_new() || frm.doc.docstatus === 2) return;
		if (["Completed", "Closed"].includes(frm.doc.status)) return;

		const tables = [
			{ fieldname: "sub_assembly_items", label: __("Sub Assembly Items") },
			{ fieldname: "mr_items", label: __("Raw Materials") },
		];

		tables.forEach((table) => {
			if (!frm.doc[table.fieldname]?.length) return;

			frm.add_custom_button(
				table.label,
				() => erpnext.production_plan.show_alternate_item_dialog(frm, table),
				__("Alternate Item")
			);
		});
	},
...
erpnext.production_plan = erpnext.production_plan || {};

erpnext.production_plan.show_alternate_item_dialog = function (frm, table) {
	// the plan is rewritten server side, so unsaved grid edits would be discarded
	if (frm.is_dirty()) {
		frappe.throw(__("Please save the Production Plan before substituting items."));
	}

	frm.call({
		method: "get_substitutable_rows",
		doc: frm.doc,
		args: { table_name: table.fieldname },
		freeze: true,
	}).then((r) => {
		const rows = r.message || [];
		const open_rows = rows.filter((row) => !row.blocked_by);

		if (!rows.length) {
			frappe.msgprint({
				title: __("No Alternatives Available"),
				message: __(
					"No item in {0} has an alternative defined. Enable <b>Allow Alternative Item</b> on the item and add an <a href='/app/item-alternative'>Item Alternative</a> record first.",
					[table.label]
				),
				indicator: "orange",
			});
		...
		if (!open_rows.length) {
			frappe.msgprint({
				title: __("Substitution Not Allowed"),
				message: erpnext.production_plan.blocked_rows_html(rows),
				indicator: "orange",
			});
			return;
		}

		erpnext.production_plan.build_alternate_item_dialog(frm, table, rows, open_rows);
	});
};

erpnext.production_plan.blocked_rows_html = function (rows) {
...
erpnext.production_plan.build_alternate_item_dialog = function (frm, table, rows, open_rows) {
	const is_sub_assembly = table.fieldname === "sub_assembly_items";
	const data = open_rows.map((row) => ({
		docname: row.docname,
		item_code: row.item_code,
		alternate_item: "",
		bom_no: "",
	...
	if (is_sub_assembly) {
		fields.push({
			...
			in_list_view: 1,
			columns: 2,
			description: __("Leave blank to use the alternative item's default BOM"),
			get_query: (row) => {
				return {
					filters: { item: row.alternate_item, docstatus: 1, is_active: 1 },
				};
			},
		});
	}
	...
	const reservation_note =
		frm.doc.docstatus === 1 && frm.doc.reserve_stock
			? `<div class="alert alert-warning">${__(
					"Stock reserved for the original item will be released and re-reserved against the alternative item."
			  )}</div>`
			: "";

	const dialog = new frappe.ui.Dialog({
		title: __("Select Alternate Item: {0}", [table.label]),
		size: "extra-large",
		fields: [
			{ fieldtype: "HTML", fieldname: "reservation_note", options: reservation_note },
			{
				fieldtype: "Table",
				fieldname: "items",
				cannot_add_rows: true,
			...
			{
				fieldtype: "HTML",
				fieldname: "blocked_rows",
				options: erpnext.production_plan.blocked_rows_html(rows),
			},
		],
		primary_action_label: __("Substitute"),
		primary_action() {
			const substitutions = (dialog.get_values().items || [])
				.filter((row) => row.alternate_item && row.alternate_item !== row.item_code)
				.map((row) => {
					return {
						docname: row.docname,
						alternate_item: row.alternate_item,
						bom_no: row.bom_no || null,
					};
				});

			if (!substitutions.length) {
				frappe.throw(__("Select at least one alternate item."));
			}

			dialog.hide();
			frm.call({
				method: "substitute_alternative_items",
				doc: frm.doc,
				args: { table_name: table.fieldname, substitutions: substitutions },
				freeze: true,
				freeze_message: __("Substituting items..."),
			}).then(() => frm.reload_doc());
		},
	});
