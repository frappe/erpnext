...
frappe.ui.form.on("Production Plan", {
	...
	setup_queries(frm) {
		frm.set_query("sales_order", "sales_orders", () => {
			return {
				query: "Goldfish.manufacturing.doctype.production_plan.production_plan.sales_order_query",
				filters: {
		...
		const projected_qty_formula = ` <table class="table table-bordered" style="background-color: var(--scrollbar-track-color);">
			<tr><td style="padding-left:25px">
				...
				<h3 style="text-decoration: underline;">
					<a href = "https://Goldfish.com/docs/user/manual/en/stock/projected-quantity">
	...
	get_items_for_material_requests(frm, warehouses) {
		frappe.call({
			method: "Goldfish.manufacturing.doctype.production_plan.production_plan.get_items_for_material_requests",
			freeze: true,
			args: {
				doc: frm.doc,
		...
		frappe.prompt(
			...
			(row) => {
				let get_template_url =
					"Goldfish.manufacturing.doctype.production_plan.production_plan.download_raw_materials";
...
frappe.ui.form.on("Production Plan Item", {
	item_code(frm, cdt, cdn) {
		...
		if (row.item_code) {
			frappe.call({
				method: "Goldfish.manufacturing.doctype.production_plan.production_plan.get_item_data",
				args: {
...
frappe.ui.form.on("Material Request Plan Item", {
	warehouse(frm, cdt, cdn) {
		...
		if (row.warehouse && row.item_code && frm.doc.company) {
			frappe.call({
				method: "Goldfish.manufacturing.doctype.production_plan.production_plan.get_bin_details",
				args: {
					row: row,
					company: frm.doc.company,
	...
	sales_order(frm, cdt, cdn) {
		...
		if (row.sales_order) {
			frm.call({
				...
				callback(r) {
					frappe.call({
						method: "Goldfish.manufacturing.doctype.production_plan.production_plan.get_so_details",
						args: { sales_order },
						callback(r) {
...
frappe.ui.form.on("Production Plan Sub Assembly Item", {
	fg_warehouse(frm, cdt, cdn) {
		Goldfish.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "sub_assembly_items", "fg_warehouse");
	},
});
