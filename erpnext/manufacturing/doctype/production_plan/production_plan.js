...
frappe.ui.form.on("Production Plan", {
	...
	combine_items(frm) {
		...
		if( frm.doc.po_items.length > 0){
			// attributes list
			const doc_arr = ['item_code', 'bom_no', 'planned_qty', 'pending_qty', 'stock_uom',
				'warehouse', 'description', 'sales_order', 'sales_order_item', 'Is_combined'];
			...
			if (frm.doc.combine_items) {
				...
				for (var i = 0; i < tmp_obj.length; i++){
					...
					// find same po_items
					var tmp_index = frm.doc.po_items.findIndex((obj) => obj.item_code == tmp_obj[i].item_code && obj.bom_no == tmp_obj[i].bom_no);
					...
					if(-1 == tmp_index){
						...
						if (!tmp_idx.includes(tmp_index)){
							...
							frm.doc.tmp_po_items.push(JSON.parse(JSON.stringify(frm.doc.po_items[tmp_index])));
							frm.doc.tmp_category += 1;
						...
						frm.doc.po_items[tmp_index].hasOwnProperty("sales_order") && delete frm.doc.po_items[tmp_index].sales_order;
				...
				if (frm.doc.hasOwnProperty("tmp_po_items")){
					...
					for(var i = 0; i < tmp_obj.length; i++){
						...
						for (var key in tmp_obj[i]){
							doc_arr.includes(key) && (frm.doc.po_items[i][key] = JSON.parse(JSON.stringify(tmp_obj[i][key])));
						}
					}
