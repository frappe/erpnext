...
class VATRETURN(Document):
	...
	def get_sales(self):
		...
		(select sum(xsi.total) from `tabSales Invoice` as xsi where si.country != "Nepal" and year(xsi.posting_date)=year(si.posting_date) 
		...
		), 0) + ifnull( (select sum(xsi.total) from`tabPurchase Invoice` as xsi where si.country != "Nepal" and year(xsi.posting_date)=year(si.posting_date) 
		and si.company=xsi.company and month(xsi.posting_date)=month(si.posting_date) and xsi.docstatus=1 group by month(xsi.posting_date) desc, year(xsi.posting_date)),0)
		...
		case  when si.country = "Nepal" then
		(select sum(xsi.total) from`tabPurchase Invoice` as xsi where si.country != "Nepal" and year(xsi.posting_date)=year(si.posting_date) 
		and si.company=xsi.company and month(xsi.posting_date)=month(si.posting_date) and xsi.docstatus=1 group by month(xsi.posting_date) desc, year(xsi.posting_date)) 
