				...
				for tax_rate in invoice_tax_map[inv]:
					...
					#item_code = itm_cd
						...
						if invoice_tax_map[inv][tax_rate]["tax_rate"] != 0:
							type="OE"
							if invoice_dict[inv]["ecommerce_gstin"] !="":
								type="E"
							datum = [
							         type,
							...
							datum =datum+[
							              invoice_tax_map[inv][tax_rate]["tax_rate"],
							              invoice_tax_map[inv][tax_rate]["inv_value"],
										  invoice_tax_map[inv][tax_rate]["cess"],
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql("""select name,customer_gstin, posting_date, place_of_supply,reverse_charge,invoice_type,ecommerce_gstin,
		...
		where docstatus = 1 and customer_gstin is NULL and base_grand_total<250000 %s order by posting_date desc, name desc""".format(additional_query_columns or '') %
