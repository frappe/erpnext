...
class VehicleRegistrationRegisterReport(object):
	...
	def get_data(self):
		...
		self.data = frappe.db.sql("""
			select m.name, m.vehicle_booking_order,
				...
				m.agent_total, m.agent_commission, m.agent_license_plate_charges, m.agent_payment, m.agent_balance,
	...
	def calculate_group_totals(self, data, group_field, group_value, grouped_by):
		...
		sum_fields = [
			...
			'agent_commission', 'agent_license_plate_charges', 'margin_amount'
		]
		for f in sum_fields:
