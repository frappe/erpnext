...
frappe.ui.form.on('Payment Entry', {
	...
	party: function(frm) {
		...
		if(frm.doc.payment_type && frm.doc.party_type && frm.doc.party) {
			...
			return frappe.call({
				method: "erpnext.accounting.doctype.payment_entry.payment_entry.get_party_details",
				args: {
					company: frm.doc.company,
	...
	set_account_currency_and_balance: function(frm, account, currency_field,
		...
		if (frm.doc.posting_date && account) {
			frappe.call({
				method: "erpnext.accounting.doctype.payment_entry.payment_entry.get_account_details",
				args: {
					"account": account,
					"date": frm.doc.posting_date,
					"cost_center": frm.doc.cost_center
	...
	get_outstanding_documents: function(frm) {
		...
		return  frappe.call({
			method: 'erpnext.accounting.doctype.payment_entry.payment_entry.get_outstanding_reference_documents',
			args: {
				args: {
	...
	set_deductions_entry: function(frm, account) {
		if(frm.doc.difference_amount) {
			frappe.call({
				method: "erpnext.accounting.doctype.payment_entry.payment_entry.get_company_defaults",
				args: {
					company: frm.doc.company
				},
				callback: function(r, rt) {
...
frappe.ui.form.on('Payment Entry Reference', {
	...
	reference_name: function(frm, cdt, cdn) {
		...
		if (row.reference_name && row.reference_doctype) {
			return frappe.call({
				method: "erpnext.accounting.doctype.payment_entry.payment_entry.get_reference_details",
				args: {
...
frappe.ui.form.on('Payment Entry', {
	cost_center: function(frm){
		if (frm.doc.posting_date && (frm.doc.paid_from||frm.doc.paid_to)) {
			return frappe.call({
				method: "erpnext.accounting.doctype.payment_entry.payment_entry.get_party_and_account_balance",
				args: {
					company: frm.doc.company,
					date: frm.doc.posting_date,
