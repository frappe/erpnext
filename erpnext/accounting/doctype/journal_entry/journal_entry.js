	...
	make_inter_company_journal_entry: function(frm) {
		...
		d.set_primary_action(__("Make"), function() {
			...
			frappe.call({
				...
				method: "erpnext.accounting.doctype.journal_entry.journal_entry.make_inter_company_journal_entry",
				callback: function (r) {
					if (r.message) {
...
erpnext.accounting.JournalEntry = frappe.ui.form.Controller.extend({
	onload: function() {
		this.load_defaults();
		this.setup_queries();
	...
	setup_queries: function() {
		...
		me.frm.set_query("reference_name", "accounts", function(doc, cdt, cdn) {
			...
			if(jvd.reference_type==="Journal Entry") {
				...
				return {
					query: "erpnext.accounting.doctype.journal_entry.journal_entry.get_against_jv",
					filters: {
						account: jvd.account,
						party: jvd.party
					}
	...
	get_outstanding: function(doctype, docname, company, child, due_date) {
		...
		return frappe.call({
			method: "erpnext.accounting.doctype.journal_entry.journal_entry.get_outstanding",
			args: { args: args},
			callback: function(r) {
				if(r.message) {
...
cur_frm.script_manager.make(erpnext.accounting.JournalEntry);
...
cur_frm.cscript.voucher_type = function(doc, cdt, cdn) {
	...
	if((!(doc.accounts || []).length) || ((doc.accounts || []).length==1 && !doc.accounts[0].account)) {
		if(in_list(["Bank Entry", "Cash Entry"], doc.voucher_type)) {
			return frappe.call({
				type: "GET",
				method: "erpnext.accounting.doctype.journal_entry.journal_entry.get_default_bank_cash_account",
				args: {
			...
			return frappe.call({
				type:"GET",
				method: "erpnext.accounting.doctype.journal_entry.journal_entry.get_opening_accounts",
				args: {
					"company": doc.company
				},
				callback: function(r) {
...
frappe.ui.form.on("Journal Entry Account", {
	party: function(frm, cdt, cdn) {
		...
		if(!d.account && d.party_type && d.party) {
			...
			return frm.call({
				method: "erpnext.accounting.doctype.journal_entry.journal_entry.get_party_account_and_balance",
				child: d,
				args: {
					company: frm.doc.company,
...
$.extend(erpnext.journal_entry, {
	...
	set_exchange_rate: function(frm, cdt, cdn) {
		...
		if(row.account_currency == company_currency || !frm.doc.multi_currency) {
			...
			frappe.call({
				method: "erpnext.accounting.doctype.journal_entry.journal_entry.get_exchange_rate",
				args: {
...
$.extend(erpnext.journal_entry, {
	set_account_balance: function(frm, dt, dn) {
		...
		if(d.account) {
			...
			return frappe.call({
				method: "erpnext.accounting.doctype.journal_entry.journal_entry.get_account_balance_and_party_type",
				args: {
					account: d.account,
