...
class DocType:
	...
	def get_items(self):
		...
		item_condition = ""
		if self.doc.fg_item:
			item_condition = ' and so_item.item_code = "' + self.doc.fg_item + '"'
		...
		items = webnotes.conn.sql("""select distinct parent, item_code, reserved_warehouse,
			...
			%s""" % (", ".join(["%s"] * len(so_list)), item_condition), tuple(so_list), as_dict=1)


		if self.doc.fg_item:
			item_condition = ' and pi.item_code = "' + self.doc.fg_item + '"'
