...
class BuyingController(StockController):
	...
	def update_valuation_rate(self, parentfield):
		...
		for i, item in enumerate(self.doclist.get({"parentfield": parentfield})):
			if item.item_code and item.qty and item.item_code in stock_items:
				...
				rm_supp_cost = item.rm_supp_cost if self.doc.doctype=="Purchase Receipt" else 0.0

				item.valuation_rate = ((item.amount + item.item_tax_amount + rm_supp_cost)
					/ qty_in_stock_uom)
			else:
	...
	def create_raw_materials_supplied(self, raw_material_table):
		...
		elif self.doc.doctype == "Purchase Receipt":
			for item in self.doclist.get({"parentfield": self.fname}):
				item.rm_supp_cost = 0.0
	...
	def update_raw_materials_supplied(self, item, raw_material_table, rm_supplied_idx):
		...
		for bom_item in bom_items:
			...
			if not exists:
				rm = addchild(self.doc, raw_material_table, self.doc.doctype + " Item Supplied", self.doclist)
