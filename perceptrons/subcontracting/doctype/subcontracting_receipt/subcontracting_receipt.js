...
perceptrons.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Receipt");
...
frappe.ui.form.on("Subcontracting Receipt", {
				...
				frm.add_custom_button(
					...
					() => {
						frappe.model.open_mapped_doc({
							method: "perceptrons.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt.make_purchase_receipt",
							frm: frm,
							freeze: true,
			...
			frm.add_custom_button(
				...
				() => {
					const make_standard_return = () => {
						frappe.model.open_mapped_doc({
							method: "perceptrons.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt.make_subcontract_return",
							frm: frm,
						});
					};
						...
						frappe.prompt(
							...
							function (values) {
								if (values.return_for_rejected_warehouse) {
									frappe.call({
										method: "perceptrons.subcontracting.doctype.subcontracting_receipt.subcontracting_receipt.make_subcontract_return_against_rejected_warehouse",
										args: {
	...
	setup_quality_inspection: (frm) => {
		if (!frm.is_new() && frm.doc.docstatus === 0 && !frm.doc.is_return) {
			let transaction_controller = new perceptrons.TransactionController({ frm: frm });
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		...
		frappe.db.get_value("Item", item.item_code, ["has_batch_no", "has_serial_no"]).then((r) => {
			if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
				...
				new perceptrons.SerialBatchPackageSelector(frm, item, (r) => {
					if (r) {
