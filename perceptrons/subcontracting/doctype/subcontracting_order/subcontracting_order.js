...
perceptrons.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Order");
...
frappe.ui.form.on("Subcontracting Order", {
	...
	set_queries: (frm) => {
		frm.set_query("contact_person", perceptrons.queries.contact_query);
		frm.set_query("supplier_address", perceptrons.queries.address_query);

		frm.set_query("billing_address", perceptrons.queries.company_address_query);

		frm.set_query("shipping_address", () => {
			return perceptrons.queries.company_address_query(frm.doc);
		});
	},
	...
	purchase_order: (frm) => {
		...
		if (frm.doc.purchase_order) {
			perceptrons.utils.map_current_doc({
				method: "perceptrons.buying.doctype.purchase_order.purchase_order.make_subcontracting_order",
					...
					fields: [
						{
							...
							get_query: () => {
								return {
									query: "perceptrons.controllers.queries.get_filtered_child_rows",
									filters: {
	...
	cancel_stock_reservation_entries(frm) {
		...
		frappe
			.call({
				method: "perceptrons.stock.doctype.stock_reservation_entry.stock_reservation_entry.get_stock_reservation_entries_for_voucher",
				args: {
	...
	update_subcontracting_order_status(frm, status) {
		frappe.call({
			method: "perceptrons.subcontracting.doctype.subcontracting_order.subcontracting_order.update_subcontracting_order_status",
			args: {
				sco: frm.doc.name,
				status: status,
			},
	...
	get_materials_from_supplier: function (frm) {
		...
		if (sco_rm_details && sco_rm_details.length) {
			frm.add_custom_button(
				...
				() => {
					frm.call({
						method: "perceptrons.controllers.subcontracting_controller.get_materials_from_supplier",
						freeze: true,
...
perceptrons.buying.SubcontractingOrderController = class SubcontractingOrderController {
	setup() {
	...
	make_subcontracting_receipt(items) {
		frappe.model.open_mapped_doc({
			method: "perceptrons.subcontracting.doctype.subcontracting_order.subcontracting_order.make_subcontracting_receipt",
			frm: cur_frm,
			args: { items: items || [] },
			freeze: true,
	...
	make_stock_entry() {
		frappe.call({
			method: "perceptrons.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
...
extend_cscript(cur_frm.cscript, new perceptrons.buying.SubcontractingOrderController({ frm: cur_frm }));
