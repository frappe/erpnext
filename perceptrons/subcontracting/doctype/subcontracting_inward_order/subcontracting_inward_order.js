...
frappe.ui.form.on("Subcontracting Inward Order", {
	...
	sales_order: (frm) => {
		...
		if (frm.doc.sales_order) {
			perceptrons.utils.map_current_doc({
				method: "perceptrons.selling.doctype.sales_order.sales_order.make_subcontracting_inward_order",
	...
	update_subcontracting_inward_order_status(frm, status) {
		frappe.call({
			method: "perceptrons.subcontracting.doctype.subcontracting_inward_order.subcontracting_inward_order.update_subcontracting_inward_order_status",
			args: {
				scio: frm.doc.name,
				status: status,
			},
