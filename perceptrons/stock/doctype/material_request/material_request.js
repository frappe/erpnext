...
frappe.ui.form.on("Material Request", {
	...
	onload: function (frm) {
		...
		perceptrons.utils.add_item(frm);
	...
	update_status: function (frm, stop_status) {
		frappe.call({
			method: "perceptrons.stock.doctype.material_request.material_request.update_status",
		...
		frappe.prompt(
			{
				...
				get_query: () => {
					return {
						query: "perceptrons.stock.doctype.material_request.material_request.get_default_supplier_query",
			...
			(values) => {
				frappe.model.open_mapped_doc({
					method: "perceptrons.stock.doctype.material_request.material_request.make_purchase_order",
					frm: frm,
	...
	make_in_transit_stock_entry(frm) {
		frappe.prompt(
			...
			(values) => {
				frappe.call({
					method: "perceptrons.stock.doctype.material_request.material_request.make_in_transit_stock_entry",
					args: {
	...
	create_pick_list: (frm) => {
		frappe.model.open_mapped_doc({
			method: "perceptrons.stock.doctype.material_request.material_request.create_pick_list",
			frm: frm,
		});
	},
	...
	raise_work_orders: function (frm) {
		frappe.call({
			method: "perceptrons.stock.doctype.material_request.material_request.raise_work_orders",
			args: {
...
frappe.ui.form.on("Material Request Item", {
	...
	schedule_date: function (frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if (!frm.doc.schedule_date) {
				perceptrons.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
perceptrons.buying.MaterialRequestController = class MaterialRequestController extends (
...
extend_cscript(cur_frm.cscript, new perceptrons.buying.MaterialRequestController({ frm: cur_frm }));

function set_schedule_date(frm) {
	if (frm.doc.schedule_date) {
		perceptrons.utils.copy_value_in_all_rows(
			frm.doc,
			frm.doc.doctype,
			frm.doc.name,
			"items",
