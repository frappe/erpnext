...
from perceptrons.controllers.accounts_controller import merge_taxes
from perceptrons.controllers.buying_controller import BuyingController
from perceptrons.stock.doctype.delivery_note.delivery_note import make_inter_company_transaction
	...
	def make_item_gl_entries(self, gl_entries, inventory_account_map=None):
		...
		for d in self.get("items"):
			...
			if (
				...
				if not (
					(perceptrons.is_perpetual_inventory_enabled(self.company) and d.item_code in stock_items)
...
def make_purchase_invoice(
	...
	from perceptrons.accounts.party import get_payment_terms_template
