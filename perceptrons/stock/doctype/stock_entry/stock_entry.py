...
from perceptrons.accounts.utils import get_account_currency
from perceptrons.buying.utils import check_on_hold_or_closed_status
from perceptrons.controllers.taxes_and_totals import init_landed_taxes_and_totals
...
from perceptrons.stock.stock_ledger import NegativeStockError, get_previous_sle, get_valuation_rate
from perceptrons.stock.utils import get_bin, get_combine_datetime, get_incoming_rate
...
from perceptrons.controllers.subcontracting_inward_controller import SubcontractingInwardController
...
class StockEntry(StockController, SubcontractingInwardController):
	...
	if TYPE_CHECKING:
		...
		from perceptrons.stock.doctype.landed_cost_taxes_and_charges.landed_cost_taxes_and_charges import (
			LandedCostTaxesandCharges,
		)
		from perceptrons.stock.doctype.stock_entry_detail.stock_entry_detail import StockEntryDetail
	...
	def before_validate(self):
		from perceptrons.stock.doctype.putaway_rule.putaway_rule import apply_putaway_rule
	...
	def validate_difference_account(self):
		if not cint(perceptrons.is_perpetual_inventory_enabled(self.company)):
			return
	...
	def validate_same_source_target_warehouse_during_material_transfer(self):
		...
		if frappe.get_single_value("Stock Settings", "validate_material_transfer_warehouses"):
			from perceptrons.stock.doctype.inventory_dimension.inventory_dimension import get_inventory_dimensions
		...
		for d in self.get("items"):
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				...
				d.basic_rate = get_valuation_rate(
					...
					currency=perceptrons.get_company_currency(self.company),
					company=self.company,
	...
	def get_bom_raw_materials(self, qty):
		from perceptrons.manufacturing.doctype.bom.bom import get_bom_items_as_dict
...
def get_expired_batch_items():
	from perceptrons.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_auto_batch_nos
