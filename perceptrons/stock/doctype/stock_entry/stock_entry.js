// Copyright (c) 2015, Hash Include Solutions FZC and Contributors // License: GNU General Public License v3. See license.txt

frappe.provide("perceptrons.stock");
frappe.provide("perceptrons.accounts.dimensions");

perceptrons.landed_cost_taxes_and_charges.setup_triggers("Stock Entry");
...
frappe.ui.form.on("Stock Entry", {
	setup: function (frm) {
		...
		if (frm.doc.purpose == "Receive from Customer") {
			frm.set_query("against_fg", "items", function () {
				return {
					query: "perceptrons.controllers.subcontracting_inward_controller.get_fg_reference_names",
					filters: {
	...
	setup_quality_inspection: function (frm) {
		...
		frm.set_query("quality_inspection", "items", function (doc, cdt, cdn) {
			...
			return {
				query: "perceptrons.stock.doctype.quality_inspection.quality_inspection.quality_inspection_query",
				filters: {
	...
	refresh: function (frm) {
			...
			if (
				...
				frm.add_custom_button(__("End Transit"), function () {
					frappe.model.open_mapped_doc({
						method: "perceptrons.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm,
					});
				});
			}
		...
		if (frm.doc.docstatus === 0 && !frm.doc.subcontracting_inward_order) {
			frm.add_custom_button(
				...
				function () {
					perceptrons.utils.map_current_doc({
						method: "perceptrons.accounts.doctype.purchase_invoice.purchase_invoice.make_stock_entry",
			...
			frm.add_custom_button(
				...
				function () {
					...
					const d = perceptrons.utils.map_current_doc({
						method: "perceptrons.stock.doctype.material_request.material_request.make_stock_entry",
		...
		if (
			...
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method: "perceptrons.stock.doctype.stock_entry.stock_entry.get_expired_batch_items",
						freeze: true,
						callback: function (r) {
							if (!r.exc && r.message) {
								frm.set_value("items", []);
	...
	validate_purpose_consumption: function (frm) {
		frappe
			.call({
				method: "perceptrons.manufacturing.doctype.manufacturing_settings.manufacturing_settings.is_material_consumption_enabled",
			})
			.then((r) => {
	...
	get_warehouse_details: function (frm, cdt, cdn) {
		...
		if (!child.bom_no) {
			frappe.call({
				method: "perceptrons.stock.doctype.stock_entry.stock_entry.get_warehouse_details",
				args: {
					args: {
	...
	get_items_from_bom: function (frm) {
		...
		d.get_input("fetch").on("click", function () {
			...
			frappe.call({
				method: "perceptrons.manufacturing.doctype.bom.bom.get_bom_items",
				args: values,
				callback: function (r) {
					if (!r.message) {
						frappe.throw(__("BOM does not contain any stock item"));
					} else {
						perceptrons.utils.remove_empty_first_row(frm, "items");
	...
	source_warehouse_address: function (frm) {
		perceptrons.utils.get_address_display(frm, "source_warehouse_address", "source_address_display", false);
	},

	target_warehouse_address: function (frm) {
		perceptrons.utils.get_address_display(frm, "target_warehouse_address", "target_address_display", false);
	},
	...
	apply_putaway_rule: function (frm) {
		if (frm.doc.apply_putaway_rule) perceptrons.apply_putaway_rule(frm, frm.doc.purpose);
	},
	...
	purchase_order: (frm) => {
		if (frm.doc.purchase_order) {
			...
			perceptrons.utils.map_current_doc({
				method: "perceptrons.stock.doctype.stock_entry.stock_entry.get_items_from_subcontract_order",
...
frappe.ui.form.on("Stock Entry Detail", {
	...
	uom(doc, cdt, cdn) {
		...
		if (d.uom && d.item_code) {
			return frappe.call({
				method: "perceptrons.stock.doctype.stock_entry.stock_entry.get_uom_details",
				args: {
					item_code: d.item_code,
					uom: d.uom,
					qty: d.qty,
				...
				callback: function (r) {
					if (r.message) {
						...
						if (no_batch_serial_number_value && !frappe.flags.hide_serial_batch_dialog) {
							...
							perceptrons.stock.select_batch_and_serial_no(frm, d);
						} else {
	...
	expense_account(frm, cdt, cdn) {
		perceptrons.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "expense_account");
	},

	cost_center(frm, cdt, cdn) {
		perceptrons.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "cost_center");
	},
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		var child = locals[cdt][cdn];
		perceptrons.stock.select_batch_and_serial_no(frm, child);
	},
});
...
perceptrons.stock.StockEntry = class StockEntry extends perceptrons.stock.StockController {
	setup() {
		var me = this;

		this.barcode_scanner = new perceptrons.utils.BarcodeScanner({
			frm: this.frm,
			warehouse_field: (doc) => {
		...
		this.frm.fields_dict.items.grid.get_field("item_code").get_query = function () {
			return perceptrons.queries.item({ is_stock_item: 1 });
		};
		...
		this.frm.fields_dict.items.grid.get_field("expense_account").get_query = function () {
			if (perceptrons.is_perpetual_inventory_enabled(me.frm.doc.company)) {
				return {
					filters: {
						company: me.frm.doc.company,
						is_group: 0,
		...
		this.frm.set_query("supplier_address", perceptrons.queries.address_query);
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 1 && perceptrons.is_perpetual_inventory_enabled(this.frm.doc.company)) {
			this.show_general_ledger();
		}
		perceptrons.hide_company(this.frm);
		perceptrons.utils.add_item(this.frm);
	...
	set_default_account(company_fieldname, fieldname) {
		...
		return this.frm.call({
			method: "perceptrons.accounts.utils.get_company_default",
			args: {
			...
			return this.frm.call({
				...
				callback: function (r) {
					...
					if (me.frm.doc.bom_no) {
						...
						perceptrons.accounts.dimensions.update_dimension(me.frm, me.frm.doctype);
					}
				},
			});
		}
	...
	work_order() {
		...
		return frappe.call({
			method: "perceptrons.stock.doctype.stock_entry.stock_entry.get_work_order_details",
			args: {
	...
	toggle_related_fields(doc) {
		...
		if (perceptrons.stock.is_subcontracting_or_return_transfer(doc)) {
			doc.customer =
				doc.customer_name =
	...
	supplier(doc) {
		perceptrons.utils.get_party_details(this.frm, null, null, null);
	}
};

perceptrons.stock.select_batch_and_serial_no = (frm, item) => {
	let path = "assets/perceptrons/js/utils/serial_no_batch_selector.js";
...
perceptrons.stock.is_subcontracting_or_return_transfer = (doc) => {
...
function attach_bom_items(bom_no) {
	...
	frappe.db.get_doc("BOM", bom_no).then((bom) => {
		...
		perceptrons.stock.bom = { name, items: {} };
		items.forEach((item) => {
			perceptrons.stock.bom.items[item.item_code] = item;
		});
	});
}

function check_should_not_attach_bom_items(bom_no) {
	return bom_no === undefined || (perceptrons.stock.bom && perceptrons.stock.bom.name === bom_no);
}

extend_cscript(cur_frm.cscript, new perceptrons.stock.StockEntry({ frm: cur_frm }));
