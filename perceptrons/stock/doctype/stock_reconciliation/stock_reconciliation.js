...
frappe.ui.form.on("Stock Reconciliation", {
	setup(frm) {
		...
		frm.barcode_scanner = new perceptrons.utils.BarcodeScanner({
			frm: frm,
			uom_field: "stock_uom",
		});
	},
	...
	get_items: function (frm) {
		...
		frappe.prompt(
			...
			function (data) {
				frappe.call({
					method: "perceptrons.stock.doctype.stock_reconciliation.stock_reconciliation.get_items",
					args: {
	...
	set_valuation_rate_and_qty: function (frm, cdt, cdn) {
		...
		if (d.item_code && d.warehouse) {
			frappe.call({
				method: "perceptrons.stock.doctype.stock_reconciliation.stock_reconciliation.get_stock_balance_for",
				args: {
	...
	toggle_display_account_head: function (frm) {
		frm.toggle_display(
			...
			perceptrons.is_perpetual_inventory_enabled(frm.doc.company)
		);
	},
	...
	set_expense_account: function (frm) {
		if (frm.doc.company && perceptrons.is_perpetual_inventory_enabled(frm.doc.company)) {
			return frm.call({
				method: "perceptrons.stock.doctype.stock_reconciliation.stock_reconciliation.get_difference_account",
				args: {
...
frappe.ui.form.on("Stock Reconciliation Item", {
	...
	add_serial_batch_bundle(frm, cdt, cdn) {
		perceptrons.utils.pick_serial_and_batch_bundle(frm, cdt, cdn, "Inward");
	},
});

perceptrons.stock.StockReconciliation = class StockReconciliation extends perceptrons.stock.StockController {
	setup() {
		var me = this;

		this.setup_posting_date_time_check();

		if (me.frm.doc.company && perceptrons.is_perpetual_inventory_enabled(me.frm.doc.company)) {
	...
	refresh() {
		if (this.frm.doc.docstatus > 0) {
			...
			if (perceptrons.is_perpetual_inventory_enabled(this.frm.doc.company)) {
...
cur_frm.cscript = new perceptrons.stock.StockReconciliation({ frm: cur_frm });
