...
frappe.provide("perceptrons.item");
...
frappe.ui.form.on("Item", {
	...
	onload: function (frm) {
		perceptrons.item.setup_queries(frm);
	...
	refresh: function (frm) {
		...
		if (frm.doc.has_variants) {
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						perceptrons.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						perceptrons.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						perceptrons.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		perceptrons.item.edit_prices_button(frm);
		perceptrons.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			perceptrons.item.make_dashboard(frm);
		}
	...
	validate: function (frm) {
		perceptrons.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		frappe.call({
			method: "perceptrons.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
...
$.extend(perceptrons.item, {
	setup_queries: function (frm) {
		...
		frm.fields_dict.reorder_levels.grid.get_field("warehouse_group").get_query = function (
			...
			let row = locals[cdt][cdn];
			return {
				query: "perceptrons.stock.doctype.warehouse.warehouse.get_warehouses_for_reorder",
				filters: {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require("item-dashboard.bundle.js", function () {
				...
				perceptrons.item.item_dashboard = new perceptrons.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "perceptrons.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				perceptrons.item.item_dashboard.refresh();
			});
		}
	},
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				method: "perceptrons.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				frappe.call({
					method: "perceptrons.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "perceptrons.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					frappe.call({
						method: "perceptrons.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "perceptrons.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
...
frappe.tour["Item"] = [
	...
	},
	{
		...
		description: __(
			"If you are maintaining stock of this Item in your Inventory, Perceptrons will make a stock ledger entry for each transaction of this item."
		),
	},
	{
