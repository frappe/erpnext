...
import perceptrons
from perceptrons import get_company_currency
from perceptrons.accounts.doctype.pricing_rule.pricing_rule import (
	get_pricing_rule_for_item,
	set_transaction_type,
)
from perceptrons.setup.doctype.brand.brand import get_brand_defaults
...
from perceptrons.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from perceptrons.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
from perceptrons.stock.doctype.price_list.price_list import get_price_list_details
...
@frappe.whitelist()
@perceptrons.normalize_ctx_input(ItemDetailsCtx)
...
def update_stock(ctx, out, doc=None):
	from perceptrons.stock.doctype.batch.batch import get_available_batches
	from perceptrons.stock.doctype.serial_no.serial_no import get_serial_nos_for_outward
...
def validate_item_details(ctx: ItemDetailsCtx, item):
	...
	from perceptrons.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(ctx: ItemDetailsCtx, item, overwrite_warehouse=True) -> ItemDetails:
	...
	if item.is_fixed_asset:
		from perceptrons.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
	...
	if ctx.doctype in purchase_doctypes and not frappe.db.get_single_value(
		...
		from perceptrons.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
...
def get_default_cost_center(ctx: ItemDetailsCtx, item=None, item_group=None, brand=None, company=None):
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = f"perceptrons.stock.get_item_details.{method}"
...
def get_price_list_rate(ctx: ItemDetailsCtx, item_doc, out: ItemDetails = None):
	...
	if meta.get_field("currency") or ctx.get("currency"):
		...
		if not ctx.is_internal_supplier and not out.price_list_rate and ctx.transaction_type == "buying":
			from perceptrons.stock.doctype.item.item import get_last_purchase_details
...
def validate_conversion_rate(ctx: ItemDetailsCtx, meta):
	from perceptrons.controllers.accounts_controller import validate_conversion_rate
...
def get_bin_details(
	...
	if warehouse:
		...
		from perceptrons.stock.doctype.warehouse.warehouse import get_child_warehouses
...
def get_batch_qty(batch_no: str, warehouse: str, item_code: str):
	from perceptrons.stock.doctype.batch import batch
