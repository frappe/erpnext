...
import perceptrons
from perceptrons.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_available_serial_nos
...
from perceptrons.stock.valuation import FIFOValuation, LIFOValuation
...
def get_incoming_rate(args: dict | str, raise_error_if_no_rate: bool = True, fallbacks: bool = True):
	...
	from perceptrons.stock.stock_ledger import get_previous_sle, get_valuation_rate
	...
	if in_rate is None:
		...
		in_rate = get_valuation_rate(
			...
			currency=perceptrons.get_company_currency(args.get("company")),
	...
	if reposting_pending and throw_error:
		...
		frappe.msgprint(
			...
			primary_action={
				...
				"client_action": "perceptrons.route_to_pending_reposts",
				"args": filters,
			},
		)
...
def scan_barcode(search_value: str, ctx: dict | str | None = None) -> BarcodeScanResult:
	def set_cache(data: BarcodeScanResult):
		frappe.cache().set_value(f"perceptrons:barcode_scan:{search_value}", data, expires_in_sec=120)
		_update_item_info(data, ctx)

	def get_cache() -> BarcodeScanResult | None:
		data = frappe.cache().get_value(f"perceptrons:barcode_scan:{search_value}")
		if not data:
			return
