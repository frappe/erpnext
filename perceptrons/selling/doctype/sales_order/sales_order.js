...
perceptrons.accounts.taxes.setup_tax_validations("Sales Order");
...
frappe.ui.form.on("Sales Order", {
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus === 0) {
			...
			if (frm.doc.docstatus === 0 && !frm.doc.is_subcontracted) {
				frappe.call({
					method: "perceptrons.selling.doctype.sales_order.sales_order.get_stock_reservation_status",
					callback: function (r) {
						if (!r.message) {
		...
		frm.add_custom_button(
			...
			() => {
				perceptrons.utils.map_current_doc({
					method: "perceptrons.buying.doctype.purchase_order.purchase_order.make_inter_company_sales_order",
	...
	onload: function (frm) {
		...
		frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
			...
			if (row.item_code) {
				query.query = "perceptrons.controllers.queries.warehouse_query";
	...
	get_subcontracting_boms_for_finished_goods: function (fg_item) {
		return frappe.call({
			method: "perceptrons.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_finished_goods",
			args: {
				fg_items: fg_item,
			},
		});
	...
	get_subcontracting_boms_for_service_item: function (service_item) {
		return frappe.call({
			method: "perceptrons.subcontracting.doctype.subcontracting_bom.subcontracting_bom.get_subcontracting_boms_for_service_item",
			args: {
...
frappe.ui.form.on("Sales Order Item", {
	...
	delivery_date: function (frm, cdt, cdn) {
		if (!frm.doc.delivery_date) {
			perceptrons.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "delivery_date");
		}
	},
...
perceptrons.selling.SalesOrderController = class SalesOrderController extends perceptrons.selling.SellingController {
	setup(doc) {
	...
	refresh(doc, dt, dn) {
		...
		if (
			...
			this.frm.add_custom_button(
				__("Quotation"),
				function () {
					let d = perceptrons.utils.map_current_doc({
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.selling.doctype.sales_order.sales_order.create_pick_list",
			frm: this.frm,
		});
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "perceptrons.selling.doctype.sales_order.sales_order.get_work_order_items",
			args: {
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.selling.doctype.sales_order.sales_order.make_material_request",
			frm: this.frm,
		});
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function () {
				...
				me.frm.call({
					method: "perceptrons.selling.doctype.sales_order.sales_order.make_raw_material_request",
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
	...
	make_delivery_note(delivery_dates, for_reserved_stock = false) {
		frappe.model.open_mapped_doc({
			method: "perceptrons.selling.doctype.sales_order.sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.selling.doctype.sales_order.sales_order.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.selling.doctype.sales_order.sales_order.make_project",
			frm: this.frm,
		});
	}
	...
	make_inter_company_order() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.selling.doctype.sales_order.sales_order.make_inter_company_purchase_order",
			frm: this.frm,
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.selling.doctype.sales_order.sales_order.make_maintenance_visit",
			frm: this.frm,
		});
	}
	...
	make_purchase_order() {
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action(args) {
				...
				dialog.hide();
				return frappe.call({
					method: "perceptrons.selling.doctype.sales_order.sales_order.make_purchase_order",
	...
	update_status(label, status) {
		...
		frappe.call({
			method: "perceptrons.selling.doctype.sales_order.sales_order.update_status",
...
extend_cscript(cur_frm.cscript, new perceptrons.selling.SalesOrderController({ frm: cur_frm }));
