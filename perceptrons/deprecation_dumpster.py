...
This file is the final resting place (or should we say, "retirement home"?) for all the deprecated functions and methods of the Perceptrons app. It's like a code nursing home, but with more monkey-patching and less bingo.
...
2. The Perceptrons version at the beginning of which it becomes an error and at the end of which it will be removed (its "graduation" to the great codebase in the sky)
...
class PerceptronsDeprecationError(Warning):
...
class PerceptronsDeprecationWarning(Warning):
	"""Deprecated feature in next version"""


class PendingPerceptronsDeprecationWarning(PerceptronsDeprecationWarning):
...
warnings.simplefilter("error", PerceptronsDeprecationError)
warnings.simplefilter("ignore", PendingPerceptronsDeprecationWarning)


class V15PerceptronsDeprecationWarning(PerceptronsDeprecationError):
	pass


class V16PerceptronsDeprecationWarning(PerceptronsDeprecationWarning):
	pass


class V17PerceptronsDeprecationWarning(PendingPerceptronsDeprecationWarning):
	pass
...
def __get_deprecation_class(graduation: str | None = None, class_name: str | None = None) -> type:
	if graduation:
		...
		class_name = f"{cleaned_graduation}PerceptronsDeprecationWarning"
...
### Party starts here
@deprecated(
	"perceptrons.controllers.taxes_and_totals.get_itemised_taxable_amount",
	"2024-11-07",
	"v17",
...
@deprecated(
	"perceptrons.stock.get_pos_profile_item_details",
	"2024-11-19",
	"v16",
	"Use perceptrons.stock.get_pos_profile_item_details_ with a flipped signature",
)
def get_pos_profile_item_details(company, ctx, pos_profile=None, update_data=False):
	from perceptrons.stock.get_item_details import get_pos_profile_item_details_

	return get_pos_profile_item_details_(ctx, company, pos_profile=pos_profile, update_data=update_data)


@deprecated(
	"perceptrons.stock.get_item_warehouse",
	"2024-11-19",
	"v16",
	"Use perceptrons.stock.get_item_warehouse_ with a flipped signature",
)
def get_item_warehouse(item, ctx, overwrite_warehouse, defaults=None):
	from perceptrons.stock.get_item_details import get_item_warehouse_
