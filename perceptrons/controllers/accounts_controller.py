...
import perceptrons
from perceptrons.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimensions,
)
from perceptrons.accounts.doctype.pricing_rule.utils import (
	apply_pricing_rule_for_free_items,
	apply_pricing_rule_on_transaction,
	get_applied_pricing_rules,
)
from perceptrons.accounts.general_ledger import get_round_off_account_and_cost_center
from perceptrons.accounts.party import (
...
from perceptrons.accounts.utils import (
...
from perceptrons.buying.utils import update_last_purchase_rate
from perceptrons.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from perceptrons.controllers.sales_and_purchase_return import validate_return
from perceptrons.exceptions import InvalidCurrency
from perceptrons.setup.utils import get_exchange_rate
from perceptrons.stock.doctype.item.item import get_uom_conv_factor
from perceptrons.stock.doctype.packed_item.packed_item import make_packing_list
from perceptrons.stock.get_item_details import (
	ItemDetailsCtx,
...
from perceptrons.utilities.regional import temporary_flag
from perceptrons.utilities.transaction_base import TransactionBase
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = perceptrons.get_company_currency(self.company)
	...
	def on_update(self):
		from perceptrons.controllers.taxes_and_totals import process_item_wise_tax_details
	...
	def on_trash(self):
		from perceptrons.accounts.utils import delete_exchange_gain_loss_journal
	...
	def calculate_taxes_and_totals(self):
		from perceptrons.controllers.taxes_and_totals import calculate_taxes_and_totals
	...
	def validate_due_date(self):
		...
		from perceptrons.accounts.party import validate_due_date
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center",
							self.get("cost_center") or perceptrons.get_default_cost_center(self.company),
						)
	...
	def update_against_document_in_jv(self):
		...
		if lst:
			from perceptrons.accounts.utils import reconcile_against_document
	...
	def on_cancel(self):
		from perceptrons.accounts.doctype.bank_transaction.bank_transaction import (
				...
				self.get_gl_dict(
					{
						...
						"cost_center": self.cost_center or perceptrons.get_default_cost_center(self.company),
					},
					item=self,
				)
			)
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		from perceptrons.controllers.status_updater import get_allowance_for
	...
	def get_company_default(self, fieldname, ignore_validation=False):
		from perceptrons.accounts.utils import get_company_default
	...
	def create_advance_and_reconcile(self, party_link):
		...
		default_currency = perceptrons.get_company_currency(self.company)
		...
		reconcilation_entry.cost_center = self.cost_center or perceptrons.get_default_cost_center(self.company)

		advance_entry.account = primary_account
		advance_entry.party_type = primary_party_type
		advance_entry.party = primary_party
		advance_entry.cost_center = self.cost_center or perceptrons.get_default_cost_center(self.company)
...
def update_bin_on_delete(row, doctype):
	...
	from perceptrons.stock.stock_balance import (
		get_indented_qty,
		get_ordered_qty,
		get_reserved_qty,
		update_bin_qty,
...
def update_child_qty_rate(
	...
	from perceptrons.buying.doctype.supplier_quotation.supplier_quotation import get_purchased_items
	from perceptrons.selling.doctype.quotation.quotation import get_ordered_items
	...
	if parent_doctype == "Sales Order" and not parent.is_subcontracted:
		from perceptrons.stock.doctype.stock_reservation_entry.stock_reservation_entry import (
