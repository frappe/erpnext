...
perceptrons.LeadController = class LeadController extends frappe.ui.form.Controller {
	setup() {
	...
	add_lead_to_prospect(frm) {
		frappe.prompt(
			...
			function (data) {
				frappe.call({
					method: "perceptrons.crm.doctype.lead.lead.add_lead_to_prospect",
					args: {
						lead: frm.doc.name,
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.crm.doctype.lead.lead.make_customer",
			frm: this.frm,
		});
	}
	...
	make_quotation() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.crm.doctype.lead.lead.make_quotation",
			frm: this.frm,
		});
	}
...
if (cur_frm) {
	extend_cscript(cur_frm.cscript, new perceptrons.LeadController({ frm: cur_frm }));
}
