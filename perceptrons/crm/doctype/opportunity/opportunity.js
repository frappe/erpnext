...
frappe.provide("perceptrons.crm");
perceptrons.pre_sales.set_as_lost("Opportunity");
...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function (frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			perceptrons.utils.get_party_details(frm);
		} else if (frm.doc.opportunity_from == "Lead") {
			perceptrons.utils.map_current_doc({
				method: "perceptrons.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function (frm, cdt, cdn) {
		perceptrons.utils.get_address_display(frm, "customer_address", "address_display", false);
	},

	contact_person: perceptrons.utils.get_contact_details,
	...
	refresh: function (frm) {
		...
		if (!frm.is_new() && doc.status !== "Lost") {
			...
			let company_currency = perceptrons.get_currency(frm.doc.company);
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "perceptrons.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "perceptrons.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm,
		});
	},
...
perceptrons.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		me.frm.set_query("customer_address", perceptrons.queries.address_query);
		...
		me.frm.set_query("contact_person", perceptrons.queries["contact_query"]);

		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query("party_name", perceptrons.queries["lead"]);
		} else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query("party_name", perceptrons.queries["customer"]);
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.crm.doctype.opportunity.opportunity.make_quotation",
			frm: this.frm,
		});
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.crm.doctype.opportunity.opportunity.make_customer",
			frm: this.frm,
		});
	}

	show_notes() {
		const crm_notes = new perceptrons.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new perceptrons.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new perceptrons.crm.Opportunity({ frm: cur_frm }));

cur_frm.cscript.item_code = function (doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "perceptrons.crm.doctype.opportunity.opportunity.get_item_details",
