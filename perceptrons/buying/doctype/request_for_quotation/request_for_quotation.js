	...
	refresh: function (frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method: "perceptrons.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails",
						freeze: true,
						args: {
						...
						(data) => {
							var w = window.open(
								frappe.urllib.get_full_url(
									"/api/method/perceptrons.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?" +
										new URLSearchParams({
											name: frm.doc.name,
	...
	make_supplier_quotation: function (frm) {
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action: (args) => {
				...
				return frappe.call({
					type: "GET",
					method: "perceptrons.buying.doctype.request_for_quotation.request_for_quotation.make_supplier_quotation_from_rfq",
					args: {
...
frappe.ui.form.on("Request for Quotation Supplier", {
	supplier: function (frm, cdt, cdn) {
		...
		frappe.call({
			method: "perceptrons.accounts.party.get_party_details",
			args: {
				party: d.supplier,
				party_type: "Supplier",
			},
...
perceptrons.buying.RequestforQuotationController = class RequestforQuotationController extends (
	...
	refresh() {
		...
		if (this.frm.doc.docstatus === 0) {
			this.frm.add_custom_button(
				...
				function () {
					perceptrons.utils.map_current_doc({
						method: "perceptrons.stock.doctype.material_request.material_request.make_request_for_quotation",
				...
				function () {
					...
					var dialog = new frappe.ui.Dialog({
						...
						primary_action: (args) => {
							...
							perceptrons.utils.map_current_doc({
								method: "perceptrons.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			this.frm.add_custom_button(
				...
				function () {
					perceptrons.buying.link_to_mrs(me.frm);
				},
				__("Tools")
			);
			...
			fields: [
				{
					...
					onchange() {
						if (dialog.get_value("search_type") == "Tag") {
							frappe
								.call({
									method: "perceptrons.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag",
								})
								.then((r) => {
...
extend_cscript(cur_frm.cscript, new perceptrons.buying.RequestforQuotationController({ frm: cur_frm }));
