...
from perceptrons.setup.setup_wizard.operations.taxes_setup import setup_taxes_and_charges
from perceptrons.stock.utils import check_pending_reposting
...
class Company(NestedSet):
	...
	def create_default_accounts(self):
		from perceptrons.accounts.doctype.account.chart_of_accounts.chart_of_accounts import create_charts
...
def install_country_fixtures(company, country):
	try:
		module_name = f"perceptrons.regional.{frappe.scrub(country)}.setup.setup"
...
def update_company_current_month_sales(company):
	...
	# Fieldname in standard Perceptrons is `total_monthly_sales`
	frappe.db.set_value(
		"Company",
		company,
		"total_monthly_sales",
