...
frappe.ui.form.on("Transaction Deletion Record", {
	...
	onload: function (frm) {
		if (frm.doc.docstatus == 0) {
			...
			frappe.call({
				method: "perceptrons.setup.doctype.transaction_deletion_record.transaction_deletion_record.get_protected_doctypes",
				callback: function (r) {
					if (r.message) {
			...
			frappe.call({
				method: "perceptrons.setup.doctype.transaction_deletion_record.transaction_deletion_record.get_doctypes_to_be_ignored",
				callback: function (r) {
		...
		if (frm.doc.docstatus == 0) {
			...
			if (frm.doc.doctypes_to_delete && frm.doc.doctypes_to_delete.length > 0) {
				frm.add_custom_button(
					...
					() => {
						open_url_post(
							"/api/method/perceptrons.setup.doctype.transaction_deletion_record.transaction_deletion_record.export_to_delete_template",
							{
								name: frm.doc.name,
							}
						);
			...
			frm.add_custom_button(
				...
				() => {
					new frappe.ui.FileUploader({
						...
						on_success: (file_doc) => {
							frappe.call({
								method: "perceptrons.setup.doctype.transaction_deletion_record.transaction_deletion_record.process_import_template",
								args: {
...
frappe.ui.form.on("Transaction Deletion Record To Delete", {
	doctype_name: function (frm, cdt, cdn) {
		...
		if (row.doctype_name) {
			...
			frappe.call({
				method: "perceptrons.setup.doctype.transaction_deletion_record.transaction_deletion_record.get_company_link_fields",
				args: {
