...
class ProcessPeriodClosingVoucher(Document):
	...
	if TYPE_CHECKING:
		...
		from perceptrons.accounts.doctype.process_period_closing_voucher_detail.process_period_closing_voucher_detail import (
		...
		if normal_balances := frappe.db.get_all(
			...
			if not is_scheduler_inactive():
				for x in normal_balances:
					...
					frappe.enqueue(
						method="perceptrons.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.process_individual_date",
						queue="long",
						timeout="3600",
						is_async=True,
