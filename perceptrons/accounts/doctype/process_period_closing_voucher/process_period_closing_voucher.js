	...
	refresh(frm) {
		if (frm.doc.docstatus == 1 && ["Queued"].find((x) => x == frm.doc.status)) {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					method: "perceptrons.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.start_pcv_processing",
					args: {
		...
		if (frm.doc.docstatus == 1 && ["Running"].find((x) => x == frm.doc.status)) {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					method: "perceptrons.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.pause_pcv_processing",
					args: {
						docname: frm.doc.name,
					},
				}).then((r) => {
		...
		if (frm.doc.docstatus == 1 && ["Paused"].find((x) => x == frm.doc.status)) {
			...
			frm.add_custom_button(execute_btn, () => {
				frm.call({
					method: "perceptrons.accounts.doctype.process_period_closing_voucher.process_period_closing_voucher.resume_pcv_processing",
					args: {
						docname: frm.doc.name,
					},
				}).then((r) => {
