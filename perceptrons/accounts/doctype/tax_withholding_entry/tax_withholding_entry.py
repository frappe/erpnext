...
from perceptrons.accounts.doctype.tax_withholding_category.tax_withholding_category import (
...
class TaxWithholdingController:
	...
	def _create_tax_row(self, account_head, tax_amount):
		cost_center = self.doc.cost_center or perceptrons.get_default_cost_center(self.doc.company)
...
class JournalTaxWithholding(TaxWithholdingController):
	...
	def _calculate_net_total(self):
		from perceptrons.accounts.report.general_ledger.general_ledger import get_account_type_map
		...
		if not tax_row:
			tax_row = self.doc.append(
				"accounts",
				{
					...
					or perceptrons.get_default_cost_center(self.doc.company),
					"credit": 0,
					"credit_in_account_currency": 0,
					"debit": 0,
					"debit_in_account_currency": 0,
