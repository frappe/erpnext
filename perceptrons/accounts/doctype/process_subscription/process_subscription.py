...
from perceptrons.accounts.doctype.subscription.subscription import DateTimeLikeObject
...
class ProcessSubscription(Document):
	...
	def process_all_subscription(self):
		...
		for subscription in create_batch(subscriptions, 500):
			frappe.enqueue(
				method="perceptrons.accounts.doctype.subscription.subscription.process_all",
				queue="long",
