...
frappe.provide("perceptrons.accounts");

cur_frm.cscript.tax_table = "Sales Taxes and Charges";

perceptrons.accounts.taxes.setup_tax_validations("Sales Invoice");
perceptrons.accounts.payment_triggers.setup("Sales Invoice");
perceptrons.accounts.pos.setup("Sales Invoice");
perceptrons.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
perceptrons.sales_common.setup_selling_controller();
perceptrons.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	perceptrons.selling.SellingController
) {
	setup(doc) {
	...
	company() {
		super.company();
		perceptrons.accounts.dimensions.update_dimension(this.frm, this.frm.doctype);
	...
	onload() {
		...
		perceptrons.queries.setup_queries(this.frm, "Warehouse", function () {
			return perceptrons.queries.warehouse(me.frm.doc);
		});
		...
		perceptrons.queries.setup_warehouse_query(this.frm);
	}
	...
	refresh(doc, dt, dn) {
		...
		perceptrons.accounts.ledger_preview.show_accounting_ledger_preview(this.frm);

		if (doc.update_stock) {
			this.show_stock_ledger();
			perceptrons.accounts.ledger_preview.show_stock_ledger_preview(this.frm);
		}
		...
		perceptrons.accounts.unreconcile_payment.add_unreconcile_btn(me.frm);
	...
	make_invoice_discounting() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.accounts.doctype.sales_invoice.sales_invoice.create_invoice_discounting",
			frm: this.frm,
		});
	}
	...
	make_dunning() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.accounts.doctype.sales_invoice.sales_invoice.create_dunning",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.accounts.doctype.sales_invoice.sales_invoice.make_maintenance_schedule",
			frm: this.frm,
		});
	}
	...
	sales_order_btn() {
		...
		this.$sales_order_btn = this.frm.add_custom_button(
			__("Sales Order"),
			function () {
				perceptrons.utils.map_current_doc({
					method: "perceptrons.selling.doctype.sales_order.sales_order.make_sales_invoice",
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				perceptrons.utils.map_current_doc({
					method: "perceptrons.selling.doctype.quotation.quotation.make_sales_invoice",
			...
			function () {
				perceptrons.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "perceptrons.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
					allow_child_item_selection: true,
	...
	customer() {
		...
		perceptrons.utils.get_party_details(
			this.frm,
			"perceptrons.accounts.party.get_party_details",
			{
		...
		if (this.frm.doc.customer) {
			frappe.call({
				method: "perceptrons.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	make_inter_company_invoice() {
		let me = this;
		frappe.model.open_mapped_doc({
			method: "perceptrons.accounts.doctype.sales_invoice.sales_invoice.make_inter_company_purchase_invoice",
			frm: me.frm,
		});
	}
	...
	items_on_form_rendered() {
		perceptrons.setup_serial_or_batch_no();
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.accounts.doctype.sales_invoice.sales_invoice.make_sales_return",
			frm: this.frm,
		});
	}
	...
	asset(frm, cdt, cdn) {
		...
		if (row.asset) {
			frappe.call({
				method: perceptrons.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
...
extend_cscript(cur_frm.cscript, new perceptrons.accounts.SalesInvoiceController({ frm: cur_frm }));

cur_frm.cscript["Make Delivery Note"] = function () {
	frappe.model.open_mapped_doc({
		method: "perceptrons.accounts.doctype.sales_invoice.sales_invoice.make_delivery_note",
		frm: cur_frm,
	});
};

cur_frm.cscript.income_account = function (doc, cdt, cdn) {
	perceptrons.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
};

cur_frm.cscript.expense_account = function (doc, cdt, cdn) {
	perceptrons.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
};
...
frappe.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		frm.set_query("income_account", "items", function (doc) {
			return {
				query: "perceptrons.controllers.queries.get_income_account",
				filters: {
					company: doc.company,
					disabled: 0,
				},
		...
		frm.set_query("time_sheet", "timesheets", function (doc, cdt, cdn) {
			return {
				query: "perceptrons.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "perceptrons.accounts.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			frappe.call({
				method: "perceptrons.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			frappe.call({
				method: "perceptrons.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
	...
	async get_timesheet_data(frm, kwargs) {
		return frappe
			.call({
				method: "perceptrons.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
					...
					let d = new frappe.ui.Dialog({
						...
						fields: [
							...
							},
							{
								...
								get_query: () => {
									return {
										query: "perceptrons.controllers.queries.item_query",
										filters: {
											is_sales_item: 1,
											customer: frm.doc.customer,
											has_variants: 0,
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	frappe.call({
		method: "perceptrons.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},
