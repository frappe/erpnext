...
import perceptrons
from perceptrons.accounts.deferred_revenue import get_deferred_booking_accounts
from perceptrons.accounts.doctype.invoice_discounting.invoice_discounting import (
...
from perceptrons.accounts.doctype.tax_withholding_entry.tax_withholding_entry import JournalTaxWithholding
...
from perceptrons.controllers.accounts_controller import AccountsController
from perceptrons.setup.utils import get_exchange_rate as _get_exchange_rate
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from perceptrons.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def validate_company_for_periodic_accounting(self):
		if perceptrons.is_perpetual_inventory_enabled(self.company):
			frappe.throw(
				_(
	...
	def build_gl_map(self):
		...
		company_currency = perceptrons.get_company_currency(self.company)
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		from perceptrons.accounts.general_ledger import make_gl_entries
				...
				if not blank_row:
					blank_row = self.append(
						"accounts",
						{
							...
							"cost_center": perceptrons.get_default_cost_center(self.company),
						},
					)
...
def get_outstanding(args: str | dict):
	...
	company_currency = perceptrons.get_company_currency(args.get("company"))
	due_date = None
...
def get_account_details_and_party_type(
	...
	company_currency = perceptrons.get_company_currency(company)
	account_details = frappe.get_cached_value(
