...
frappe.provide("perceptrons.journal_entry");
	...
	refresh: function (frm) {
		...
		if (frm.doc.docstatus == 1) {
			frm.add_custom_button(
				...
				function () {
					return perceptrons.journal_entry.reverse_journal_entry(frm);
				},
				__("Actions")
			);
		}
		...
		if (frm.doc.__islocal) {
			frm.add_custom_button(__("Quick Entry"), function () {
				return perceptrons.journal_entry.quick_entry(frm);
			});
		}

		// hide /unhide fields based on currency
		perceptrons.journal_entry.toggle_fields_based_on_currency(frm);
		...
		if (frm.doc.voucher_type !== "Exchange Gain Or Loss") {
			$.each(frm.doc.accounts || [], function (i, row) {
				perceptrons.journal_entry.set_exchange_rate(frm, row.doctype, row.name);
			});
		}
	},
	...
	make_inter_company_journal_entry: function (frm) {
		...
		d.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				...
				method: "perceptrons.accounts.doctype.journal_entry.journal_entry.make_inter_company_journal_entry",
				callback: function (r) {
					if (r.message) {
...
perceptrons.accounts.JournalEntry = class JournalEntry extends frappe.ui.form.Controller {
	onload() {
		this.load_defaults();
		this.setup_queries();
		perceptrons.accounts.dimensions.setup_dimension_filters(this.frm, this.frm.doctype);
	}
	...
	setup_queries() {
		...
		me.frm.set_query("account", "accounts", function (doc, cdt, cdn) {
			return perceptrons.journal_entry.account_query(me.frm);
		});
		...
		me.frm.set_query("reference_name", "accounts", function (doc, cdt, cdn) {
			...
			if (jvd.reference_type === "Journal Entry") {
				...
				return {
					query: "perceptrons.accounts.doctype.journal_entry.journal_entry.get_against_jv",
					filters: {
						account: jvd.account,
						party: jvd.party,
					},
	...
	get_outstanding(doctype, docname, company, child) {
		...
		return frappe.call({
			method: "perceptrons.accounts.doctype.journal_entry.journal_entry.get_outstanding",
			args: { args: args },
			callback: function (r) {
				if (r.message) {
	...
	accounts_add(doc, cdt, cdn) {
		...
		perceptrons.accounts.dimensions.copy_dimension_from_first_row(this.frm, cdt, cdn, "accounts");
	}
};

cur_frm.script_manager.make(perceptrons.accounts.JournalEntry);
...
frappe.ui.form.on("Journal Entry Account", {
	party: function (frm, cdt, cdn) {
		...
		if (!d.account && d.party_type && d.party) {
			...
			return frm.call({
				method: "perceptrons.accounts.doctype.journal_entry.journal_entry.get_party_account_and_currency",
				child: d,
				args: {
					company: frm.doc.company,
	...
	account: function (frm, dt, dn) {
		perceptrons.journal_entry.set_account_details(frm, dt, dn);
	},

	debit_in_account_currency: function (frm, cdt, cdn) {
		perceptrons.journal_entry.set_exchange_rate(frm, cdt, cdn);
	},
	...
	exchange_rate: function (frm, cdt, cdn) {
		...
		perceptrons.journal_entry.set_debit_credit_in_company_currency(frm, cdt, cdn);
	},
});
...
$.extend(perceptrons.journal_entry, {
	...
	set_exchange_rate: function (frm, cdt, cdn) {
		...
		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			...
			frappe.call({
				method: "perceptrons.accounts.doctype.journal_entry.journal_entry.get_exchange_rate",
				args: {
		...
		var dialog = new frappe.ui.Dialog({
			...
			fields: [
				{ fieldtype: "Currency", fieldname: "debit", label: __("Amount"), reqd: 1 },
				{
					...
					options: "Account",
					get_query: function () {
						return perceptrons.journal_entry.account_query(frm);
					},
				},
				{
					fieldtype: "Link",
	...
	reverse_journal_entry: function (frm) {
		frappe.model.open_mapped_doc({
			method: "perceptrons.accounts.doctype.journal_entry.journal_entry.make_reverse_journal_entry",
			frm: frm,
		});
	},
});
...
$.extend(perceptrons.journal_entry, {
	set_account_details: function (frm, dt, dn) {
		...
		if (d.account) {
			...
			return frappe.call({
				method: "perceptrons.accounts.doctype.journal_entry.journal_entry.get_account_details_and_party_type",
				args: {
					account: d.account,
				...
				callback: function (r) {
					if (r.message) {
						$.extend(d, r.message);
						perceptrons.journal_entry.set_amount_on_last_row(frm, dt, dn);
						perceptrons.journal_entry.set_debit_credit_in_company_currency(frm, dt, dn);
						refresh_field("accounts");
					}
				},
			});
		} else {
			perceptrons.journal_entry.clear_fields(frm, dt, dn);
		}
	},
