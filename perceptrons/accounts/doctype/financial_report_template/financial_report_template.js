...
async function refresh_tree_view(dialog, account_rows) {
	...
	const { message: filtered_accounts } = await frappe.call({
		method: "perceptrons.accounts.doctype.financial_report_template.financial_report_engine.get_filtered_accounts",
	...
	const tree = new FilteredTree({
		...
		label: company,
		root_value: company,
		method: "perceptrons.accounts.doctype.financial_report_template.financial_report_engine.get_children_accounts",
	...
	if (data_source === "Account Data") {
		...
		description_html = `
			<div ${container_style}>
				...
				<ul ${list_style}>
					<li><code>perceptrons.custom.financial_apis.get_custom_revenue</code></li>
