...
perceptrons.accounts.pos.setup("POS Invoice");
perceptrons.selling.POSInvoiceController = class POSInvoiceController extends perceptrons.selling.SellingController {
	settings = {};
	...
	onload(doc) {
		...
		this.frm.set_query("item_code", "items", function (doc) {
			return {
				query: "perceptrons.accounts.doctype.pos_invoice.pos_invoice.item_query",
				filters: {
					has_variants: ["=", 0],
					is_sales_item: ["=", 1],
					disabled: ["=", 0],
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.accounts.doctype.pos_invoice.pos_invoice.make_sales_return",
			frm: this.frm,
		});
	}
...
extend_cscript(cur_frm.cscript, new perceptrons.selling.POSInvoiceController({ frm: cur_frm }));
