...
frappe.ui.form.on("Journal Entry Template", {
	onload: function (frm) {
		...
		if (frm.is_new()) {
			frappe.call({
				type: "GET",
				method: "perceptrons.accounts.doctype.journal_entry_template.journal_entry_template.get_naming_series",
				callback: function (r) {
					if (r.message) {
	...
	refresh: function (frm) {
		...
		frm.set_query("party_type", "accounts", function (doc, cdt, cdn) {
			...
			return {
				query: "perceptrons.setup.doctype.party_type.party_type.get_party_type",
				filters: {
					account: row.account,
				},
			};
	...
	voucher_type: function (frm) {
		...
		switch (frm.doc.voucher_type) {
			...
			case "Cash Entry":
				frappe.call({
					type: "GET",
					method: "perceptrons.accounts.doctype.journal_entry.journal_entry.get_default_bank_cash_account",
					args: {
						account_type:
