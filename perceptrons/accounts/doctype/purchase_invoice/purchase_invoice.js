...
perceptrons.accounts.payment_triggers.setup("Purchase Invoice");
perceptrons.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
perceptrons.accounts.taxes.setup_tax_validations("Purchase Invoice");
perceptrons.buying.setup_buying_controller();

perceptrons.accounts.PurchaseInvoice = class PurchaseInvoice extends perceptrons.buying.BuyingController {
	setup(doc) {
			...
			this.frm.add_custom_button(
				...
				function () {
					perceptrons.utils.map_current_doc({
						method: "perceptrons.buying.doctype.purchase_order.purchase_order.make_purchase_invoice",
	...
	unblock_invoice() {
		const me = this;
		frappe.call({
			method: "perceptrons.accounts.doctype.purchase_invoice.purchase_invoice.unblock_invoice",
	...
	make_comment_dialog_and_block_invoice() {
		...
		this.dialog.set_primary_action(__("Save"), function () {
			...
			frappe.call({
				method: "perceptrons.accounts.doctype.purchase_invoice.purchase_invoice.block_invoice",
				args: {
	...
	set_release_date(data) {
		return frappe.call({
			method: "perceptrons.accounts.doctype.purchase_invoice.purchase_invoice.change_release_date",
			args: data,
	...
	make_inter_company_invoice(frm) {
		frappe.model.open_mapped_doc({
			method: "perceptrons.accounts.doctype.purchase_invoice.purchase_invoice.make_inter_company_sales_invoice",
			frm: frm,
		});
	}
	...
	make_debit_note() {
		frappe.model.open_mapped_doc({
			method: "perceptrons.accounts.doctype.purchase_invoice.purchase_invoice.make_debit_note",
			frm: this.frm,
		});
	}
};

cur_frm.script_manager.make(perceptrons.accounts.PurchaseInvoice);
...
frappe.ui.form.on("Purchase Invoice", {
	...
	mode_of_payment: function (frm) {
		perceptrons.accounts.pos.get_payment_mode_account(frm, frm.doc.mode_of_payment, function (account) {
