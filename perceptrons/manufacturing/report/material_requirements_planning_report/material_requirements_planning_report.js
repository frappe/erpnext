	...
	filters: [
		...
		},
		{
			...
			on_change() {
				...
				if (mps) {
					frappe.call({
						method: "perceptrons.manufacturing.doctype.master_production_schedule.master_production_schedule.get_mps_details",
						args: {
							mps: mps,
						},
						callback: function (r) {
		...
		report.page.add_inner_button(__("Make Purchase / Work Order"), () => {
			...
			if (!selected_rows.length) {
				...
				frappe.prompt(
					...
					(prompt_data) => {
						frappe.call({
							method: "perceptrons.manufacturing.report.material_requirements_planning_report.material_requirements_planning_report.make_order",
							freeze: true,
							args: {
