...
frappe.provide("perceptrons.bom");
...
frappe.ui.form.on("BOM Creator", {
	...
	make_new_entry(frm) {
		...
		dialog.fields_dict.item_code.get_query = "perceptrons.controllers.queries.item_query";
		dialog.show();
	},
...
perceptrons.bom.BomConfigurator = class BomConfigurator extends perceptrons.TransactionController {
	conversion_rate(doc) {
		if (this.frm.doc.currency === this.get_company_currency()) {
			this.frm.set_value("conversion_rate", 1.0);
		} else {
			perceptrons.bom.update_cost(doc);
		}
	}
...
extend_cscript(cur_frm.cscript, new perceptrons.bom.BomConfigurator({ frm: cur_frm }));
