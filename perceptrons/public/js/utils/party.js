...
perceptrons.utils.get_party_details = function (frm, method, args, callback) {
	if (!method) {
	...
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (
			!perceptrons.utils.validate_mandatory(
				frm,
...
perceptrons.utils.add_item = function (frm) {
	if (frm.is_new()) {
...
perceptrons.utils.get_address_display = function (frm, address_field, display_field, is_your_company_address) {
...
perceptrons.utils.set_taxes_from_address = function (
	frm,
	...
	frappe.call({
		method: "perceptrons.accounts.party.get_address_tax_category",
		args: {
		...
		callback: function (r) {
			if (!r.exc) {
				if (frm.doc.tax_category != r.message) {
					...
					perceptrons.utils.set_taxes(frm, triggered_from_field);
				}
			}
		},
	});
...
perceptrons.utils.set_taxes = function (frm, triggered_from_field) {
	if (frappe.meta.get_docfield(frm.doc.doctype, "taxes")) {
		if (!perceptrons.utils.validate_mandatory(frm, "Company", frm.doc.company, triggered_from_field)) {
			return;
		}
	...
	frappe.call({
		method: "perceptrons.accounts.party.set_taxes",
		args: {
			party: party,
			party_type: party_type,
...
perceptrons.utils.get_contact_details = function (frm) {
	if (frm.updating_party_details) return;
...
perceptrons.utils.get_employee_contact_details = function (frm) {
	...
	frappe.call({
		method: "perceptrons.setup.doctype.employee.employee.get_contact_details",
...
perceptrons.utils.validate_mandatory = function (frm, label, value, trigger_on) {
	if (!value) {
		frm.doc[trigger_on] = "";
...
perceptrons.utils.get_shipping_address = function (frm, callback) {
	if (frm.doc.company) {
		if (
		...
		frappe.call({
			method: "perceptrons.accounts.custom.address.get_shipping_address",
			args: {
				company: frm.doc.company,
