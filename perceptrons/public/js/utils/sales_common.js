...
frappe.provide("perceptrons.selling");

perceptrons.sales_common = {
	setup_selling_controller: function () {
		perceptrons.selling.SellingController = class SellingController extends perceptrons.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], perceptrons.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", perceptrons.queries.contact_query);
				me.frm.set_query("company_contact_person", perceptrons.queries.company_contact_query);
				me.frm.set_query("customer_address", perceptrons.queries.address_query);
				me.frm.set_query("shipping_address_name", perceptrons.queries.address_query);
				me.frm.set_query("dispatch_address_name", perceptrons.queries.dispatch_address_query);
				me.frm.set_query("company_address", perceptrons.queries.company_address_query);
			...
			set_default_company_address() {
				...
				if (this.frm.doc.company) {
					frappe.call({
						method: "perceptrons.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			customer_address() {
				perceptrons.utils.get_address_display(this.frm, "customer_address");
			...
			shipping_address_name() {
				perceptrons.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				perceptrons.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				perceptrons.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			project(doc, cdt, cdn) {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						frappe.call({
							method: "perceptrons.projects.doctype.project.project.get_cost_center_name",
