...
perceptrons.TransactionController = class TransactionController extends perceptrons.taxes_and_totals {
	setup() {
		super.setup();
		let me = this;
		this.barcode_scanner = new perceptrons.utils.BarcodeScanner({ frm: this.frm });
		...
		frappe.ui.form.on(this.frm.doctype + " Item", "rate", function (frm, cdt, cdn) {
			...
			if (item.item_code && item.rate) {
				frappe.call({
					method: "perceptrons.stock.get_item_details.get_item_tax_template",
					args: {
						ctx: {
		...
		frappe.ui.form.on(this.frm.doctype + " Item", {
			items_add: function (frm, cdt, cdn) {
				...
				perceptrons.accounts.dimensions.copy_dimension_from_first_row(frm, cdt, cdn, "items");
			},
		});
		...
		if (this.frm.fields_dict["items"].grid.get_field("uom")) {
			this.frm.set_query("uom", "items", function (doc, cdt, cdn) {
				...
				return {
					query: "perceptrons.controllers.queries.get_item_uom_query",
					filters: {
		...
		if (this.frm.fields_dict["items"].grid.get_field("blanket_order")) {
			this.frm.set_query("blanket_order", "items", function (doc, cdt, cdn) {
				...
				return {
					query: "perceptrons.controllers.queries.get_blanket_orders",
					filters: {
						company: doc.company,
	...
	toggle_enable_for_stock_uom(field) {
		frappe.call({
			method: "perceptrons.stock.doctype.stock_settings.stock_settings.get_enable_stock_uom_editing",
			callback: (r) => {
				if (r.message) {
	...
	make_payment_request() {
		...
		frappe.call({
			method: "perceptrons.accounts.doctype.payment_request.payment_request.make_payment_request",
			args: {
	...
	make_payment_request_with_schedule = async function () {
		...
		const { message: schedules } = await frappe.call({
			method: "perceptrons.accounts.doctype.payment_request.payment_request.get_available_payment_schedules",
			args: {
	...
	refresh() {
		...
		perceptrons.hide_company(this.frm);
		this.set_dynamic_labels();
		this.setup_sms();
		this.setup_quality_inspection();
		this.validate_has_items();
		perceptrons.utils.view_serial_batch_nos(this.frm);
	...
	barcode(doc, cdt, cdn) {
		...
		if (row.barcode && !frappe.flags.trigger_from_barcode_scanner) {
			perceptrons.stock.utils.set_item_details_using_barcode(this.frm, row, (r) => {
	...
	apply_default_taxes() {
		...
		if (taxes_and_charges_field) {
			return frappe.call({
				method: "perceptrons.controllers.accounts_controller.get_default_taxes_and_charges",
				args: {
	...
	send_sms() {
		var sms_man = new perceptrons.SMSManager(this.frm.doc);
	}
		...
		if (item.item_code || item.serial_no) {
			if (!this.validate_company_and_party()) {
				...
				return this.frm.call({
					method: "perceptrons.stock.get_item_details.get_item_details",
					child: item,
					args: {
						doc: me.frm.doc,
						ctx: {
		...
		frappe.run_serially([
			...
			() => {
				if (show_batch_dialog && !frappe.flags.hide_serial_batch_dialog && !frappe.flags.dialog_set) {
					...
					perceptrons.show_serial_batch_selector(
						me.frm,
						item,
						(item) => {
	...
	get_incoming_rate(item, posting_date, posting_time, voucher_type, company) {
		...
		frappe.call({
			method: "perceptrons.stock.utils.get_incoming_rate",
			args: {
				args: item_args,
			},
			callback: function (r) {
	...
	company() {
		...
		var set_party_account = function (set_pricing) {
			if (["Sales Invoice", "Purchase Invoice"].includes(me.frm.doc.doctype)) {
				...
				if (
					...
					return frappe.call({
						method: "perceptrons.accounts.party.get_party_account",
						args: {
							company: me.frm.doc.company,
		...
		if (
			...
			if (!is_drop_ship) {
				perceptrons.utils.get_shipping_address(this.frm, function () {
		...
		if (this.frm.doc.company) {
			perceptrons.last_selected_company = this.frm.doc.company;
		}
	}
	...
	posting_date() {
		...
		if (this.frm.doc.posting_date) {
			...
			if (
				...
				return frappe.call({
					method: "perceptrons.accounts.party.get_due_date",
					args: {
	...
	cost_center(doc, cdt, cdn) {
		perceptrons.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "cost_center");
	}
	...
	get_company_currency() {
		return perceptrons.get_currency(this.frm.doc.company);
	}

	contact_person() {
		perceptrons.utils.get_contact_details(this.frm);
	}
	...
	conversion_rate() {
		...
		this.frm.set_df_property("conversion_rate", "read_only", perceptrons.stale_rate_allowed() ? 0 : 1);
	}
	...
	get_exchange_rate(transaction_date, from_currency, to_currency, callback) {
		...
		return frappe.call({
			method: "perceptrons.setup.utils.get_exchange_rate",
			args: {
	...
	uom(doc, cdt, cdn) {
		...
		if (item.item_code && item.uom) {
			return this.frm.call({
				method: "perceptrons.stock.get_item_details.get_conversion_factor",
				args: {
					item_code: item.item_code,
					uom: item.uom,
				},
	...
	service_start_date(frm, cdt, cdn) {
		...
		if (child.service_start_date) {
			frappe.call({
				method: "perceptrons.stock.get_item_details.calculate_service_end_date",
				args: { ctx: child },
				callback: function (r) {
	...
	batch_no(frm, cdt, cdn) {
		...
		if (row.batch_no) {
			...
			frappe.call({
				method: "perceptrons.stock.get_item_details.get_batch_based_item_price",
				args: {
					pctx: params,
					item_code: row.item_code,
				},
	...
	ignore_pricing_rule() {
		if (this.frm.doc.ignore_pricing_rule) {
			...
			return this.frm.call({
				method: "perceptrons.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rules",
	...
	remove_pricing_rule_for_item(item) {
		...
		if (item.pricing_rules) {
			let me = this;
			return this.frm.call({
				method: "perceptrons.accounts.doctype.pricing_rule.pricing_rule.remove_pricing_rule_for_item",
				args: {
	...
	apply_pricing_rule(item, calculate_taxes_and_totals) {
		...
		return this.frm.call({
			method: "perceptrons.accounts.doctype.pricing_rule.pricing_rule.apply_pricing_rule",
	...
	apply_price_list(item, reset_plc_conversion) {
		...
		// `perceptrons.stock.get_item_details.apply_price_list` is sensitive to its value
		...
		return this.frm
			.call({
				method: "perceptrons.stock.get_item_details.apply_price_list",
	...
	get_terms() {
		...
		perceptrons.utils.get_terms(this.frm.doc.tc_name, this.frm.doc, function (r) {
			if (!r.exc) {
	...
	taxes_and_charges() {
		...
		if (this.frm.doc.taxes_and_charges) {
			return this.frm.call({
				method: "perceptrons.controllers.accounts_controller.get_taxes_and_charges",
				args: {
	...
	tax_category() {
		...
		frappe.run_serially([
			...
			() => perceptrons.utils.set_taxes(this.frm, "tax_category"),
		]);
	}
	...
	update_item_tax_map() {
		...
		if (item_codes.length) {
			return this.frm.call({
				method: "perceptrons.stock.get_item_details.get_item_tax_info",
				args: {
					doc: me.frm.doc,
	...
	item_tax_template(doc, cdt, cdn) {
		...
		if (item.item_tax_template) {
			return this.frm.call({
				method: "perceptrons.stock.get_item_details.get_item_tax_map",
				args: {
					doc: me.frm.doc,
	...
	setup_item_selector() {
		...
		// 	this.item_selector = new perceptrons.ItemSelector({frm: this.frm});
		// }
	}
	...
	make_quality_inspection() {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function () {
				...
				frappe.call({
					method: "perceptrons.controllers.stock_controller.make_quality_inspections",
					args: {
						doctype: me.frm.doc.doctype,
		...
		frappe.call({
			method: "perceptrons.controllers.stock_controller.check_item_quality_inspection",
			args: {
	...
	get_method_for_payment() {
		let method = "perceptrons.accounts.doctype.payment_entry.payment_entry.get_payment_entry";
		if (this.frm.doc.__onload && this.frm.doc.__onload.make_payment_via_journal_entry) {
			if (["Sales Invoice", "Purchase Invoice"].includes(this.frm.doc.doctype)) {
				method =
					"perceptrons.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_invoice";
			} else {
				method =
					"perceptrons.accounts.doctype.journal_entry.journal_entry.get_payment_entry_against_order";
			}
		}
	...
	set_query_for_batch(doc, cdt, cdn) {
		...
		if (!item.item_code) {
			...
			return {
				query: "perceptrons.controllers.queries.get_batch_no",
				filters: filters,
			};
		}
	}
	...
	set_query_for_item_tax_template(doc, cdt, cdn) {
		...
		if (!item.item_code) {
			...
			return {
				query: "perceptrons.controllers.queries.get_tax_template",
				filters: filters,
			};
		}
	}
	...
	payment_terms_template() {
		...
		if (doc.payment_terms_template && doc.doctype !== "Delivery Note" && !doc.is_return) {
			...
			frappe.call({
				method: "perceptrons.controllers.accounts_controller.get_payment_terms",
				args: {
	...
	payment_term(doc, cdt, cdn) {
		...
		if (row.payment_term) {
			frappe.call({
				method: "perceptrons.controllers.accounts_controller.get_payment_term_details",
				args: {
	...
	blanket_order(doc, cdt, cdn) {
		...
		if (item.blanket_order && (item.parenttype == "Sales Order" || item.parenttype == "Purchase Order")) {
			frappe.call({
				method: "perceptrons.stock.get_item_details.get_blanket_order_details",
				args: {
					ctx: {
						item_code: item.item_code,
	...
	setup_accounting_dimension_triggers() {
		frappe.call({
			method: "perceptrons.accounts.doctype.accounting_dimension.accounting_dimension.get_dimensions",
			callback: function (r) {
				if (r.message && r.message[0]) {
					...
					dimensions.forEach((dim) => {
						...
						cur_frm.cscript[dim] = function (doc, cdt, cdn) {
							perceptrons.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", dim);
						};
					});
				}
			},
...
perceptrons.show_serial_batch_selector = function (frm, item_row, callback, on_close, show_dialog) {
	...
	new perceptrons.SerialBatchPackageSelector(frm, item_row, (r) => {
		if (r) {
			let update_values = {
...
perceptrons.apply_putaway_rule = (frm, purpose = null) => {
	if (!frm.doc.company) {
	...
	frappe.call({
		method: "perceptrons.stock.doctype.putaway_rule.putaway_rule.apply_putaway_rule",
		args: {
			doctype: frm.doctype,
			items: frm.doc.items,
			company: frm.doc.company,
...
perceptrons.set_unit_price_items_note = (frm) => {
