...
// get tax rate
frappe.provide("perceptrons.taxes");

perceptrons.accounts.taxes = {
			...
			onload: function (frm) {
				if (frm.get_field("taxes")) {
					frm.set_query("account_head", "taxes", function (doc) {
						...
						return {
							query: "perceptrons.controllers.queries.tax_account_query",
							filters: {
		...
		frappe.ui.form.on(doctype, {
			account_head: function (frm, cdt, cdn) {
				...
				if (!d.charge_type && d.account_head) {
					...
					frappe.call({
						type: "GET",
						method: "perceptrons.controllers.accounts_controller.get_tax_rate",
...
perceptrons.accounts.payment_triggers = {
...
perceptrons.accounts.pos = {
	setup: function (doctype) {
		frappe.ui.form.on(doctype, {
			mode_of_payment: function (frm, cdt, cdn) {
				var d = locals[cdt][cdn];
				perceptrons.accounts.pos.get_payment_mode_account(frm, d.mode_of_payment, function (account) {
	...
	get_payment_mode_account: function (frm, mode_of_payment, callback) {
		...
		return frappe.call({
			method: "perceptrons.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
			args: {
