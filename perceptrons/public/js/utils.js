...
frappe.provide("perceptrons.utils");
frappe.provide("perceptrons.stock.utils");
...
$.extend(perceptrons, {
	...
	hide_company: function (frm) {
		if (frm?.fields_dict.company) {
			...
			} else if (perceptrons.last_selected_company) {
				if (!frm.doc.company) frm.set_value("company", perceptrons.last_selected_company);
			}
		}
	},
...
$.extend(perceptrons.utils, {
	set_party_dashboard_indicators: function (frm) {
		if (frm.doc.__onload && frm.doc.__onload.dashboard_info) {
			var company_wise_info = frm.doc.__onload.dashboard_info;
			if (company_wise_info.length > 1) {
				company_wise_info.forEach(function (info) {
					perceptrons.utils.add_indicator_for_multicompany(frm, info);
				});
	...
	get_terms: function (tc_name, doc, callback) {
		if (tc_name) {
			return frappe.call({
				method: "perceptrons.setup.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions",
				args: {
	...
	add_inventory_dimensions: function (report_name, index) {
		...
		frappe.call({
			method: "perceptrons.stock.doctype.inventory_dimension.inventory_dimension.get_inventory_dimensions",
			callback: function (r) {
	...
	get_fiscal_year: function (date, with_dates = false, raise_on_missing = true) {
		...
		if (
			...
			frappe.call({
				method: "perceptrons.accounts.utils.get_fiscal_year",
...
perceptrons.utils.select_alternate_items = function (opts) {
	const frm = opts.frm;
			...
			{ fieldtype: "Section Break", label: __("Items") },
			{
				...
				fields: [
					...
					},
					{
						...
						get_query: (e) => {
							return {
								query: "perceptrons.stock.doctype.item_alternative.item_alternative.get_alternative_items",
								filters: {
...
perceptrons.utils.update_child_items = function (opts) {
	const frm = opts.frm;
	...
	let dialog = new frappe.ui.Dialog({
		...
		update_items: function () {
			...
			frappe.call({
				method: "perceptrons.controllers.accounts_controller.update_child_qty_rate",
				freeze: true,
				args: {
...
perceptrons.utils.map_current_doc = function (opts) {
	function _map() {
	...
	$.each(frappe.boot.service_level_agreement_doctypes, function (_i, d) {
		frappe.ui.form.on(d, {
			onload: function (frm) {
				...
				frappe.call({
					method: "perceptrons.support.doctype.service_level_agreement.service_level_agreement.get_service_level_agreement_filters",
					args: {
						doctype: frm.doc.doctype,
...
$.extend(perceptrons.stock.utils, {
	set_item_details_using_barcode(frm, child_row, callback) {
		const barcode_scanner = new perceptrons.utils.BarcodeScanner({ frm: frm });
