...
perceptrons.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new perceptrons.accounts.NumberCard(summary);
...
perceptrons.accounts.NumberCard = class NumberCard {
	constructor(options) {
