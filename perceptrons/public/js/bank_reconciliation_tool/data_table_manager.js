frappe.provide("perceptrons.accounts.bank_reconciliation");

perceptrons.accounts.bank_reconciliation.DataTableManager = class DataTableManager {
	constructor(opts) {
		Object.assign(this, opts);
		this.dialog_manager = new perceptrons.accounts.bank_reconciliation.DialogManager(
			this.company,
			this.bank_account,
	...
	make_dt() {
		const me = this;
		frappe.call({
			method: "perceptrons.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_bank_transactions",
			args: {
	...
	get_cleared_balance() {
		if (this.bank_account && this.bank_statement_to_date) {
			return frappe.call({
				method: "perceptrons.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool.get_account_balance",
				args: {
					bank_account: this.bank_account,
