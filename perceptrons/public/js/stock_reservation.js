frappe.provide("perceptrons.stock_reservation");

$.extend(perceptrons.stock_reservation, {
	make_entries(frm, table_name) {
		perceptrons.stock_reservation.setup(frm, table_name);
	},

	setup(frm, table_name) {
		let parms = perceptrons.stock_reservation.get_parms(frm, table_name);

		perceptrons.stock_reservation.dialog = new frappe.ui.Dialog({
			title: __("Stock Reservation"),
			size: "extra-large",
			fields: perceptrons.stock_reservation.get_dialog_fields(frm, parms),
			primary_action_label: __("Reserve Stock"),
			primary_action: () => {
				perceptrons.stock_reservation.reserve_stock(frm, table_name, parms);
			},
		});

		perceptrons.stock_reservation.render_items(frm, parms);
	},
	...
	get_parms(frm, table_name) {
		...
		params["method"] = {
			"Sales Order": "delivered_qty",
			"Production Plan":
				"perceptrons.manufacturing.doctype.production_plan.production_plan.make_stock_reservation_entries",
			"Work Order":
				"perceptrons.manufacturing.doctype.work_order.work_order.make_stock_reservation_entries",
		}[frm.doc.doctype];
	...
	get_dialog_fields(frm, parms) {
		let fields = perceptrons.stock_reservation.fields || [];
		let qty_field = parms.qty_field;
		let dialog = perceptrons.stock_reservation.dialog;
	...
	unreserve_stock(frm) {
		perceptrons.stock_reservation.get_stock_reservation_entries(frm.doctype, frm.docname).then((r) => {
			if (!r.exc && r.message) {
				if (r.message.length > 0) {
					perceptrons.stock_reservation.prepare_for_cancel_sre_entries(frm, r.message);
				} else {
			...
			fields: [
				{
					...
					in_place_edit: true,
					data: [],
					fields: perceptrons.stock_reservation.get_fields_for_cancel(),
				},
			],
			primary_action_label: __("Unreserve Stock"),
			primary_action: () => {
				perceptrons.stock_reservation.cancel_stock_reservation(dialog, frm);
			},
		});
	...
	cancel_stock_reservation(dialog, frm) {
		...
		let method = "perceptrons.manufacturing.doctype.work_order.work_order.cancel_stock_reservation_entries";

		if (frm.doc.doctype === "Production Plan") {
			method =
				"perceptrons.manufacturing.doctype.production_plan.production_plan.cancel_stock_reservation_entries";
		}
