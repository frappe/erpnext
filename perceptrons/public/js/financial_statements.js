frappe.provide("perceptrons.financial_statements");
...
perceptrons.financial_statements = {
	filters: get_filters(),
	baseData: null,
	...
	formatter: function (value, row, column, data, default_formatter, filter) {
		...
		// Growth/Margin
		if (perceptrons.financial_statements._is_special_view(column, data))
			return perceptrons.financial_statements._format_special_view(...report_params);

		if (get_filter_value("report_template"))
			return perceptrons.financial_statements._format_custom_report(...report_params);
		else return perceptrons.financial_statements._format_standard_report(...report_params);
	},
	...
	_format_custom_report: function (value, row, column, data, default_formatter, filter) {
		const columnInfo = perceptrons.financial_statements._parse_column_info(column.fieldname, data);
		const formatting = perceptrons.financial_statements._get_formatting_for_column(data, columnInfo);

		if (columnInfo.isAccount) {
			return perceptrons.financial_statements._format_custom_account_column(
				value,
				data,
				formatting,
				column,
			...
			return perceptrons.financial_statements._format_custom_value_column(
				value,
				data,
				formatting,
				column,
	...
	_parse_column_info: function (fieldname, data) {
		...
		return {
			isAccount: baseName === perceptrons.financial_statements.name_field,
	...
	_format_custom_account_column: function (value, data, formatting, column, default_formatter, row) {
		...
		if (should_link_to_ledger) {
			...
			column.link_onclick =
				"perceptrons.financial_statements.open_general_ledger(" + JSON.stringify(glData) + ")";
		...
		// Style
		return perceptrons.financial_statements._style_custom_value(formattedValue, formatting, null);
	},
	...
	_format_custom_value_column: function (value, data, formatting, column, default_formatter, row) {
		...
		return perceptrons.financial_statements._style_custom_value(formattedValue, formatting, value);
	},
	...
	_format_standard_report: function (value, row, column, data, default_formatter, filter) {
		if (data && column.fieldname == perceptrons.financial_statements.name_field) {
			...
			if (data.account || data.accounts) {
				column.link_onclick =
					"perceptrons.financial_statements.open_general_ledger(" + JSON.stringify(data) + ")";
			}
			column.is_tree = true;
		}
	...
	onload: function (report) {
		...
		perceptrons.financial_statements.filters = get_filters();

		let fiscal_year = perceptrons.utils.get_fiscal_year(frappe.datetime.get_today());
...
function get_filters() {
	let filters = [
		...
		// accordingly.
		{
			...
			label: __("Currency"),
			fieldtype: "Select",
			options: perceptrons.get_presentation_currency_list(),
		},
		{
	...
	let fiscal_year = perceptrons.utils.get_fiscal_year(frappe.datetime.get_today(), false, false);
	if (fiscal_year) {
