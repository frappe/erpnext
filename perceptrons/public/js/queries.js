...
frappe.provide("perceptrons.queries");
$.extend(perceptrons.queries, {
	user: function () {
		return { query: "frappe.core.doctype.user.user.user_query" };
	},

	lead: function () {
		return { query: "perceptrons.controllers.queries.lead_query" };
	},

	item: function (filters) {
		var args = { query: "perceptrons.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "perceptrons.controllers.queries.bom" };
	},

	task: function () {
		return { query: "perceptrons.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "perceptrons.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "perceptrons.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
perceptrons.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
perceptrons.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = perceptrons.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "perceptrons.controllers.queries.warehouse_query" });
