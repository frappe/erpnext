frappe.provide("perceptrons.setup");
...
frappe.setup.on("before_load", function () {
	if (
		...
		frappe.boot.setup_wizard_completed_apps.includes("perceptrons")
	) {
		return;
	}

	perceptrons.setup.slides_settings.map(frappe.setup.add_slide);
});

perceptrons.setup.slides_settings = [
	{
		// Organization
		name: "organization",
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = perceptrons.setup.fiscal_years[country];
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: "perceptrons.accounts.utils.get_coa",
				args: {
...
perceptrons.setup.fiscal_years = {
