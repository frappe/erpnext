...
function fetch_company_details(current_doctype, current_docname) {
	frappe.call({
		method: "perceptrons.controllers.accounts_controller.get_missing_company_details",
...
function save_company_details(dialog, data, values, current_doctype) {
	frappe.call({
		method: "perceptrons.controllers.accounts_controller.update_company_master_and_address",
		args: {
			name: data.name,
