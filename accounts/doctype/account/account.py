...
from webnotes.utils import flt, fmt_money, cstr, cint, getdate, add_days, formatdate
...
class DocType:
	...
	def validate_due_date(self, posting_date, due_date):
		credit_days = (self.doc.credit_days or
			webnotes.conn.get_value("Company", self.doc.company, "credit_days"))
		...
		if diff < 0:
			...
			is_credit_controller = webnotes.conn.get_value("Accounts Settings", None,
			...
			if is_credit_controller:
				...
				webnotes.throw(_("Due Date cannot be after {0}").format(max_due_date))
